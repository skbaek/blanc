...
2. **Query the Goal State:** Do not guess or hallucinate the goal. Call the `lean_goal` tool (MCP server `lean-lsp`, full tool name `mcp__lean-lsp__lean_goal`), passing the `file_path`, `line`, and `column` you just located.
