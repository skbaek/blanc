...
2. **Query the Goal State:** Do not guess or hallucinate the goal. Call the `lean_goal` MCP tool (provided by the `lean-lsp-mcp` server), passing the `file_path`, `line`, and `character` you just located.
