...
/-- **The callback, and the induction hypothesis.**  `flashLoan` is fmint's
only target that consumes the deeper-frame hypothesis.

The walk itself is `ContractSpec.ofStorageOnly_of_call`
(`Blanc/StorageOnlySpec.lean`), hoisted there once the WETH-backed PRORATA
vault became its second consumer: its four ERC-4626 flows each call WETH, and
each needs exactly this lemma at its own invariant. -/
