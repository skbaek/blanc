...
def recipes : List Recipe := [
  ...
  },
  {
    ...
    preferredPath := "Use the `ForwardCall` module and the live `ExecSat`/`ExecWitness` layer to cross calls or package multiple outcomes. At a proof-carrying `DELEGATECALL` boundary, use `DelegatecallSpawnDescriptor.step` for the exact spawn and `.crossing` for the entered child's successful continuation instead of reconstructing the descriptor fields at the consumer."
    symbols := ["module:Blanc/ForwardCall.lean", "module:Blanc/DelegatecallEnvelope.lean", "declaration:Func.ExecSat", "declaration:Prog.ExecSat", "declaration:Func.ExecWitness", "declaration:Prog.ExecWitness", "declaration:DelegatecallSpawnDescriptor.step", "declaration:DelegatecallSpawnDescriptor.crossing"]
