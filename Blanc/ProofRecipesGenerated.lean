...
def recipes : List Recipe := [
  ...
  },
  {
    ...
    preferredPath := "When two expensive bodies repeat the same post-kernel walk, freeze a committed-row decision rule, factor the body-from-kernel boundary, preserve the old statements as instantiations, profile the generic and instances, run the bare elaboration gate, withhold the generic in an isolated falsifier, and reject any candidate that adds a proof-resource ceiling before landing."
    boundary := "G1: deleting 687 exact proof-copy lines moved `PauseWorldRunKit` 23.536 to 23.273 s (-1.1%) and raised `RegistrySubstrate` 17.513 to 26.909 s, so exact source duplication alone is not a timing signal. G2: the private `foundZeroOldLast`/`foundZeroOldLastSwapPop` body summary moved `UnregisterRegistration` 38.482 to 21.610 s (-43.8%), but without a new `maxRecDepth` scope it failed at the 24-step walk; a split after static-argument validation still failed in the canonical-address tail, so the no-escalation invariant refuted it and the proof edit was reverted. G3: the real `ReplacementRegistration.oldLastNonzero` body fell below 0.5 s and its module reached 21.433 s, but required shared placement made `RegistrySubstrate` 35.288 s (>2x baseline), so that extension is refuted until independent shared-ancestor headroom exists. G4: the Registry tail is sparse and kernel/defeq-owned, not a compression family. G5: `PauseWorldRun` (8.995 s), the WETH accounting pair (25.573 s combined), and the rich/local twin (4.424 s) are maintainability-only; source symmetry does not license a timing retrofit."
  },
  {
