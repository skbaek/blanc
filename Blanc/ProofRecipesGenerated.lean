...
def recipes : List Recipe := [
  ...
  },
  {
    ...
    preferredPath := "Use `MessageExecution.processMessage_eq_settle_exec` for the raw-to-settled bridge. For payable calls, use the `*_afterTransfer_of_codeEntry` adapters and derive ordinary non-precompile entry with `executeCode_enter_of_codeAddress_not_precompile`; this works for normal `disablePrecompiles = false` messages. Then use the clean/revert/halt adapters and canonical `settledRevert` or `settledHalt` machines instead of unfolding message settlement at the contract site. For an already-retained `ProcessMessage`, use `processMessage_clean_rawPost` to recover the successful raw post, `processMessage_entry_facts` for the actual entry frame projections, and the separate `processMessage_entry_stack` / `processMessage_entry_memory` empty-entry projections."
    symbols := ["module:Blanc/MessageExecution.lean", "module:Blanc/MessageExecutionInversion.lean", "declaration:MessageExecution.processMessage_eq_settle_exec", "declaration:MessageExecution.processMessage_eq_settle_exec_afterTransfer_of_codeEntry", "declaration:MessageExecution.processMessage_clean_of_exec_afterTransfer_of_codeEntry", "declaration:MessageExecution.executeCode_enter_of_codeAddress_not_precompile", "declaration:MessageExecution.processMessage_clean_of_exec", "declaration:MessageExecution.processMessage_revert_of_exec", "declaration:MessageExecution.processMessage_halt_of_exec", "declaration:MessageExecution.processMessage_clean_rawPost", "declaration:MessageExecution.processMessage_entry_facts", "declaration:MessageExecution.processMessage_entry_stack", "declaration:MessageExecution.processMessage_entry_memory", "declaration:MessageExecution.settledRevert", "declaration:MessageExecution.settledHalt", "declaration:Msg.initDevm_stack", "declaration:Msg.initSevm_data"]
    boundary := "The simplest forward bridge requires entry-state identity and disabled precompiles. The post-transfer exact-code-entry bridge instead accepts the interpreter-entry equation, which ordinary messages obtain from a non-precompile code address without changing `disablePrecompiles`. The retained-frame inversion exposes storage equality rather than whole-state equality because value transfer may change balances. These facts describe ordinary call-message settlement, not CREATE settlement."
  },
  {
