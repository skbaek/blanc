...
def recipes : List Recipe := [
  ...
  },
  {
    ...
    preferredPath := "Use `Bytes.sliceD_writeAt` for the written window, `Bytes.sliceD_writeAt_before` or `Bytes.sliceD_writeAt_after` for disjoint neighboring windows, and `Bytes.readWord_writeAt_self` or `Bytes.readWord_writeAt_of_disjoint` for word reads. For adjacent event words use `Bytes.read_two_word_writes_at` or `Mem.read_two_word_writes_at`. For padded or abstract memory, start from `Mem.Wf` and `Mem.Reads`; in scratch decoders, `of_run_mstoreAt_image` and `of_run_loadWordAt_image` advance the stack, image, well-formedness, and state equation together, while `of_run_loadWordAt_logs` preserves event chronology. For creation copies use `of_run_codecopy_image`; its lower-level components are `prefix_of_codecopy_val` plus `of_run_codecopy_logs`. For fixed logs use `of_logWith_image`. Keep compiled-emitter `List.drop` equalities local unless a profile proves that a structural helper moves their kernel cost."
    symbols := ["module:Blanc/CommonProofs.lean", "declaration:Bytes.sliceD_writeAt", "declaration:Bytes.sliceD_writeAt_before", "declaration:Bytes.sliceD_writeAt_after", "declaration:Bytes.readWord_writeAt_self", "declaration:Bytes.readWord_writeAt_of_disjoint", "declaration:Bytes.read_two_word_writes_at", "declaration:Mem.Wf", "declaration:Mem.Reads", "declaration:Mem.read_two_word_writes_at", "declaration:of_run_mstoreAt_image", "declaration:of_run_loadWordAt_image", "declaration:of_run_loadWordAt_logs", "declaration:of_run_codecopy_image", "declaration:prefix_of_codecopy_val", "declaration:of_run_codecopy_logs", "declaration:of_logWith_image"]
  ...
  },
  {
    ...
    preferredPath := "Use `MessageExecution.processMessage_eq_settle_exec_of_enter` when an exact successful `Frame.enter` equation is already retained, including for delegated children. Use `processMessage_eq_settle_exec_afterTransfer` when value transfer prepared a distinct entry environment and precompiles are disabled, then its clean/revert/halt `_afterTransfer` adapters and canonical `settledRevert` or `settledHalt` machines instead of unfolding message settlement at the contract site. Use the unsuffixed `processMessage_eq_settle_exec` and outcome adapters for the identity-entry specialization. For an already-retained `ProcessMessage`, use `processMessage_clean_rawPost` to recover the successful raw post, `processMessage_entry_facts` for the actual entry frame projections, and the separate `processMessage_entry_stack` / `processMessage_entry_memory` empty-entry projections."
    symbols := ["module:Blanc/MessageExecution.lean", "module:Blanc/MessageExecutionInversion.lean", "declaration:MessageExecution.processMessage_eq_settle_exec_of_enter", "declaration:MessageExecution.processMessage_eq_settle_exec_afterTransfer", "declaration:MessageExecution.processMessage_eq_settle_exec", "declaration:MessageExecution.processMessage_clean_of_exec_afterTransfer", "declaration:MessageExecution.processMessage_clean_of_exec", "declaration:MessageExecution.processMessage_revert_of_exec_afterTransfer", "declaration:MessageExecution.processMessage_revert_of_exec", "declaration:MessageExecution.processMessage_halt_of_exec_afterTransfer", "declaration:MessageExecution.processMessage_halt_of_exec", "declaration:MessageExecution.processMessage_clean_rawPost", "declaration:MessageExecution.processMessage_entry_facts", "declaration:MessageExecution.processMessage_entry_stack", "declaration:MessageExecution.processMessage_entry_memory", "declaration:MessageExecution.settledRevert", "declaration:MessageExecution.settledHalt", "declaration:Msg.initDevm_stack", "declaration:Msg.initSevm_data"]
    boundary := "The generic enter bridge requires only the exact successful entry equation. The post-transfer bridge additionally names the successful post-transfer environment and requires disabled precompiles; the unsuffixed specialization also requires entry-state identity. The retained-frame inversion exposes storage equality rather than whole-state equality because value transfer may change balances. These facts describe ordinary call-message settlement, not CREATE settlement."
  },
  {
  ...
  },
  {
    ...
    preferredPath := "Before proving a record projection by `rfl`, search the public `Devm` laws in `CommonProofs`: memory writes, accessed-storage/setMach cancellation, storage read-after-write, and the reusable RETURN/SSTORE post projection cuts are named there. For Solidity address-typed storage, use `addressSlotReadWord`/`addressSlotWriteWord` and the value-carrying `loadAddressWordAt`/`storeAddressWordAt` inversions rather than restating mask algebra locally. Jaune also supplies update-first projection laws such as `Devm.memWrite_gasLeft` and `Devm.setMach_accessedStorageKeys`."
    symbols := ["module:Blanc/CommonProofs.lean", "declaration:Devm.memWrite_memory", "declaration:Devm.memWrite_stack", "declaration:Devm.addAccessedStorageKey_setMach_setMach", "declaration:Devm.getStorVal_setStorVal_self", "declaration:Devm.retPost_getStorVal", "declaration:Devm.sstoreBase_state", "module:Blanc/AddressSlot.lean", "module:Blanc/AddressSlotProofs.lean", "declaration:addressSlotReadWord", "declaration:addressSlotWriteWord", "declaration:addressSlotReadWord_eq_toAdr_toB256", "declaration:of_loadAddressWordAt_val", "declaration:of_storeAddressWordAt_val"]
