...
def recipes : List Recipe := [
  ...
  },
  {
    ...
    preferredPath := "Use the `ForwardCall` module and the live `ExecSat`/`ExecWitness` layer to cross calls or package multiple outcomes. At a proof-carrying `DELEGATECALL` boundary, use `DelegatecallSpawnDescriptor.step` for the exact spawn and `.crossing` for the entered child's successful continuation instead of reconstructing the descriptor fields at the consumer. When an existing compiled call step has already resumed, use `DelegatecallSpawnDescriptor.settled_of_runCompiled` to obtain a `DelegatecallSettledBoundary` with the exact retained child and parent observations."
    symbols := ["module:Blanc/ForwardCall.lean", "module:Blanc/DelegatecallEnvelope.lean", "declaration:Func.ExecSat", "declaration:Prog.ExecSat", "declaration:Func.ExecWitness", "declaration:Prog.ExecWitness", "declaration:DelegatecallSpawnDescriptor.step", "declaration:DelegatecallSpawnDescriptor.crossing", "declaration:DelegatecallSettledBoundary", "declaration:DelegatecallSpawnDescriptor.settled_of_runCompiled"]
  ...
  },
  {
    ...
    preferredPath := "Use `Bytes.sliceD_writeAt` for the written window, `Bytes.sliceD_writeAt_before` or `Bytes.sliceD_writeAt_after` for disjoint neighboring windows, and `Bytes.readWord_writeAt_self` or `Bytes.readWord_writeAt_of_disjoint` for word reads. For padded or abstract memory, start from `Mem.Wf` and `Mem.Reads`; in scratch decoders, `of_run_mstoreAt_image` and `of_run_loadWordAt_image` advance the stack, image, well-formedness, and state equation together. Keep compiled-emitter `List.drop` equalities local unless a profile proves that a structural helper moves their kernel cost."
    symbols := ["module:Blanc/CommonProofs.lean", "declaration:Bytes.sliceD_writeAt", "declaration:Bytes.sliceD_writeAt_before", "declaration:Bytes.sliceD_writeAt_after", "declaration:Bytes.readWord_writeAt_self", "declaration:Bytes.readWord_writeAt_of_disjoint", "declaration:Mem.Wf", "declaration:Mem.Reads", "declaration:of_run_mstoreAt_image", "declaration:of_run_loadWordAt_image"]
