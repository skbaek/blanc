...
private theorem stageCopy_runCompiled
    ...
    Func.RunCompiled fs sevm
      ...
      ((((base.setMach
          ⟨[], memory.write 0 runtimeBytes, G - 418⟩).memRead
            0 2197).2).withOutput runtimeBytes) := by
  ...
  · exact Devm.extCost_add_of_size
      (a := gVerylow + gasCopy * ceilDiv 2197 32) hsize (by decide)
  ...
  have hreturn := stageReturnEnd_runCompiled
    ...
    (G := G - 413) hsizeCopied houtputCopied (by omega)
  have hgasTail : (G - 413) - 5 = G - 418 := by omega
  simpa only [hgasTail] using hreturn
...
private theorem stageMask_complete
  ...
  exact stageCopy_runCompiled
    ...
    hsize2 hcode hruntimeLength hruntimeNonempty (by omega)
  rfl
...
theorem ossifiableConstructorAfterSetup_zeroAdmin_runCompiled
  ...
  rcases htail with ⟨post, htail, houtput⟩
