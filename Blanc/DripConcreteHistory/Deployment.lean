...
theorem concreteDeploymentBase :
    CanonicalDeploymentBase concreteConfig .prague concreteBase
...
theorem concreteCreateValidated :
    validateTransaction pragueRules concreteCreateTx 0 =
      .ok (calculateIntrinsicCost pragueRules concreteCreateTx 0) := by
  decide +kernel

theorem concreteCreateGasBound :
    deploymentTransactionGasBound
      (initBenv .prague concreteBase concreteExecutionHeader)
      concreteCreateTx concreteCreateSender ≤ concreteCreateTx.gas := by
  decide +kernel

theorem concreteCreateChecked :
    checkTransaction (initBenv .prague concreteBase concreteExecutionHeader).beginTransaction
  ...
  have hgas : checkTransactionGasLimits
      (initBenv .prague concreteBase concreteExecutionHeader).beginTransaction
  ...
  change (do
    ...
    let (effective, maxFee) ← Except.mapError TransitionError.transaction
      (checkTransactionGasFee (initBenv .prague concreteBase concreteExecutionHeader).beginTransaction concreteCreateTx)
    let (maxFee, hashes) ← Except.mapError TransitionError.transaction
      (checkTransactionBlobData (initBenv .prague concreteBase concreteExecutionHeader).beginTransaction concreteCreateTx maxFee)
...
theorem concreteCreateGasUsed : deploymentTransactionGasBound
    (initBenv .prague concreteBase concreteExecutionHeader)
    concreteCreateTx concreteCreateSender = 479909 := by
  decide +kernel
...
theorem concreteDeploymentHeader_benv (sr tr rr wr rh : B256) :
    initBenv .prague concreteBase (concreteDeploymentHeader sr tr rr wr rh) =
      initBenv .prague concreteBase concreteExecutionHeader := rfl
...
theorem concreteDeploymentHeader_decode (sr tr rr wr rh : B256) :
  ...
  have h0 (name : String) : (0 : Nat).toBytes.toRlpNat name 32 = .ok 0 := by
    rw [h0b]
    rfl
  have h1 (name : String) : (1 : Nat).toBytes.toRlpNat name 32 = .ok 1 := by
    rw [h1b]
    rfl
  have hl : (10000000 : Nat).toBytes.toRlpNat "header gasLimit" 32 = .ok 10000000 := by
    decide +kernel
  have hg : (479909 : Nat).toBytes.toRlpNat "header gasUsed" 32 = .ok 479909 := by
    decide +kernel
  have hn : (0 : UInt64).toBytes.toRlpFixedB64 "header nonce" = .ok 0 := by
    decide +kernel
  have h64 (name : String) : (0 : Nat).toBytes.toRlpB64 name = .ok 0 := by
    rw [h0b]
    rfl
  refine RlpConcrete.header21_roundtrip
    (header := concreteDeploymentHeader sr tr rr wr rh) (requestsHash := rh)
    (blobGasUsed := 0) (excessBlobGas := 0) ?_ ?_ ?_ ?_ ?_ ?_ ?_ ?_ ?_ ?_ ?_ ?_ ?_ ?_ ?_ ?_
  · rfl
  · rfl
  · rfl
  · exact hc _
...
theorem concreteDeploymentHeader_valid (sr tr rr wr rh : B256) :
  ...
  simp only [validateHeader, concreteBase, concreteGenesisBlock,
    ...
    Header.hash, pragueRules_header, ne_eq, not_true_eq_false, ite_false]
...
theorem concreteCanonicalEnvelope (sr tr rr wr rh : B256) :
    CanonicalDripDeploymentBlock concreteConfig .prague concreteBase
  ...
  refine {
    ...
    ommers_eq := rfl
    withdrawals_eq := rfl
    forkAt := ChainConfig.pragueOnly_forkAt 1 _
    type_eq := ⟨1, 8, rfl⟩
    value_eq := rfl
    data_eq := rfl
    nonce_eq := ?_
  ...
  · change checkTransaction
      (initBenv .prague concreteBase (concreteDeploymentHeader sr tr rr wr rh)).beginTransaction
      (deploymentTxPreludeBout .init concreteCreateTx 0) concreteCreateTx =
      .ok (concreteCreateSender,
        deploymentEffectiveGasPrice (initBenv .prague concreteBase
...
private theorem concreteDeploymentBody_exists : ∃ output : State × BlockOutput,
    applyBody (initBenv .prague concreteBase concreteExecutionHeader)
  ...
  obtain ⟨ctx⟩ := prepareCanonicalDeploymentContext concreteConfig .prague concreteBase
    preview concreteCreateTx concreteCreateSender concreteCreateTarget concreteDeploymentBase henv
    CoveredFork.prague
  obtain ⟨post, bout, htx⟩ := canonicalDeploymentTransaction_succeeds concreteConfig .prague
    concreteBase preview concreteCreateTx concreteCreateSender concreteCreateTarget
    concreteDeploymentBase henv ctx CoveredFork.prague
  obtain ⟨suffix⟩ := canonicalDeploymentSuffix_succeeds concreteConfig .prague concreteBase
    preview concreteCreateTx concreteCreateSender concreteCreateTarget concreteDeploymentBase
    ctx post bout htx CoveredFork.prague
  have hrun := canonicalDeploymentApplyBody_succeeds concreteConfig .prague concreteBase
    preview concreteCreateTxRlp concreteCreateTx concreteCreateSender concreteCreateTarget
    henv CoveredFork.prague ctx post bout htx suffix
  change applyBody (initBenv .prague concreteBase (concreteDeploymentHeader 0 0 0 0 0))
    [.inl concreteCreateTxRlp] [] = .ok (post, bout) at hrun
  rw [concreteDeploymentHeader_benv] at hrun
  have hgas : bout.blockGasUsed = 479909 := by
    rw [htx.blockGasUsed, ctx.systemPrefix.environment_eq]
    exact concreteCreateGasUsed
  exact ⟨(post, bout), hrun, hgas, htx.blockLogs, htx.blobGasUsed⟩
...
theorem concreteDeploymentBody_finalHeader :
    applyBody (initBenv .prague concreteBase concreteDeploymentEnvelope.block.header)
      concreteDeploymentEnvelope.block.txs concreteDeploymentEnvelope.block.wds =
      .ok concreteDeploymentBody := by
  change applyBody (initBenv .prague concreteBase (concreteDeploymentHeader _ _ _ _ _))
    [.inl concreteCreateTxRlp] [] = .ok concreteDeploymentBody
  rw [concreteDeploymentHeader_benv]
...
theorem concreteDeploymentChecks :
  ...
  have hlegacy := applyBody_legacyGasAccounting
    (benv := initBenv .prague concreteBase concreteDeploymentEnvelope.block.header)
    (by rfl) concreteDeploymentBody_finalHeader
  have hstateGas : concreteDeploymentBody.2.blockStateGasUsed = 0 := hlegacy.2
  simp only [stateTransitionChecks, concreteDeploymentBody_gas, concreteDeploymentBody_logs,
    concreteDeploymentBody_blobGas, hstateGas, concreteDeploymentEnvelope, concreteCanonicalBlock,
...
theorem concreteDeploymentStep :
  ...
  rw [show concreteConfig.forkAt concreteDeploymentEnvelope.block.header.timestamp =
    .ok .prague from ChainConfig.pragueOnly_forkAt 1 _]
  change stateTransitionAt .prague concreteBase concreteDeploymentEnvelope.block = _
  rw [stateTransitionAt_eq_ok_iff, stateTransitionE]
  have henvelope : concreteDeploymentEnvelope.block =
      concreteDeploymentBlock concreteDeploymentBody.1.root
        (getTransactionsRoot concreteDeploymentBody.2)
        (getReceiptRoot concreteDeploymentBody.2)
        (getWithdrawalsRoot concreteDeploymentBody.2)
        (computeRequestsHash concreteDeploymentBody.2.requests) := by
    rfl
  have hheader : validateHeader (Fork.ruleSet .prague)
      concreteBase concreteDeploymentEnvelope.block.header = .ok () := by
    rw [henvelope]
    change validateHeader pragueRules concreteBase
      (concreteDeploymentHeader concreteDeploymentBody.1.root
        (getTransactionsRoot concreteDeploymentBody.2)
        (getReceiptRoot concreteDeploymentBody.2)
        (getWithdrawalsRoot concreteDeploymentBody.2)
        (computeRequestsHash concreteDeploymentBody.2.requests)) = .ok ()
    exact concreteDeploymentHeader_valid _ _ _ _ _
  rw [hheader]
  change (do
    Except.mapError TransitionError.block
      (stateTransitionOmmersCheck concreteDeploymentEnvelope.block.ommers)
    let output ← applyBody (initBenv .prague concreteBase concreteDeploymentEnvelope.block.header)
      concreteDeploymentEnvelope.block.txs concreteDeploymentEnvelope.block.wds
    ...
    Except.mapError TransitionError.block
      (blockAccessListCheck (Fork.ruleSet .prague) output.2
        concreteDeploymentEnvelope.block.header)
  ...
  rw [henvelope]
  have hommers : stateTransitionOmmersCheck
      (concreteDeploymentBlock concreteDeploymentBody.1.root
        (getTransactionsRoot concreteDeploymentBody.2) (getReceiptRoot concreteDeploymentBody.2)
        (getWithdrawalsRoot concreteDeploymentBody.2)
        (computeRequestsHash concreteDeploymentBody.2.requests)).ommers = .ok () := by
    rfl
  have hchecks : stateTransitionChecks concreteDeploymentBody.2
      (concreteDeploymentBlock concreteDeploymentBody.1.root
        (getTransactionsRoot concreteDeploymentBody.2) (getReceiptRoot concreteDeploymentBody.2)
        (getWithdrawalsRoot concreteDeploymentBody.2)
        (computeRequestsHash concreteDeploymentBody.2.requests)).header
      (getTransactionsRoot concreteDeploymentBody.2) concreteDeploymentBody.1.root
      (getReceiptRoot concreteDeploymentBody.2) (logsBloom concreteDeploymentBody.2.blockLogs)
      (getWithdrawalsRoot concreteDeploymentBody.2)
      (computeRequestsHash concreteDeploymentBody.2.requests) = .ok () := by
    exact concreteDeploymentChecks
  have hbal : blockAccessListCheck (Fork.ruleSet .prague) concreteDeploymentBody.2
      (concreteDeploymentBlock concreteDeploymentBody.1.root
        (getTransactionsRoot concreteDeploymentBody.2) (getReceiptRoot concreteDeploymentBody.2)
        (getWithdrawalsRoot concreteDeploymentBody.2)
        (computeRequestsHash concreteDeploymentBody.2.requests)).header = .ok () := by
    rfl
  simp only [hommers, Except.mapError, bind, Except.bind, hchecks, hbal]
  rfl
...
theorem concreteDeploymentRoot :
  ...
  canonicalDeploymentStep_establishes_root concreteConfig .prague concreteBase concreteDeployed
    ...
    CoveredFork.prague concreteDeploymentStep
...
noncomputable def concreteDeploymentTrace :
    ExecutionTrace.AppliedBodyTrace
      (initBenv .prague concreteBase concreteDeploymentEnvelope.block.header)
      concreteDeploymentEnvelope.block.txs concreteDeploymentEnvelope.block.wds
    ...
    ∃ ctx : PreparedDeploymentContext concreteConfig .prague concreteBase
      ...
      ∃ entry : Benv,
        ...
        concreteDeployed.state = deploymentFinalState ctx.txInput concreteCreateTx concreteCreateSender
          ...
          (deploymentTransactionGasBound ctx.txInput concreteCreateTx concreteCreateSender) := by
  let preview := concreteCanonicalBlock 0 0 0 0 0
  have hpreviewBenv :
      initBenv .prague concreteBase preview.block.header =
        initBenv .prague concreteBase concreteExecutionHeader := by
    change initBenv .prague concreteBase (concreteDeploymentHeader 0 0 0 0 0) =
      initBenv .prague concreteBase concreteExecutionHeader
    exact concreteDeploymentHeader_benv _ _ _ _ _
  have henv := concreteCanonicalEnvelope 0 0 0 0 0
  obtain ⟨ctx⟩ := prepareCanonicalDeploymentContext concreteConfig .prague concreteBase
    preview concreteCreateTx concreteCreateSender concreteCreateTarget concreteDeploymentBase henv
    CoveredFork.prague
...
def concreteDeploymentState : State :=
  deploymentFinalState (initBenv .prague concreteBase concreteExecutionHeader)
...
theorem concreteDeployed_state : concreteDeployed.state = concreteDeploymentState := by
  ...
  have hpreviewBenv :
      initBenv .prague concreteBase (concreteCanonicalBlock 0 0 0 0 0).block.header =
        initBenv .prague concreteBase concreteExecutionHeader := by
    change initBenv .prague concreteBase (concreteDeploymentHeader 0 0 0 0 0) =
      initBenv .prague concreteBase concreteExecutionHeader
    exact concreteDeploymentHeader_benv _ _ _ _ _
  ...
  have hentryState : entry.state = concreteConstructorEntry := by
    ...
    simp only [ctx.target_eq]
    rfl
  rw [hstate, hentryState, ctx.msg_time_eq, ctx.systemPrefix.environment_eq,
    hpreviewBenv, concreteCreateGasUsed]
  rfl
