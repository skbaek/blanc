...
/-!
The composed **row-19 pause witness-world run**.
...
the two legs the landed statements cannot serve directly.  This leaf composes
them at the row-19 world (`pauseLastWorldStor`): the pauser's single
assignment, so the pause retires it — the kernel decrements the count to zero
and `pauseSuccess` takes its zero arm, clearing the pauser's heartbeat expiry.

Everything is concrete: the walk enters from `pauseWorldMsg pauseLastWorldStor
pauseLastWorldGas` with exactly `41656` gas and both accessed sets empty, and
exits at `STOP` with the gas exhausted to zero.  The full per-leg ledger:
...
The row-18 world (`pauseRetainedWorldStor`) is **not** composed here: its
swap-pop removal writes two cells the pause path never reads — the hole
`arrayEntrySlot 1` and the moved target's `indexSlot` — so those two `SSTORE`s
are cold at the worlds and the substrate's swap-pop store suffix, which stores
through warm-only `temporal_sstore_runCompiled`, cannot be entered.  Composing
row 18 needs a cold-`SSTORE` sibling of the temporal convention first.
-/
...
private theorem pauseLastWorld_master :
  ...
  obtain ⟨mid, hstk, hmem, hgas, herrF, houtF, hretF, hlogsF, hrefundF,
    ...
    pauseAfterSet_toSuccess_exists_runCompiled
      ((runtime officialParams).main :: (runtime officialParams).aux)
