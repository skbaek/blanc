...
/-! ## Counted `.call` traversal

The counted cursor API of `Weth10AttributionChronology` has no `.call`
entry primitive, so this module builds the two it needs locally: the
counted mirrors of `Exec.Frame.CompiledCursor.selectBranch` and
`Exec.Frame.CompiledCursor.enterCall`, in the style of
`Exec.Frame.advance_cont_counted`. -/
