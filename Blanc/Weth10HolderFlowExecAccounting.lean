...
/-- The currently closed childless storage branches, expressed only by their
concrete invocation tests.  This incremental sum includes both primary-flow
branches and the ordinary storage-neutral `approve` leaf. -/
