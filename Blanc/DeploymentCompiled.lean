...
/-- The arbitrary-outcome compiled liveness bridge with compiler-table
evidence restricted to an exact prefix.  This is the appended-code counterpart
of `Func.exec_of_runCompiledTo_core`; it is needed by constructor failures whose
runtime and ABI bytes follow the executable creation program. -/
theorem Func.exec_of_runCompiledTo_appended_core :
    ∀ {f₀ : Func} {fs' : List Func} {sevm : Sevm} {FS : List Func}
      {devm : Devm} {p : Func} {ex : Execution} {pfxCode sfxData : Bytes},
  ...
  intro f₀ fs' sevm FS devm p ex pfxCode sfxData h_run
  induction h_run with
  | zero h_room h_pop h_f ih =>
    intro h_compile h_code hFS pc sub hb
    rcases subcode_compile_branch_jumpable sub hb with
...
/-- A gas-exact arbitrary-outcome run of a compiled program executes with that
exact outcome when the compiled bytes are an exact prefix of the complete EVM
code image. -/
theorem Prog.exec_of_runCompiledTo_appended
    {sevm : Sevm} {pre : Devm} {p : Prog} {ex : Execution}
    {pfxCode sfxData : Bytes}
  ...
  have h1 : Evm.step ⟨0, sevm, pre⟩ = .cont 1 mid :=
    Evm.jumpdest_cont h_jd h_burn
  obtain ⟨exc⟩ := Func.exec_of_runCompiledTo_appended_core
    h_run h_compile' h_code rfl 1 h_sub h_npb
  rw [← exec_iff_exec_eq]
  exact ⟨Exec.cont h1 exc⟩
