...
/-- Exact retained-effect companion of
...
theorem insertionLoop_firstLive_exists_storageEffectRun
  ...
  obtain ⟨finalBase, finalMemory, finalCarrier, run⟩ :=
    insertionLoop_deadThenLive_exists_storageEffectRun
      ...
      (by rw [hcost]; exact hbound)
      hinsertionContinuation hinsertionLoop
  refine ⟨finalBase, finalMemory, finalCarrier, ?_⟩
  rw [hcost, hkey, hnode] at run
  simpa only [InsertionLoopState.height, insertionNatState] using run
