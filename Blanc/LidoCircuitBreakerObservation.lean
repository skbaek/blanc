...
The contract-neutral `Func.RunCompiledTo` and instruction-walk inverters live
in `Blanc.CompiledWalkInversion`.  This module keeps only CircuitBreaker
observation-specific consequences of those shared facts. -/

/-! `ISZERO` inversion is provided by `Blanc.CompiledWalkInversion`. -/
