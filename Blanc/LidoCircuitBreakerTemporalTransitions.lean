    ...
    · simpa only [Devm.setMach_setMach, Devm.stack_setMach,
        ...
        (Devm.burnBy_setMach_gas
          ...
          (by simp only [Devm.gasLeft_setMach]
              norm_num [gVerylow, gMid, gJumpdest]
              omega))
    · simpa only [fs, MIndex, MLength, MLast, eventLog, Nat.add_zero]
        using hremove
  have hbranch : Func.RunCompiled fs sevm
...
def absentZeroSetPauserKernelGas (sevm : Sevm) (base : Devm)
  ...
  let assigned := freshAssignmentPost sevm base target 0
...
theorem setPauserKernel_absentZero_runCompiled
    ...
    (hassignmentCost : sstoreValueCost assignmentOriginal 0 0 =
      assignmentCost)
    (harray : (freshAssignmentPost sevm base target 0).getStorVal
      sevm.currentTarget
        (arrayEntrySlot (Nat.toB256 (entries.length + 1))) = 0)
    (harrayOrig : getOrigStorVal sevm sevm.currentTarget
      (arrayEntrySlot (Nat.toB256 (entries.length + 1))) = arrayOriginal)
    ...
    (hwarmArray : (sevm.currentTarget,
      arrayEntrySlot (Nat.toB256 (entries.length + 1))) ∈
        (freshAssignmentPost sevm base target 0).accessedStorageKeys)
    (hindexOrig : getOrigStorVal sevm sevm.currentTarget
      (indexSlot target) = indexOriginal)
    (hindexCost : sstoreValueCost indexOriginal 0
      (Nat.toB256 (entries.length + 1)) = indexCost)
  ...
  let assignmentKey := assignmentSlot target
  let assignmentBase := freshAssignmentBase sevm base target
  let assignmentPost := freshAssignmentPost sevm base target 0
  let next := Nat.toB256 (entries.length + 1)
  ...
  have hlength : assignmentPost.getStorVal sevm.currentTarget
    ...
    have hne := registryAddressFamilies_ne_arrayLengthSlot
      htargetValid.2 htargetValid.2
    change (temporalSstorePost sevm assignmentBase assignmentKey 0).getStorVal
      sevm.currentTarget arrayLengthSlot = oldLength
    rw [temporalSstorePost_other sevm assignmentBase assignmentKey 0
      sevm.currentTarget arrayLengthSlot (by
        intro hp
        exact hne.1 (congrArg Prod.snd hp).symm)]
    change (temporalSloadBase sevm base assignmentKey).getStorVal
  ...
  have hcontinuation' : Bytes.toB256
    ...
    rw [Bytes.sliceD_writeAt_after _ _ _ _ _ (by
      ...
      decide)]
    exact hcontinuation
  let lengthBase := temporalSloadBase sevm assignmentPost arrayLengthSlot
  let arrayPost := temporalSstorePost sevm lengthBase
    (arrayEntrySlot next) target
  ...
  have hstore : Func.RunCompiled fs sevm
      (assignmentBase.setMach
        ⟨[assignmentKey, 0, 0], M', appendGas + 29 + assignmentCost⟩)
      (Ninst.sstore ::: Ninst.iszero :::
        ((.call appendTargetSlot) <?>
          (previousCountKey +++ Ninst.sload ::: pushB256 1 ::: swap 0 ::: sub :::
            previousCountKey +++ Ninst.sstore ::: .call afterOldPauserSlot)))
  ...
  have htargetValue' : (M'.read (targetWord * 32).toNat 32).1.toB256 =
      target := by rw [Mem.Reads.read hreads']; exact htarget'
  have hnewValue' : (M'.read (newPauserWord * 32).toNat 32).1.toB256 = 0 := by
    rw [Mem.Reads.read hreads']
    exact hnew'
  have htargetKeySecond : Func.RunCompiled fs sevm
      (assignmentBase.setMach ⟨[0, 0], M', appendGas + 41 + assignmentCost⟩)
      (targetKey +++ Ninst.sstore ::: Ninst.iszero :::
        ((.call appendTargetSlot) <?>
          (previousCountKey +++ Ninst.sload ::: pushB256 1 ::: swap 0 ::: sub :::
            previousCountKey +++ Ninst.sstore ::: .call afterOldPauserSlot)))
  ...
  · have hg : G + absentZeroSetPauserKernelGas sevm base target
        ...
        appendGas + 90 + assignmentCost +
          temporalSloadCost sevm base assignmentKey := by
      dsimp only [absentZeroSetPauserKernelGas, appendGas, assignmentPost,
        assignmentBase, assignmentKey, freshAssignmentPost,
        freshAssignmentBase]
      omega
    rw [hg]
    simpa only [fs] using hkernel
  · dsimp only [post]
    have hlogs := appendTarget_absentZero_logs sevm assignmentPost target
      oldLength next
  ...
  · intro pauser hpauser
    have hexp := appendTarget_absentZero_expiry_unchanged sevm assignmentPost
    ...
    have hne := expirySlot_ne_registryAddressFamilies hpauser
      htargetValid.2 htargetValid.2
    have hassignmentExpiry : assignmentPost.getStorVal sevm.currentTarget
        (expirySlot pauser) =
        base.getStorVal sevm.currentTarget (expirySlot pauser) := by
      change (temporalSstorePost sevm assignmentBase assignmentKey 0).getStorVal
      ...
      dsimp only [assignmentBase, freshAssignmentBase]
      rw [temporalSloadBase_getStorVal]
    calc
      post.getStorVal sevm.currentTarget (expirySlot pauser) =
          (absentZeroRemovePost sevm
      ...
      _ = assignmentPost.getStorVal sevm.currentTarget
...
theorem registerPauser_body_absentZero_runCompiled
    ...
    ∃ trace post,
      ...
      ∀ pauser, canonicalAddress pauser →
        post.getStorVal sevm.currentTarget (expirySlot pauser) =
          base.getStorVal sevm.currentTarget (expirySlot pauser) := by
  let M := freshRegisterMemory target 0
  let img := freshRegisterImage target 0
  rcases freshRegisterMemory_spec target 0 with
    ⟨hwf, hreads, hsize, htargetRead, hnewRead,
  ...
  have hstage : Func.RunCompiled fs sevm
      (base.setMach ⟨[], Mem.empty,
        G + absentZeroSetPauserKernelGas sevm base target assignmentCost
          arrayCost indexCost lengthCost holeCost movedIndexCost
          tailClearCost lengthRestoreCost indexClearCost + 112⟩)
      (arg 0 +++ mstoreAt targetWord +++
        arg 1 +++ mstoreAt newPauserWord +++
        pushB256 0 ::: mstoreAt previousPauserWord +++
        pushB256 0 ::: mstoreAt continuationWord +++
  ...
  all_goals try { simp [hadmin, B256.eqCheck] }
  all_goals first
    | (simp only [Devm.gasLeft_setMach, absentZeroRegisterBodyGas]
       norm_num [gBase, gVerylow, gHigh, gMid, gJumpdest]
       omega)
    | skip
  case h_arm =>
    simp only [absentZeroRegisterBodyGas]
    have hg : G + (221 + absentZeroSetPauserKernelGas sevm base target
          assignmentCost arrayCost indexCost lengthCost holeCost
          movedIndexCost tailClearCost lengthRestoreCost indexClearCost) - 109 =
        G + absentZeroSetPauserKernelGas sevm base target assignmentCost
          arrayCost indexCost lengthCost holeCost movedIndexCost tailClearCost
...
private theorem finishSetPauser_retainedOldZero_runCompiled
  ...
  have hreadZero : M.read 0 0 = ([], M) := by
    ...
    rfl
  let fs := (runtime dp).main :: (runtime dp).aux
  have hregister : Func.RunCompiled fs sevm
      (eventBase.setMach ⟨stack, M, G + 173⟩) registerAfterSet
      (eventBase.setMach ⟨stack, M, G⟩) := by
    apply registerAfterSet_retainedOldZero_runCompiled fs sevm eventBase
      M img oldPauser remaining stack G hstack hreads hprevious hnew holdNonzero
      hremaining
    · exact hcount
    · exact hwarmCount
    · exact hsize
  ...
  have hlookup : fs[registerAfterSetSlot]? = some registerAfterSet := by
    simp [fs, runtime, aux, registerAfterSetSlot]
  have hcall : Func.RunCompiled fs sevm
      (eventBase.setMach ⟨stack, M, G + 185⟩)
      (.call registerAfterSetSlot)
      (eventBase.setMach ⟨stack, M, G⟩) := by
    apply Func.RunCompiled.call hlookup
      (by simp only [Devm.stack_setMach]; omega)
    · simpa only [Devm.setMach_setMach, Devm.stack_setMach,
        Devm.memory_setMach] using
        (Devm.burnBy_setMach_gas
          (devm := eventBase.setMach ⟨stack, M, G + 185⟩)
          (cost := gVerylow + gMid + gJumpdest) (G := G + 173)
          (by simp only [Devm.gasLeft_setMach]
              norm_num [gVerylow, gMid, gJumpdest]))
    · exact hregister
  have hbranch : Func.RunCompiled fs sevm
      (eventBase.setMach ⟨1 :: stack, M, G + 199⟩)
      ((.call registerAfterSetSlot) <?> (.call pauseAfterSetSlot))
      (eventBase.setMach ⟨stack, M, G⟩) := by
    apply Func.RunCompiled.succ (w := (1 : B256)) (by decide)
    ...
    · simpa only [Devm.setMach_setMach, Devm.stack_setMach,
        Devm.memory_setMach] using
        (Devm.popBurnBy_setMach
          (devm := eventBase.setMach ⟨1 :: stack, M, G + 199⟩)
          (x := (1 : B256)) (s := stack)
          (cost := gVerylow + gHigh + gJumpdest) (G := G + 185)
          (h_stk := rfl) (h := by
            simp only [Devm.gasLeft_setMach]
            norm_num [gVerylow, gHigh, gJumpdest]))
    · exact hcall
  have hcontinuationRun : Func.RunCompiled fs sevm
      (eventBase.setMach ⟨stack, M, G + 208⟩)
      (loadWord continuationWord +++ Ninst.iszero :::
        ((.call registerAfterSetSlot) <?> (.call pauseAfterSetSlot)))
      (eventBase.setMach ⟨stack, M, G⟩) := by
    func_run (3) [3]
...
private theorem removeTarget_restoreTail_foundZeroRetained_runCompiled
  ...
  apply removeTarget_restoreTail_absentZero_runCompiled dp sevm base M img
    target oldLength next oldPauser stack hstack lengthOriginal indexOriginal
...
private theorem removeTarget_storePrefix_foundZeroRetained_runCompiled
  ...
  have hrun := removeTarget_storePrefix_absentZero_runCompiled
    dp sevm base M img target oldLength next oldPauser stack hstack arrayOriginal
    indexOriginal lengthOriginal holeCost movedIndexCost tailClearCost
...
private theorem afterOldPauser_foundZeroRetained_runCompiled
  ...
  have hcall : Func.RunCompiled fs sevm
      (base.setMach ⟨stack, M,
        G + 2563 + indexExtCost + lengthExtCost + holeCost + movedIndexCost +
          tailClearCost + lengthRestoreCost + indexClearCost⟩)
      (.call removeTargetSlot)
      (((absentZeroRemovePost sevm
          (absentZeroTailClearPost sevm base target next)
    ...
    · simpa only [Devm.setMach_setMach, Devm.stack_setMach,
        Devm.memory_setMach] using
        (Devm.burnBy_setMach_gas
          (devm := base.setMach ⟨stack, M,
            G + 2563 + indexExtCost + lengthExtCost + holeCost +
              movedIndexCost + tailClearCost +
              lengthRestoreCost + indexClearCost⟩)
          (cost := gVerylow + gMid + gJumpdest)
          (G := G + 2551 + indexExtCost + lengthExtCost + holeCost +
            movedIndexCost + tailClearCost +
            lengthRestoreCost + indexClearCost)
          (by simp only [Devm.gasLeft_setMach]
              norm_num [gVerylow, gMid, gJumpdest]
  ...
  have hbranch : Func.RunCompiled fs sevm
      (base.setMach ⟨1 :: stack, M,
        G + 2577 + indexExtCost + lengthExtCost + holeCost + movedIndexCost +
          tailClearCost + lengthRestoreCost + indexClearCost⟩)
      ((.call removeTargetSlot) <?>
        (newCountKey +++ Ninst.sload ::: pushB256 1 ::: add :::
          newCountKey +++ Ninst.sstore ::: .call finishSetPauserSlot))
    ...
    · simpa only [Devm.setMach_setMach, Devm.stack_setMach,
        Devm.memory_setMach] using
        (Devm.popBurnBy_setMach
          (devm := base.setMach ⟨1 :: stack, M,
            G + 2577 + indexExtCost + lengthExtCost + holeCost +
              movedIndexCost + tailClearCost +
              lengthRestoreCost + indexClearCost⟩)
          (x := (1 : B256)) (s := stack)
          (cost := gVerylow + gHigh + gJumpdest)
          (G := G + 2563 + indexExtCost + lengthExtCost + holeCost +
            movedIndexCost + tailClearCost +
            lengthRestoreCost + indexClearCost)
          (h_stk := rfl) (h := by
            simp only [Devm.gasLeft_setMach]
    ...
    · exact hcall
  have hnewCovered : (newPauserWord * 32).toNat + 32 ≤ M.size := by
    rw [hsize, show (newPauserWord * 32).toNat + 32 = 576 by decide]
    omega
  have hnewMemory : (M.read (newPauserWord * 32).toNat 32).2 = M := by
    rw [Mem.read_snd_eq_self (memExtSize_of_le halign hnewCovered)]
  simp only [afterOldPauser]
  ...
  case a =>
    rw [Mem.Reads.read hreads, hnew, hnewMemory]
    have hg : G + 2586 + indexExtCost + lengthExtCost + holeCost +
          movedIndexCost + tailClearCost +
          lengthRestoreCost + indexClearCost - 9 =
        G + 2577 + indexExtCost + lengthExtCost + holeCost + movedIndexCost +
          tailClearCost + lengthRestoreCost + indexClearCost := by omega
...
private def foundSetPauserKernelPrefixGas (sevm : Sevm) (base : Devm)
  ...
  122 + temporalSloadCost sevm base (assignmentSlot target) + assignmentCost +
    temporalSloadCost sevm (freshAssignmentPost sevm base target newPauser)
...
private theorem setPauserKernel_found_runCompiled
    ...
    (hcount : (freshAssignmentPost sevm base target newPauser).getStorVal
    ...
    (hafter :
      ...
      let countBase := temporalSloadBase sevm
        (freshAssignmentPost sevm base target newPauser)
        (countSlot oldPauser)
      Func.RunCompiled ((runtime dp).main :: (runtime dp).aux) sevm
  ...
  let assignmentBase := freshAssignmentBase sevm base target
  let assignmentPost := freshAssignmentPost sevm base target newPauser
  let countBase := temporalSloadBase sevm assignmentPost (countSlot oldPauser)
  ...
  let elseGas := G + afterGas + 45 + countCost +
    temporalSloadCost sevm assignmentPost (countSlot oldPauser)
  have halign : M.size % 32 = 0 := by rw [hsize]
  have hwf' : Mem.Wf M' := hwf.write _ _
  have hreads' : Mem.Reads M' img' := Mem.Reads.write hwf hreads _ _
  have hsizeM' : M'.size = M.size := by
  ...
  have hwarmCount : (sevm.currentTarget, countSlot oldPauser) ∈
    ...
    temporalSloadBase_warm sevm assignmentPost (countSlot oldPauser)
  have hstoreCount : Func.RunCompiled fs sevm
  ...
  have hcountSload : Func.RunCompiled fs sevm
      (assignmentPost.setMach ⟨[countSlot oldPauser], M',
  ...
  have helse : Func.RunCompiled fs sevm
      (assignmentPost.setMach ⟨[], M', elseGas⟩)
      (previousCountKey +++ Ninst.sload ::: pushB256 1 ::: swap 0 ::: sub :::
        previousCountKey +++ Ninst.sstore ::: .call afterOldPauserSlot)
      out := by
    have hrun := previousCountKey_prepend_runCompiled hpreviousValue'
    ...
    have hg : G + afterGas + 33 + countCost +
          temporalSloadCost sevm assignmentPost (countSlot oldPauser) + 12 =
        elseGas := by
      dsimp only [elseGas]
      omega
    rw [hg] at hrun
    exact hrun
  have hbranch : Func.RunCompiled fs sevm
      (assignmentPost.setMach ⟨[0], M', elseGas + 13⟩)
      ((.call appendTargetSlot) <?>
        (previousCountKey +++ Ninst.sload ::: pushB256 1 ::: swap 0 ::: sub :::
          previousCountKey +++ Ninst.sstore ::: .call afterOldPauserSlot))
      out := by
    ...
    · simpa only [Devm.setMach_setMach, Devm.stack_setMach,
        Devm.memory_setMach] using
        (Devm.popBurnBy_setMach
          (devm := assignmentPost.setMach ⟨[0], M', elseGas + 13⟩)
          (x := (0 : B256)) (s := []) (cost := gVerylow + gHigh)
          (G := elseGas)
          (h_stk := rfl) (h := by
            simp only [Devm.gasLeft_setMach]
    ...
    · exact helse
  have hiszero : Func.RunCompiled fs sevm
      (assignmentPost.setMach ⟨[oldPauser], M', elseGas + 16⟩)
      (Ninst.iszero ::: ((.call appendTargetSlot) <?>
        (previousCountKey +++ Ninst.sload ::: pushB256 1 ::: swap 0 ::: sub :::
          previousCountKey +++ Ninst.sstore ::: .call afterOldPauserSlot)))
      out := by
  ...
  have hstore : Func.RunCompiled fs sevm
    ...
    exact Func.RunCompiled.next
      ...
      hiszero
  have htargetKeySecond : Func.RunCompiled fs sevm
      (assignmentBase.setMach ⟨[newPauser, oldPauser], M',
    ...
    exact hrun
  have hnewTail : Func.RunCompiled fs sevm
      (assignmentBase.setMach ⟨[oldPauser], M',
    ...
    exact hrun
  have hsavePrevious : Func.RunCompiled fs sevm
      (assignmentBase.setMach ⟨[oldPauser, oldPauser], M,
  ...
  have hg : G + afterGas + foundSetPauserKernelPrefixGas sevm base target
    ...
    dsimp only [foundSetPauserKernelPrefixGas, elseGas, assignmentPost]
    omega
  rw [hg]
  simpa only [fs] using hkernel
...
private def foundZeroRetainedLastSetPauserKernelGas (sevm : Sevm) (base : Devm)
  ...
  2714 + temporalSloadCost sevm base (assignmentSlot target) + assignmentCost +
    temporalSloadCost sevm (freshAssignmentPost sevm base target 0)
...
theorem setPauserKernel_foundZeroRetainedLast_runCompiled
    ...
    (hassignmentCost : sstoreValueCost assignmentOriginal oldPauser 0 =
      assignmentCost)
    (hcount : (freshAssignmentPost sevm base target 0).getStorVal
      sevm.currentTarget (countSlot oldPauser) = oldCount)
    (hcountOrig : getOrigStorVal sevm sevm.currentTarget
      (countSlot oldPauser) = countOriginal)
    (hcountCost : sstoreValueCost countOriginal oldCount (oldCount - 1) =
  ...
  have htransport : ∀ k : B256, k ≠ countSlot oldPauser →
    ...
    dsimp only [countPost, countBase, assignmentPost, freshAssignmentPost,
      freshAssignmentBase]
  ...
  let eventLog : Log :=
    ⟨sevm.currentTarget, [pauserSetEvent, target, oldPauser, 0], []⟩
  let post := ((absentZeroRemovePost sevm
      (absentZeroTailClearPost sevm countPost target next)
  ...
  have hkernel := setPauserKernel_found_runCompiled dp sevm base M img post
    ...
    (by omega) hstatic
    (by
      simpa only [countPost, countBase, assignmentPost, M', MIndex, MLength,
  ...
  · have logs_setMach (d : Devm) (mach : Mach) :
    ...
    have logs_addLog (d : Devm) (log : Log) :
        (d.addLog log).logs = d.logs ++ [log] := rfl
    dsimp only [post, countPost, countBase, assignmentPost,
      freshAssignmentPost, freshAssignmentBase, eventLog]
    ...
    calc
      ...
      _ = base.getStorVal sevm.currentTarget (expirySlot pauser) := by
        dsimp only [countPost, countBase, assignmentPost, freshAssignmentPost,
          freshAssignmentBase]
        simp only [absentZeroRemovePost, absentZeroLengthRestorePost,
          absentZeroTailClearPost, absentZeroMovedIndexPost,
          absentZeroHolePost]
        rw [temporalSstorePost_other _ _ (indexSlot target) 0 _
...
private theorem registerPauser_stageArgs_runCompiled
    ...
    (hkernel : Func.RunCompiled ((runtime dp).main :: (runtime dp).aux) sevm
      (base.setMach ⟨[], freshRegisterMemory target newPauser, kernelGas⟩)
      setPauserKernel post) :
    Func.RunCompiled ((runtime dp).main :: (runtime dp).aux) sevm
...
theorem registerPauser_body_foundZeroRetainedLast_runCompiled
  ...
  rcases setPauserKernel_foundZeroRetainedLast_runCompiled dp sevm base
      (freshRegisterMemory target 0) (freshRegisterImage target 0)
