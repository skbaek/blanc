...
def ForallSubExecAdmittedSem (k : Nat) (ca : Adr) (sem : CodeSem)
...
lemma lift_admitted_sem.atTarget
    ...
    (depth_ind :
      ∀ {sevm pre post} (run : Exec 0 sevm pre (.ok post)),
        ...
        Exec.FrameAdmitted ca entry run →
        ForallSubExecAdmittedSem sevm.depth ca sem entry R →
        R sevm pre post)
    {pc : Nat} {sevm : Sevm} {pre : Devm} {out : Execution}
    (run : Exec pc sevm pre out)
    (h_fa : ForallDeeperAtSem sevm.depth ca sem
      (fun _ sevm' pre' out' run' =>
        Exec.FrameAdmitted ca entry run' → ifOk (R sevm' pre') out'))
    (h_at : sem.At ca pc sevm pre)
  ...
  | ok post =>
      ...
      refine depth_ind run
        (sem.correct run (h_at.right target).left)
        target admitted ?_
...
lemma lift_admitted_sem
    ...
    ∀ pc sevm pre post (run : Exec pc sevm pre (.ok post)),
      sem.At ca pc sevm pre →
      Exec.FrameAdmitted ca entry run →
      R sevm pre post := by
  have all : Exec.Fa
      (Exec.WknSem ca sem (fun _ sevm pre out run =>
        Exec.FrameAdmitted ca entry run → ifOk (R sevm pre) out)) := by
    ...
    · intro pc sevm pre out hstep h_fa h_at admitted
      ...
      · exact lift_admitted_sem.atTarget depth_ind (.halt hstep) h_fa h_at target admitted
      · cases out with
    ...
    · intro pc sevm pre frame resume pc' result error hstep henter hresume
        h_fa h_at admitted
      rcases em (sevm.currentTarget = ca) with target | targetNe
      · exact lift_admitted_sem.atTarget depth_ind
        ...
        | ok post =>
            ...
            have hcode : inter.getCode ca = pre.getCode ca :=
              lift_core_sem.stepCode (xl := .none) trivial
                ...
                ca h_ne_code
            have h_at' : sem.At ca (pc + 1) sevm inter :=
    ...
    · intro pc sevm pre frame resume pc' childEvm raw inter out hstep henter child
      ...
      · cases out with
        ...
        | ok post =>
            ...
            have h_ne_code : (pre.getCode ca).toList ≠ [] := fun empty =>
              (sem.ne_nil (Eq.trans h_at.left.symm (congrArg some empty))) rfl
            have h_at_child : sem.At ca childEvm.pc childEvm.sta childEvm.dyna := by
              ...
              have hcode := hsrc targetsNe
                (by rw [childTarget]; exact not_empty_of_codeSem h_at.left)
                (by rw [childTarget]; exact sem.not_delegation h_at.left)
            ...
            have hcode : inter.getCode ca = pre.getCode ca :=
              lift_core_sem.stepCode (xl := .some ⟨childEvm, raw⟩) hchild
                (by rw [hstep]
                    exact ⟨frame.settle raw, RunFrame.of_run henter, hresume.symm⟩)
                ca h_ne_code
            have h_at' : sem.At ca (pc + 1) sevm inter :=
...
lemma lift_inv_admitted_sem
    ...
    ∀ pc sevm pre post (run : Exec pc sevm pre (.ok post)),
      ...
      σ sevm pre →
      ρ sevm post := by
  apply @lift_admitted_sem entry (fun sevm pre post => σ sevm pre → ρ sevm post)
    ca sem
