...
lemma Xinst.inv_nof {sevm : Sevm} {s r : Devm} {x : Xinst} {xl : Xlot}
  ...
  have hxl : Xlot.Rel Devm.BalNoninc xl :=
    Xlot.rel_of_filled balNoninc_refl_trans.2.1 balNoninc_refl_trans.2.2
      Ninst.balance_effectGen Jinst.balance_effect Linst.balance_effect h_fill
  exact Nat.lt_of_le_of_lt (Xinst.balance_effectGen x hxl h) h_nof
...
theorem processMessage_inv_solvent {wa : Adr} {msg : Msg} {evm : Devm} {lim : Nat}
  ...
  | succ k =>
    ...
    rcases Fueled.of_bind_eq_ok h_run with ⟨benv, hb, h_run'⟩
    rcases Fueled.of_bind_eq_ok h_run' with ⟨evm', hec, h_if⟩
    ...
    · -- clean success : `evm = evm'` comes straight from `executeCode`
      ...
      | succ j =>
        ...
        · -- no code address : run the interpreter directly
          rw [hca] at hec
          simp only [] at hec
          rcases Fueled.of_mapResult_eq hec with ⟨exn, h_exec, h_handle⟩
          have hexn := exec_ok_of_handleError h_handle herr
          rw [hexn] at h_exec
        ...
        · rw [hca] at hec
          ...
          · -- precompile : the state is left untouched
            ...
            exact State.Inv.of_benvAfterTransfer h_ne (Fueled.ofExcept_inj.mp hb) h_inv
...
theorem processCreateMessage_inv_solvent {wa : Adr} {msg : Msg} {evm : Devm} {lim : Nat}
  ...
  | succ k =>
    ...
    rcases Fueled.of_bind_eq_ok h_run with ⟨evm2, hpm, h_rest⟩
    ...
    · rw [if_pos herr] at h_rest
      ...
      · -- code-gas charge failed
        ...
        · -- exceptional halt : state rolled back to `msg.benv.state`
          ...
          rw [← Fueled.ok_inj.mp h_rest]; exact h_inv
        · rw [if_neg hex] at h_rest; exact absurd h_rest (by simp)
      · -- clean success : install the returned code at `currentTarget ≠ wa`
        rw [hcg] at h_rest; dsimp only at h_rest
        rw [← Fueled.ok_inj.mp h_rest, Devm.setCode_state, chargeCodeGas_state_ok hcg]
...
theorem executeCode_inv_noDel {wa : Adr} {msg : Msg} {lim : Nat} {evm : Devm}
  ...
  | succ j =>
    ...
    · rw [hca] at h_run
      ...
      have h_ex := exec_inv_noDel j (initSevm msg) (initDevm msg) exn h_exec
    ...
    · rw [hca] at h_run
      ...
      · rw [if_pos hp] at h_run
        ...
        have h_res := handleError_noDel h_ex
        have h_handle := Fueled.ofExcept_inj.mp h_run
        rw [h_handle] at h_res
        exact h_res
  ...
  | succ k =>
    ...
    · rw [if_pos herr] at h_if
      have h_evm : evm = evm'.rollback msg.benv.state msg.tenv.transientStorage := by
        rw [← Fueled.ok_inj.mp h_if]
      rw [h_evm]
      have hbenv := Msg.NoDel.benvAfterTransfer (Fueled.ofExcept_inj.mp hb) h
...
lemma UInt64.toNat_add_carry (a b : UInt64) :
    (a + b).toNat + (if a + b < a then 2 ^ 64 else 0) = a.toNat + b.toNat := by
  have ha := UInt64.toNat_lt a
  have hb := UInt64.toNat_lt b
  simp only [UInt64.toNat_add, UInt64.lt_iff_toNat_lt]
  split <;> omega
...
lemma B64.toNat_mulx (x y : B64) :
  ...
  norm_num at hP1 hP2 hP3 hP4
  have hprod : UInt64.toNat x * UInt64.toNat y < 2 ^ 128 := by
    calc
      _ < (2 ^ 64) * (2 ^ 64) := Nat.mul_lt_mul'' hX hY
      _ = 2 ^ 128 := by norm_num
  ...
  unfold B64.mulx
  rw [B128.toNat_mk]
  have hc1 := UInt64.toNat_add_carry
    (x.lows' * y.lows') ((x.lows' * y.highs') <<< 32)
  have hc2 := UInt64.toNat_add_carry
    (x.lows' * y.lows' + (x.lows' * y.highs') <<< 32)
    ((x.highs' * y.lows') <<< 32)
  cases h1 : decide
      (x.lows' * y.lows' + (x.lows' * y.highs') <<< 32 < x.lows' * y.lows') <;>
    cases h2 : decide
      (x.lows' * y.lows' + (x.lows' * y.highs') <<< 32 +
          (x.highs' * y.lows') <<< 32 <
        x.lows' * y.lows' + (x.lows' * y.highs') <<< 32)
  ·
    have hn1 := decide_eq_false_iff_not.mp h1
    ...
    simp only [decide_eq_true_eq, decide_eq_false_iff_not,
      ...
      Nat.mod_eq_of_lt hP2, Nat.mod_eq_of_lt hP3, Nat.mod_eq_of_lt hP4]
      at hc1 hc2 ⊢
    omega
  ·
    have hn1 := decide_eq_false_iff_not.mp h1
    have hp2 := of_decide_eq_true h2
    simp only [if_neg hn1] at hc1
    simp only [if_pos hp2] at hc2
    simp only [B64.highs', B64.lows', B64.toNat, UInt64.toNat_add,
      UInt64.toNat_mul, UInt64.toNat_and, UInt64.toNat_shiftLeft,
      UInt64.toNat_shiftRight, UInt64.reduceToNat, Nat.shiftLeft_eq,
      Nat.shiftRight_eq_div_pow, Nat.reducePow, Nat.reduceMod, nat_and_mask32,
      Nat.mod_eq_of_lt hP1, Nat.mod_eq_of_lt hP2, Nat.mod_eq_of_lt hP3,
      Nat.mod_eq_of_lt hP4] at hc1 hc2 ⊢
    omega
  ·
    have hp1 := of_decide_eq_true h1
    have hn2 := decide_eq_false_iff_not.mp h2
    simp only [if_pos hp1] at hc1
    simp only [if_neg hn2] at hc2
    simp only [B64.highs', B64.lows', B64.toNat, UInt64.toNat_add,
      UInt64.toNat_mul, UInt64.toNat_and, UInt64.toNat_shiftLeft,
      UInt64.toNat_shiftRight, UInt64.reduceToNat, Nat.shiftLeft_eq,
...
lemma B128.toNat_add_carry (a b : B128) :
    (a + b).toNat + (if a + b < a then 2 ^ 128 else 0) = a.toNat + b.toNat := by
  have ha := @B128.toNat_lt a
  have hb := @B128.toNat_lt b
  simp only [B128.toNat_add, B128.lt_iff_toNat_lt_toNat', Nat.lo_eq]
  split <;> omega

lemma B128.toNat_add3_carry (a b c : B128) :
    (match decide (a + b < a), decide (a + b + c < a + b) with
      | true, true => 2
      | false, false => 0
      | _, _ => 1) * 2 ^ 128 + (a + b + c).toNat =
      a.toNat + b.toNat + c.toNat := by
  have hc1 := B128.toNat_add_carry a b
  have hc2 := B128.toNat_add_carry (a + b) c
  cases h1 : decide (a + b < a) <;>
    cases h2 : decide (a + b + c < a + b)
  ·
    have hn1 := decide_eq_false_iff_not.mp h1
    have hn2 := decide_eq_false_iff_not.mp h2
    simp only [if_neg hn1] at hc1
...
lemma B128.toNat_carry {p q : Prop} [Decidable p] [Decidable q] :
    (match decide p, decide q with
      | true, true => show B128 from (0, 2)
      | false, false => show B128 from (0, 0)
      | _, _ => show B128 from (0, 1)).toNat =
    (match decide p, decide q with
      | true, true => 2
      | false, false => 0
      | _, _ => 1) := by
  cases h1 : decide p <;> cases h2 : decide q <;> rfl

lemma B128.toNat_mulx (x y : B128) :
    (B128.mulx x y).toNat = x.toNat * y.toNat := by
  have hx1 := UInt64.toNat_lt x.1
  have hx2 := UInt64.toNat_lt x.2
  have hy1 := UInt64.toNat_lt y.1
  have hy2 := UInt64.toNat_lt y.2
  have hP1 : x.1.toNat * y.1.toNat < 2 ^ 128 := by
    calc
      _ < (2 ^ 64) * (2 ^ 64) := Nat.mul_lt_mul'' hx1 hy1
      _ = 2 ^ 128 := by norm_num
  have hP2 : x.2.toNat * y.1.toNat < 2 ^ 128 := by
    calc
      _ < (2 ^ 64) * (2 ^ 64) := Nat.mul_lt_mul'' hx2 hy1
      _ = 2 ^ 128 := by norm_num
  have hP3 : x.1.toNat * y.2.toNat < 2 ^ 128 := by
    calc
      _ < (2 ^ 64) * (2 ^ 64) := Nat.mul_lt_mul'' hx1 hy2
      _ = 2 ^ 128 := by norm_num
  have hP4 : x.2.toNat * y.2.toNat < 2 ^ 128 := by
    calc
      _ < (2 ^ 64) * (2 ^ 64) := Nat.mul_lt_mul'' hx2 hy2
      _ = 2 ^ 128 := by norm_num
  have hprod : x.toNat * y.toNat < 2 ^ 256 := by
    calc
      _ < (2 ^ 128) * (2 ^ 128) := Nat.mul_lt_mul'' B128.toNat_lt B128.toNat_lt
      _ = 2 ^ 256 := by norm_num
  have hxnat : x.toNat = x.1.toNat * 2 ^ 64 + x.2.toNat := by
    rw [show x = (x.1, x.2) from (Prod.eta x).symm, B128.toNat_mk]
  have hynat : y.toNat = y.1.toNat * 2 ^ 64 + y.2.toNat := by
    rw [show y = (y.1, y.2) from (Prod.eta y).symm, B128.toNat_mk]
  have hexp : x.toNat * y.toNat =
      x.1.toNat * y.1.toNat * 2 ^ 128 +
      x.2.toNat * y.1.toNat * 2 ^ 64 +
      x.1.toNat * y.2.toNat * 2 ^ 64 +
      x.2.toNat * y.2.toNat := by
    rw [hxnat, hynat]
    ring
  let ll : B128 := x.2.mulx y.2
  let lh : B128 := x.2.mulx y.1
  let hl : B128 := x.1.mulx y.2
  let hh : B128 := x.1.mulx y.1
  let lhl : B128 := show B128 from (lh.2, 0)
  let hll : B128 := show B128 from (hl.2, 0)
  let lt : B128 := ll + lhl
  let l : B128 := lt + hll
  let c : B128 :=
    match decide (lt < ll), decide (l < lt) with
    | true, true => show B128 from (0, 2)
    | false, false => show B128 from (0, 0)
    | _, _ => show B128 from (0, 1)
  let h : B128 := hh + (show B128 from (0, lh.1)) +
    (show B128 from (0, hl.1)) + c
  have hout : B128.mulx x y = (h, l) := by
    unfold B128.mulx
    rw [show x.2.mulx y.2 = ll from rfl, show x.2.mulx y.1 = lh from rfl,
      show x.1.mulx y.2 = hl from rfl, show x.1.mulx y.1 = hh from rfl]
    dsimp only
    dsimp only [h, l, c, lt, lhl, hll]
    clear_value ll lh hl hh
    cases h1 : decide (ll + (show B128 from (lh.2, 0)) < ll) <;>
  ...
  have ell : ll.toNat = x.2.toNat * y.2.toNat := by
    simpa only [ll] using B64.toNat_mulx x.2 y.2
  have elh : lh.toNat = x.2.toNat * y.1.toNat := by
    simpa only [lh] using B64.toNat_mulx x.2 y.1
  have ehl : hl.toNat = x.1.toNat * y.2.toNat := by
    simpa only [hl] using B64.toNat_mulx x.1 y.2
  have ehh : hh.toNat = x.1.toNat * y.1.toNat := by
    simpa only [hh] using B64.toNat_mulx x.1 y.1
  have elhdec : lh.toNat = lh.1.toNat * 2 ^ 64 + lh.2.toNat := by
    rw [show lh = (lh.1, lh.2) from (Prod.eta lh).symm, B128.toNat_mk]
  have ehldec : hl.toNat = hl.1.toNat * 2 ^ 64 + hl.2.toNat := by
    rw [show hl = (hl.1, hl.2) from (Prod.eta hl).symm, B128.toNat_mk]
  let k : Nat :=
    match decide (B128.add ll (lh.2, 0) < ll),
      decide (B128.add (B128.add ll (lh.2, 0)) (hl.2, 0) <
        B128.add ll (lh.2, 0)) with
    | true, true => 2
    | false, false => 0
    | _, _ => 1
  change k * 2 ^ 128 +
  ...
  have htotal : x.toNat * y.toNat =
      ...
      l.toNat := by
    omega
  have hraw : hh.toNat + lh.1.toNat + hl.1.toNat + k < 2 ^ 128 := by
    omega
  have hc : c.toNat = k := by
    dsimp only [c, k, lt, l, lhl, hll]
    exact B128.toNat_carry
...
lemma B256.toNat_mul (x y : B256) :
  ...
  change (B256.mul x y).toNat = _
  unfold B256.mul
  let ll : B256 := x.2.mulx y.2
  let lh : B256 := x.2.mulx y.1
  let hl : B256 := x.1.mulx y.2
  rw [show x.2.mulx y.2 = ll from rfl, show x.2.mulx y.1 = lh from rfl,
    show x.1.mulx y.2 = hl from rfl]
  dsimp only
  have ell : ll.toNat = x.2.toNat * y.2.toNat := by
    simpa only [ll] using B128.toNat_mulx x.2 y.2
  have elh : lh.toNat = x.2.toNat * y.1.toNat := by
    simpa only [lh] using B128.toNat_mulx x.2 y.1
  have ehl : hl.toNat = x.1.toNat * y.2.toNat := by
    simpa only [hl] using B128.toNat_mulx x.1 y.2
  have hxnat : x.toNat = x.1.toNat * 2 ^ 128 + x.2.toNat := by
    rw [show x = (x.1, x.2) from (Prod.eta x).symm, B256.toNat_mk]
  have hynat : y.toNat = y.1.toNat * 2 ^ 128 + y.2.toNat := by
    rw [show y = (y.1, y.2) from (Prod.eta y).symm, B256.toNat_mk]
  have elhdec : lh.toNat = lh.1.toNat * 2 ^ 128 + lh.2.toNat := by
    rw [show lh = (lh.1, lh.2) from (Prod.eta lh).symm, B256.toNat_mk]
  ...
  have heq : x.toNat * y.toNat =
      x.1.toNat * y.1.toNat * 2 ^ 256 +
      lh.1.toNat * 2 ^ 256 + hl.1.toNat * 2 ^ 256 +
      (ll.toNat + lh.2.toNat * 2 ^ 128 + hl.2.toNat * 2 ^ 128) := by
    rw [hxnat, hynat]
    calc
      _ = x.1.toNat * y.1.toNat * 2 ^ 256 +
          (x.2.toNat * y.1.toNat) * 2 ^ 128 +
          (x.1.toNat * y.2.toNat) * 2 ^ 128 +
          x.2.toNat * y.2.toNat := by ring
      _ = _ := by rw [← ell, ← elh, ← ehl, elhdec, ehldec]; ring
  rw [heq]
  simp [Nat.lo_eq, Nat.add_mod]
