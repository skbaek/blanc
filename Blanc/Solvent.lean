-- Solvent.lean : formal proof of solvency preservation for the ported WETH contract
...
def wbsum (f : Storage) : Nat := sum f.rest
    ...
    · constructor
      ...
      · have h_sv : Storage.Solvent (s.stor wa) 0 (s.bal wa) := by
        ...
        have h_le : (s.bal wa).toNat ≤ (r.bal wa).toNat := by
          ...
          | rev => cases h_run
          | dest =>
            rcases of_run_dest h_run with ⟨a, h_le, bal, hd, hi⟩
            by_cases ha : a = wa
