...
by the `.succ` arm; the leaf's `PUSH`/`EQ` and its taken arm; then `balanceOf`'s
own body.
...
/-- 2241 gas, of which `gasColdSload`'s 2100 is the storage read. -/
theorem balanceOfGas_eq : balanceOfGas = 2241 := by decide
...
* `1` — the leaf's `EQ`, which matches, so `balanceOf`'s own body is taken and
  the `.call 1` miss arm is not;
...
theorem weth_balanceOf_succeeds {sevm : Sevm} {pre : Devm}
  ...
  obtain ⟨post, h_run, h_out⟩ :=
    weth_balanceOf_runCompiled h_sel h_stack h_mem h_cold h_gas
