...
| [7934](https://eips.ethereum.org/EIPS/eip-7934) RLP block-size cap | Rejects an oversized RLP-encoded execution block | **Touches the envelope** | It does not alter message execution, but an oversized authored block would be inadmissible. The lane must bound its singleton transaction and block payload below the Osaka limit independently of gas. | Exact encoded transaction/payload byte bound; no claim from gas alone. |
