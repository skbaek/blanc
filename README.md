...
2. **the axiom audit** below, which is stricter than Jaune's own gates: its
   current source inventory pins the exact axiom set of 1145 named results and
   fails on an extra *or* missing axiom.
   Run `scripts/check.sh --no-build`; its `1145/1145` summary belongs to the
...
current source inventory contains **1145** top theorems. `scripts/check.sh`'s
