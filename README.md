...
2. **the axiom audit** below, which is stricter than Jaune's own gates: its
   current source inventory pins the exact axiom set of 362 named results and
   fails on an extra *or* missing axiom.
   Run `scripts/check.sh --no-build`; its `362/362` summary belongs to the
...
current source inventory contains **362** top theorems. `scripts/check.sh`'s
