...
2. **the axiom audit** below, which is stricter than Jaune's own gates: its
   current source inventory pins the exact axiom set of 708 named results and
   fails on an extra *or* missing axiom.
   Run `scripts/check.sh --no-build`; its `708/708` summary belongs to the
...
current source inventory contains **708** top theorems. `scripts/check.sh`'s
