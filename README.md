...
2. **the axiom audit** below, which is stricter than Jaune's own gates: its
   current source inventory pins the exact axiom set of 1231 named results and
   fails on an extra *or* missing axiom.
   Run `scripts/check.sh --no-build`; its `1231/1231` summary belongs to the
...
current source inventory contains **1231** top theorems. `scripts/check.sh`'s
