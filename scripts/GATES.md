...
| `scripts/check-keccak-rate-boundary.sh` | holds every independent in-repo pure-Python Keccak-256 sponge to rate-straddling digests derived from the pinned execution-specs oracle `ethereum.crypto.hash.keccak256` at `4198b9c5996713b268aed602739d5aa40e277694`, not from anything in this repository. The lengths bracket every `len % 136` boundary that a wrong `pad10*1` merge can hide (135, 271, 407, 543 and their neighbours) plus the two published selector digests; the declared enumeration is additionally reconciled against a **structural** sponge scan of `scripts/**/*.py`, so a surface that grows or loses an implementation fails here rather than escaping the control. The scan reads each file's syntax tree, not its text, and recognises a sponge by the Keccak-f round constants, by the rho rotation offsets over 64-bit lanes, or by the rate/lane/pad parameters — values, so notation cannot hide them — with a name-shaped arm that never fires alone. It replaced a `padded = bytearray(` text match on 2026-09-08: that pattern detected a local variable's name, and a scan of its shape is what hid the eighth defective sponge for a session. The driver's self-test plants two tenth implementations written differently — one tabulating the constants in decimal, one deriving them from the LFSR, both carrying the historical defect — and requires the reconciliation to redden and the vectors to catch them; the retired pattern is required not to match the first. **Known limit, pinned by its own control:** a sponge that tabulates neither the constants, the offsets nor the sponge parameters, deriving all of them from variables, is not detected. Reverting only the 2026-09-07 `pad10*1` repair reddens it with 8 x 4 rate-boundary failures | 9 implementations x 14 lengths + 2 selectors = 144 comparisons; 1 enumeration reconciliation; 2 planted sponges and 1 pinned limit | sub-second |
