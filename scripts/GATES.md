...
| `scripts/check-drip.sh` | CI-safe artifact/oracle controls and mocked evaluator protocols, then 39 real Lean arithmetic rows compared with independently computed Python integers, and every committed BPO2 fixture through pinned Jaune. Requires owned full/runner build, current evaluator imports and adjacent artifact-integrity/certification; never builds implicitly. Source/compiler/cache/import identity drift, missing prerequisites, malformed arithmetic output or incomplete native replay fail closed. Finite arithmetic comparison is not the deployed-loop theorem. Native replay authenticates block execution and state/receipt roots; independent manifest receipt-annotation authentication remains pending | 38 frozen obligations; 50 mocked evaluator/identity tests; 39 independent arithmetic rows; expected 91 fixtures / 137 transactions; 1,762-byte runtime + 2,001-byte creation; standalone arithmetic validated; complete family replay pending | arithmetic validated; complete replay unmeasured; contained exclusive execution |
| `scripts/check-drip-current-mainnet.sh --composed-prerequisites` | DRIP's isolated BPO2 consumer first consumes the shared current-mainnet verdict, then runs `gen-drip-fixtures.py --check-runtime` under the verified target interpreter and byte-compares its complete generated JSON population. The fixture population is currently absent and is established only by the generator's explicit `--write` mode after a successful runtime validation; this row therefore has no green runtime verdict yet. CI replay is owned by `check-drip.sh`; its first real integration verdict remains pending | 38 obligation-owned runtime scenarios, including exact block/header/signature/transaction-root and observer/rollback assertions once generated | unavailable until the generated population exists |
