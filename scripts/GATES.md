...
Import `keccak256`, `keccak256_hex`, or `selector` from `scripts/keccak.py`.
They return, respectively, exactly 32 digest bytes, lowercase `0x`-prefixed
digest text, and a four-byte selector; `selector` rejects signatures containing
spaces.  The implementation is dependency-free and suitable for generators
...
independently frozen external/reference artifact.  Do not make a generator and
its expected-value constructor agree by importing this helper into both.
...
| `scripts/check-keccak-rate-boundary.sh` | holds the canonical helper and every remaining independent in-repo pure-Python Keccak-256 sponge to rate-straddling digests derived from the pinned execution-specs oracle `ethereum.crypto.hash.keccak256` at `4198b9c5996713b268aed602739d5aa40e277694`, not from anything in this repository. It separately pins the Beacon and TWG compatibility adapters' bytes, bare-hex, and `0x`-hex return shapes. The lengths bracket every `len % 136` boundary that a wrong `pad10*1` merge can hide (135, 271, 407, 543 and their neighbours) plus the two published selector digests; the declared enumeration is additionally reconciled against a **structural** sponge scan of `scripts/**/*.py`, so a surface that grows or loses an implementation fails here rather than escaping the control. The scan reads each file's syntax tree, not its text, and recognises a sponge by the Keccak-f round constants, by the rho rotation offsets over 64-bit lanes, or by the rate/lane/pad parameters — values, so notation cannot hide them — with a name-shaped arm that never fires alone. It replaced a `padded = bytearray(` text match on 2026-09-08: that pattern detected a local variable's name, and a scan of its shape is what hid the eighth defective sponge for a session. The driver's self-test plants two implementations written differently — one tabulating the constants in decimal, one deriving them from the LFSR, both carrying the historical defect — and requires the reconciliation to redden and the vectors to catch exactly 135, 271, 407, and 543 while adjacent lengths and selectors stay green; the retired pattern is required not to match the first. **Known limit, pinned by its own control:** a sponge that tabulates neither the constants, the offsets nor the sponge parameters, deriving all of them from variables, is not detected. | 8 sponges x 14 lengths + 2 selectors = 128 comparisons; 2 migrated adapters; 1 enumeration reconciliation; 2 planted sponges and 1 pinned limit | sub-second |
...
| `scripts/test-keccak-rate-boundary.py` | `check-keccak-rate-boundary.sh` | enumerates every independent Keccak-256 sponge in `scripts/`, reconciles that enumeration against a structural scan of each file's syntax tree, compares each implementation with the external rate-boundary vectors, and runs the planted-sponge self-test that shows the scan catches an implementation written differently |
