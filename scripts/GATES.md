...
| `scripts/check-drip.sh` | CI-safe DRIP arithmetic/artifact controls, complete structural manifest validation, mocked replay and evaluator-protocol controls, then every committed BPO2 fixture through the existing pinned Jaune executable. Missing fixtures/runner, changed literal/manifest population, nonzero child exit or incomplete case selection fail closed. No implicit build or external EELS checkout. Jaune authenticates canonical block execution and state/receipt roots; manifest receipt annotations and independent arithmetic replay remain separate obligations; the 41 evaluator-protocol tests mock every child and do not establish those obligations | 38 frozen obligations; 41 mocked evaluator tests; expected 91 fixtures / 137 transactions; 1,827-byte runtime + 2,066-byte creation; real replay integration pending | real replay unmeasured; contained exclusive execution |
