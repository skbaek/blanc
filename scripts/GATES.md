...
| `scripts/generate-proof-recipes.py` | `check-proof-recipes.sh` and explicit `--write` regeneration | validates the strict TOML registry, referenced symbols, example anchors and review dates through the shared module-path policy, validates disjoint explicit trigger inventories in core `Tactics` and leaf `ProofRecipeTactic` with final false fallbacks, then deterministically generates `docs/PROOF_RECIPES.md` and the import-safe `Blanc.ProofRecipesGenerated` lookup table, consumed only by the unimported `Blanc.ProofRecipeTactic` leaf, through raw-string, alias-checked output bindings. Trigger soundness is checked in three layers and the terminal line reports what each one actually covered, so a run that could resolve nothing does not read like a run that resolved everything. `validate_trigger_dispatch` resolves, per arm, every `Blanc.` name the arm compares against, with `UNCHECKED_DISPATCH_ARMS` naming each arm it cannot see and why. `validate_jaune_dispatch` closes the other namespace, which was unchecked in two distinct ways: six arms dispatch on `Jaune.` names only, and two further arms sat *inside* the checked population with silently unchecked `Jaune.` disjuncts. `JAUNE_DISPATCH_SURFACE` therefore names all 32 such names, must equal exactly what the arms dispatch on in both directions, and is bound to `JAUNE_DISPATCH_PIN`, which must equal the `lake-manifest.json` and `lakefile.lean` rev — so a pin bump, the event that silently kills these arms, fails until the surface is re-verified, with no dependency materialized. When `.lake/packages/jaune` is materialized at that exact rev every name is additionally resolved against a census of its library (`Jaune.lean` and `Jaune/**` only, reading `mutual` scopes, French-quoted structure fields and constructors, none of which Blanc's own reader needs); the dependency being absent is reported, never silently skipped. `validate_harness_coverage` enumerates the registry against `scripts/ProofRecipeSuggestions.lean`, because liveness is not reachability and reachability is not statically decidable here: the dispatcher's head comes from a goal at tactic time, neither the registry nor `Blanc/Tactics.lean` contains a goal, and the property belongs to the corpus of statements authors write. The only evidence is an exhibited goal, so every registered recipe has a `-- EXPECT:` case or is named in `UNWITNESSED_RECIPES` with what a case would have to exhibit, every `-- EXPECT:` id is registered, every trigger string in the harness names a real arm — a misspelled `expect_no_recipe_trigger` otherwise passes over nothing, because the dispatcher's fail-closed wildcard returns false for an unknown trigger — and the triggers with a positive `expect_recipe_trigger` case, the only machine-checked reachability evidence in the repository, are pinned by name in `REACHABILITY_WITNESSED_TRIGGERS` so a lost witness cannot be masked by an unrelated new one. A `-- EXPECT:` comment and a bare `blanc_suggest` are narration, not evidence: the tactic only logs, so an example compiles whether or not any trigger matched. `--check` byte-compares both outputs and `--self-test` exercises 30 schema/trigger/matcher-ownership/trigger-dispatch/Jaune-dispatch/harness-enumeration/symbol/anchor/drift controls, the shared 15-control path campaign, 4 actual registry/aggregate out-and-back controls, and the optional aggregate wrong-case control |
...
| `scripts/check-lido-circuit-breaker-deployment.py` | `check-lido-circuit-breaker-deployment.sh` | comment/string-aware extraction and complete-body pins for the exact public deployment family, plus independent semantic-fragment, premise-smuggling, prepared-state, execution-site, trust, layering, and scope controls; derives all 165 public theorem names across the nine new owners and pins their exact axiom-inventory/expectation channels |
