...
|---|---|---|
| `scripts/generate-proof-recipes.py` | `check-proof-recipes.sh` and explicit `--write` regeneration | validates the strict TOML registry, referenced symbols, example anchors and review dates, then deterministically generates `docs/PROOF_RECIPES.md` and the import-safe `Blanc.ProofRecipesGenerated` lookup table consumed only by the unimported `Blanc.ProofRecipeTactic` leaf; `--check` byte-compares both outputs and `--self-test` exercises schema, trigger, symbol, anchor, and drift controls |
