...
|---|---|---|---|
| `scripts/check-drip-stack-certificate.sh` | **LOCAL DRAFT:** byte-compares deterministic finite stack data and fixed untrusted proof text against analysis of the compiler-owned DRIP runtime and runs disposable byte, row, branch, height, post-pattern, named-subtree, region-topology, cross-region successor and complete same-frame facade controls. This is data/source provenance, not a substitute for the kernel-checked production certificate required in Phase 2; it invokes no Lean or external evaluator | accepted 735-row table over the exact 1,762-byte runtime; 30 deterministic test groups | unmeasured on this candidate (LOCAL DRAFT) |
