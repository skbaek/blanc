...
| the execution-settlement substrate, its consumers, lift manifest, retained-trace compatibility surface, or the Jaune-relocation manifest | `scripts/check-extraction-ownership.sh` + `scripts/check-execution-settlement.sh` | the **full set**, in the order below |
...
| the harness, fixture or control table of the extraction-ownership, execution-occurrence, cycle-write-free, transient-settlement or Lido deployment gate (including `scripts/lean_mutant_batch.py`), or a DRIP corruption or mocked-protocol suite | that gate's `--self-test` | the **full set**, in the order below |
...
| the proxy-pair upgrade programs, R2 relation, migration/identity realization, compiled forwarding result, stack-certificate producer/generated data, public claim boundary, executable witnesses, or axiom pins | `lake build` then `scripts/check-proxy-pair-upgrade.sh` | the **full set**, in the order below |
...
| the DRIP BPO2 fixture generator, block/observer/cost helpers, measurement controls, generated fixture population, or external consumer wrapper | `scripts/check-current-mainnet.sh` then `scripts/check-drip-current-mainnet.sh` | the **full set**, in the order below |
| the Lido CircuitBreaker × TWG BPO2 consumer, Prague→Osaka applicability ledger, compiler-owned artifact evaluator, composed/family replay rows, or generated result | `scripts/check-current-mainnet.sh` then `scripts/check-lido-twg-pinned-target-current-mainnet.sh` | the **full set**, in the order below |
| a module's imports, or added a contract | `scripts/check-layering.sh` | `lake build && scripts/check.sh --no-build` |
| a proof, a theorem statement, or an axiom-relevant definition | `scripts/check.sh --no-build` | `scripts/check-elab.sh --no-build` after the goal-owned build |
| the proof-recipe dispatch `Blanc.proofRecipeMatches`, either trigger matcher, the recipe registry, or the suggestion harness | `scripts/check.sh --suggestions-only` | `scripts/check-proof-recipes.sh`, then `scripts/check.sh --no-build` |
...
| anything that could move elaboration cost | `scripts/check-elab.sh --no-build` after the goal-owned build | — |
| a new module that must state its elaboration cost | `scripts/check-elab.sh --calibrate --no-build` after the goal-owned build | — |
| the elaboration selector, cache contract, or timing-gate implementation | `scripts/check-elab.sh --self-test` and `python3 scripts/test-check-elab-warmup.py` (mocked shell routing) | `scripts/check-elab.sh --full --no-build` after the goal-owned build |
| the read-only elaboration-migration comparator or its synthetic controls | `python3 -B scripts/test-elab-migration-comparison.py` | run the production comparator only after two explicit, clean shared-baseline records exist |
...
| `scripts/check-prorata-weth-vault-reference.sh` | verifies offline that the vendored OpenZeppelin v5.7.0 closure is exactly the 17-source closure frozen at G1 (per-file SHA-256 and Git blob, upstream MIT LICENSE, tree membership), that the committed standard-JSON input is built from those bytes under the frozen settings, that the committed `solc 0.8.36+commit.8a079791` output carries the frozen creation/runtime template identities and the creation input with the configured asset word, and that the reference's method identifiers are exactly the 25 signatures of `vaultFuncs` with no permit or ERC-165 on either side. The two optional recompilation legs are independent and separately reported. `--recompile` runs the `$SOLC` binary whose SHA-256 is the lock's recorded *native* identity; that build is reconnaissance against the frozen output and is not the compiler the SF selects. `--recompile-wasm` runs only the SF-selected `emscripten-wasm32` `$SOLJSON` artifact, and is the leg that carries selected-compiler evidence; both require the same frozen output identities and neither may move a frozen identity | 17 sources + LICENSE; 3 frozen artifact identities; 25 selectors; 2 optional recompilation legs (1 native reconnaissance, 1 SF-selected wasm) | sub-second |
| `scripts/check-prorata-weth-vault-oracle.sh` | runs the independent exact-integer oracle written from the frozen statement: the representability identity, both capacity bounds' tightness, the four rounding directions, round-trip non-profitability, ledger conservation over randomized transcripts and donation classification over 63 boundary states, regenerates the committed golden vectors byte-for-byte, and requires the offset-disabled control to bite | 12 property batteries; 63 boundary states; committed vectors; 1 biting control | sub-second |
| `scripts/check-prorata-weth-vault-differential.sh` | executes the committed 17481-byte vault runtime and the 4347-byte constructor-patched OpenZeppelin reference (deployed on Jaune from the locked creation input against Blanc's WETH and identity-checked) through `jaune t8n` at BPO2, projects each side's storage through its own layout, and compares normalized application state, status, exact logs and captured return data against the independent oracle over one boundary matrix: the empty, nonempty and donated states; all 25 selectors on both sides (a selector that drops out fails); every rounding direction observed on inexact divisions; the zero-receiver and nonpayable guards and malformed calldata; a representative failure with complete rollback for every mutation (exact-WETH child failures, spent-allowance restoration, a rejected transaction distinguished from an accepted revert); the ERC-20 allowance boundary; zero-amount flows; pre-transfer quote order; `max*` attainability at a near-cap state; registered deviations 2, 3, 5 and 6 (Blanc-empty versus reference `Panic(0x11)` returndata, the A=U 257-bit route); the SF A2 receiver-wrap payouts; the D9 finite evaluation over executed allowance pairs; and the frozen first-depositor transcript. It regenerates the committed F14 wrap proof and byte-compares both runtime sizes and six gas rows with `scripts/prorata-weth-vault-reference-measurements.json`. Unique protection: fidelity of the compiled runtime to the frozen statement at its boundaries, agreement with the compiled reference, reference identity and measurements. Families that a PRORATA G4-G7 theorem covers, and EVM conformance, are not re-run (evidence economy, below; ledger in Plans `reports/evidence-economy-20260923/vault-differential-ledger.md`) | 31 check groups per compiled side; 25 selectors × 2 sides; 1 reference deployment; 6 gas rows | ~24 s (measured 2026-09-23; was 158 s) |
| `scripts/check-prorata-weth-vault-oracle.sh --self-test` | perturbs the oracle (offset, each rounding direction, capacity and conservation formulas) in an isolated copy and requires every property battery to notice | 8 perturbations | sub-second |
| `scripts/check-prorata-weth-vault-differential.sh --self-test` | shows the kept controls bite: 11 mutants in a disposable copy — the oracle's offset and its four rounding directions, the locked reference runtime identity, the committed measurements, the exact rollback projection, the deposit event order, the zero-receiver guard expectation and selector coverage — each run the narrowest `--only CHECK --side SIDE` mode and must print their own named diagnostic; restoring is shown by the restored file's SHA-256 equalling the committed file's, whose green verdict is the main row (a `depends_on` prerequisite), never by a rerun. In-process probes add the valid-call-as-revert status diagnostic, four receipt/rollback falsifiers and five executed return-capture controls. Its cache inputs are the harness's own files only | 11 mutants; 1 valid-call probe; 4 receipt/rollback falsifiers; 5 return-capture controls | ~20 s (measured 2026-09-23; was a 2041 s median, 7618 s maximum) |
| `scripts/check-prorata-weth-vault-reference.sh --self-test` | corrupts a vendored source byte, the lock, the committed output's bytecode, tree membership (extra and dropped source), the optimizer settings and the vault's own selector table in temporary copies; each corruption must make the reference gate fail with its own named diagnostic, and undoing only that corruption must restore green | 7 corruptions | sub-second |
| the DRIP exact-integer model, canonical vectors, committed replay, or evidence checker | `scripts/check-drip.sh` | the **full set**, in the order below |
...
### I need stdlib-only Ethereum Keccak-256 or ABI selectors in a Python script

Import `keccak256`, `keccak256_bare_hex`, `keccak256_hex`, or `selector` from `scripts/keccak.py`.
They return, respectively, exactly 32 digest bytes, bare lowercase digest text,
lowercase `0x`-prefixed digest text, and a four-byte selector; `selector` rejects
signatures containing spaces.  The implementation is dependency-free and suitable for generators
and primary evidence checkers.  It uses Ethereum's `0x01` domain and correctly
merges the final `0x80` bit when an input ends one byte short of the 136-byte
rate.  It is not an independent oracle: expected values must still come from
the frozen vector module, the pinned EELS/pycryptodome oracle, or an
independently frozen external/reference artifact.  Do not derive frozen expected hash values
with the implementation under test. Shared hashing does not replace independent
schema, artifact-provenance or semantic checks.
...
a new population and evidence identity first. Use
`scripts/check-elab.sh --full --no-build` after the goal-owned build, without
rebasing, inventory proof debt separately, and run Jaune by the
...
scripts/check-elab.sh --no-build      # only if a .lean file was touched
...
scripts/check-drip-current-mainnet.sh --composed-prerequisites
scripts/check-lido-twg-pinned-target-current-mainnet.sh --composed-prerequisites
scripts/check.sh --suggestions-only
python3 -B scripts/test-elab-migration-comparison.py
scripts/check-extraction-ownership.sh --self-test
scripts/check-execution-occurrence.sh --self-test
scripts/check-cycle-write-free.sh --self-test
scripts/check-transient-settlement.sh --self-test
scripts/check-lido-circuit-breaker-deployment.sh --self-test
scripts/check-drip.sh --self-test
```

The full set uses `check-elab.sh --no-build` after the exact goal-owned build
has been certified. Its content-addressed local cache measures only modules
whose own source, transitive repository-local import closure, shared Lean/Lake
configuration, or Lake-recorded transitive imported artifacts changed since
their last successful measurement. It still represents and baseline-checks
every module,
...
**CI trust boundary.** Production CI executes its 41 registered commands fresh
...
Every row also carries a measured cost that nobody typed (evidence economy
rule 4): `scripts/gate-economy.py --import-costs` reads the selective runner's
shared evidence ledger — the `duration_s` of every retained green record — and
freezes the median and newest duration per row into
`scripts/gate-measured-costs.json`, which `docs/GATE_ECONOMY.md` quotes beside
the cell below. A cell here that says *ledger median* was filled from that
import; re-import when the ledger has newer evidence, and the import's UTC and
record counts say how current it is.

### Cheap — run these constantly

| gate | proves | scale | time |
|---|---|---|---|
| `scripts/check-drip-stack-certificate.sh` | byte-compares deterministic finite stack data and fixed untrusted proof text against analysis of the compiler-owned DRIP runtime and runs disposable byte, row, branch, height, post-pattern, named-subtree, region-topology, cross-region successor and complete same-frame facade controls. This is data/source provenance for the generated certificate; the kernel-checked certificate is `Blanc.Drip.StackSafety.table_checked` / `actual_entry_safe` (both in `Blanc/DripStackSafetyCertificate.lean`), checked by the separately required Lean builds; this gate invokes no Lean or external evaluator | accepted 735-row table over the exact 1,762-byte runtime; 30 deterministic test groups | sub-second (ledger median 0.9 s of 3 record(s), imported 2026-09-23) |
| `scripts/check-doc-counts.sh` | every published claim this tree determines agrees with what the tree produces. A published claim here is one of three things, and nothing else: a NUMBER whose producer is a committed artifact readable with no toolchain, no build and no network (the audited-theorem count from `scripts/AxiomCheck.lean`, the claim-pin count from `scripts/ClaimCheck.lean` — which `scripts/check-claims.sh` restates as its own hardcoded expectation, so a `sed` on that literal now fails here — and the WETH10 differential row count from the committed `scripts/fixtures/weth10/manifest.json`, and the Lido CircuitBreaker differential row count from the committed `scripts/fixtures/lido-circuit-breaker/manifest.json`); a published verbatim GATE TRANSCRIPT whose wording is determined by that gate's own verdict format string; or a published REFERENCE into this tree, whose target must exist and, for a Lean file, be imported by `Blanc.lean`. Prose judgment is deliberately out of scope: scoring published prose against the tree would fail on rewordings that are not drift and pass on drift that is not a rewording. Anti-vacuous twice over — per pattern, so a surface reworded out of the gate's sight FAILS rather than passing silently; and per surface, where a census of standalone occurrences of each produced value makes an unregistered or drifted quotation fail even though no registered pattern covers it. A number is registered only when its producer is static AND its occurrences can be enumerated exactly, so that coverage is whole rather than partial; the rest are named in the script with their producer and their blocker, under a count that cannot grow silently. The catalogue row you are reading is checked against the run, so widening the gate without saying so here FAILS. Jaune's site quotes the audited-theorem count and no gate can cross the repository boundary, so a pass prints that reminder rather than implying coverage it does not have | 64 quotations across 8 surfaces for 10 produced claims; 3 published transcripts pinned to their gate's own verdict wording; 7 published repository references; 7 published numbers deliberately unchecked and named in the script | sub-second |
| `scripts/check-lido-circuit-breaker-assurance.sh` | every claim `LIDO_CIRCUIT_BREAKER_ASSURANCE.md` makes is still true of the tree it describes. Each `####` row under a `## Pillar —` heading must carry all seven labelled fields exactly once, in the frozen order, non-empty; each name in a **Declarations** field must be pinned — fully qualified, never by last component, because `Blanc.Weth10.…` and `Blanc.LidoCircuitBreaker.…` share last components — by the authority that row's own **Gate** field names, which is what makes that field load-bearing rather than decorative; each **Axioms** field must equal that authority's expectation exactly and order-insensitively, with an empty expectation required to be written as the single word `none` and both directions checked; each **Gate** path must exist and be catalogued here; and each pinned load-bearing non-claim phrase must still be written somewhere. A fifth pinned list holds the clauses of the register's **shared-shape paragraph**, which is the only statement of six registration rows' common premises — each of those rows says "the shared registration shape above, plus …" and lists only its additions — so a clause reworded away silently strips premises from all six, and its three counts (all six, five of six, four of six) go stale the moment a chronology is added. Blanc has SEVERAL axiom-expectation authorities, and this gate reads every one of their pin tables without writing any of them, with `ast`, never importing or executing them: `scripts/check.sh`'s `ROWS` table over `scripts/AxiomCheck.lean` (with `check-lido-circuit-breaker-deployment.sh` registered as an alias for it, since that gate's axiom section verifies exactly those two files), and the access, enumeration and registry gates' own tables. A name pinned by two authorities that disagree is reported as a REPOSITORY INCONSISTENCY and resolved in neither direction. `check-lido-circuit-breaker-history.sh` states a uniform expectation over a population DERIVED from its owner modules rather than read from a table — every public `theorem`/`lemma` its own probe covers — so that population is reproduced here by the same rule, and independently comes to the same 104 names that gate reports. Its `HEADER_PINS` digest table is a different and wider population that also covers the `def`/`structure` layer; those names are RESOLUTION-ONLY, which is what keeps `RegistryStable`, `Coherent` and `registrySpec` out of **Declarations** while the register goes on citing them as premise vocabulary. A row carrying real evidence with no pinned declaration behind it writes the literal `no audited declaration — gate-owned row` and `not applicable`. Anti-vacuous by construction: per-pillar row counts, the total, the gate-owned count and the count of rows naming more than one gate all live in the checker's own source, so a row deleted, renamed, reworded out of the gate's sight FAILS rather than shrinking a green count, and a missing, unparseable or empty register is a REGRESSION rather than a green run over nothing. The last two counts pin the gate's two escape hatches, in both directions: a row may decline the axiom check by becoming gate-owned, and a row may make a MIS-ATTRIBUTED declaration resolve by naming a second authority — appending `scripts/check.sh` to a row whose family gate does not pin the name is measured to rescue it, so multi-gate rows are counted rather than trusted. What it does NOT own: it elaborates no Lean and re-derives no axiom set, and it does not judge whether a row's prose is a fair summary, whether **Premises** are complete, or whether a **Differential channel** names a real oracle case — those are review obligations, and mechanising a pretence of them would be the vacuity this gate exists to prevent. ITS DEFAULT MODE IS STATIC and its authority over the axiom column is exactly that of the gates whose tables it reads. `--probe` is an OPTIONAL NON-DEFAULT mode that closes that loop directly rather than transitively — it regenerates a from-scratch `#full_axioms` probe (the shared walker, the pinned Jaune's `AxiomAudit` module, driven by `scripts/axiom_audit.py`) from the register's own citations and elaborates it with `lake env lean` — and REQUIRES the Lean toolchain and a built dependency graph; it is not what CI or this row runs, and must not be run beside a measurement that owns the host | 75 rows across 11 pillars, 9 gate-owned, 7 naming two gates; 158 cited declarations, all 158 axiom expectations matched; 82 gate paths; 14 pinned non-claim phrases; 4 pinned shared-shape clauses; resolved against 5 authorities — repository audit 1331 pins, access 249, history 104 derived public theorems (plus 155 resolution-only digest pins), enumeration 10, and registry 38 pinned names of which 4 carry an axiom expectation | sub-second |
...
| `scripts/check-proxy-pair-upgrade.sh --static-only` | static ownership, generic/product placement, claim text, selector/layout and corpus-wide shadow/export controls | 10 headlines; 3 assurance theorems; 3 generic definitions | sub-second (ledger median 0.4 s of 12 record(s), imported 2026-09-23) |
| `scripts/check-layering.sh` | contracts are siblings in the import hierarchy: no cross-contract import, no shared module importing a contract, and the `Blanc/Composition/*` stratum strictly downstream of both. Its header-only reader follows Lean 4.32.1's `public? meta? import all? identWithPartialTrailingDot` grammar, including quoted and Lean `isLetterLike` identifier components, nested/trailing comments and line-spanning whitespace; it stops at the first non-header command. It fails closed on unterminated header comments/strings, incomplete trailing-dot names, `public import all`, and reversed `meta public import` modifiers, each paired with Lean. `imports_of` is category-agnostic: it reads a future local module without consulting the classification table, so later category rules reuse the reader. A fifth check attaches a discovery obligation to the same SHARED classification: every active `scripts/proof-recipes.toml` recipe whose `owner_module` is classified shared must have that module path cited in `docs/COMMON_API.md`, and every `Blanc/*.lean` path that file cites must exist — the missing-entry and stale-entry directions of a reusable facility the need-first registry cannot reach. Both populations are asserted non-empty, so a registry reworded out of the check's sight fails rather than passing over nothing; it judges reachability only and is deliberately not a similarity detector. A sixth check closes the population the fifth cannot see: check 5 binds only a shared module that *owns an active recipe*, so a SHARED module with no recipe escaped it entirely — `Blanc/SymbolicProgram.lean` shipped as a 1,091-line shared facility with no `docs/COMMON_API.md` entry and no recipe while the gate reported green. Check 6 therefore attaches the discovery obligation to the SHARED classification itself: every module the table calls shared must have its path cited in `docs/COMMON_API.md`, and the only escape is a `DISCOVERY_EXEMPT` row naming the module and the concrete reason it is not a facility an author could be told to find (a compatibility re-export that declares nothing, the registry's own machinery, a control fixture). That table is validated in both directions — an exemption for a module that is absent, no longer shared, or now cited fails — so it prunes itself instead of absorbing the next shipped facility. Check 6 requires no recipe (`docs/COMMON_API.md`'s own workflow makes a recipe conditional on a reliable goal shape existing), judges no entry's quality, and binds exactly the SHARED-registration-to-registry-entry relation: an edit to an already-cited shared file is unaffected. `scripts/check-layering-controls.py` independently pairs each accepted form with Lean elaboration (rule and rationale in `README.md`) | 10 contracts, 495 modules, 493 non-root, 40 composition downstream, 38 shared-facility recipes over 126 registry citations, 109/113 shared modules cited and 4 exempt with a stated reason; 11 accepted-form, 3 rejected-header, 3 non-import, 3 malformed-header, 3 architecture, 5 composition-edge, 4 discovery, 3 shared-coverage and 1 category-agnostic controls | sub-second (production gate); ~14 s controls |
| `scripts/check-proof-recipes.sh --base main` | first fail-closed checks the proof-recipe registry and its generated documentation/Lean lookup, then reports high-confidence anti-patterns only in declarations changed from the selected base: byte-identical imported declaration copies after name/documentation normalization, with a parsed-header-only fallback that additionally erases `private`/`protected` and their immediate raw spaces/tabs while preserving header comments, attributes, options, declaration kind, semantic modifiers, body bytes and other whitespace, and new local selector tables outside the registry-declared owner. Every module/source path passes the shared raw-string validator before path construction, then exact-entry, no-symlink, single-link and resolved-containment enforcement; the bidirectional checked census covers the recursive walk, Git/import source index, three registry dereferences and two generated-output bindings. Its exact-source parser supports the production wrapped form whose modifier/kind line is followed immediately by an indented qualified name, retaining the keyword boundary and the name's original span; unsupported declaration-looking syntax still fails closed. Exceptions are declaration-scoped, active-recipe-bound, expiring, and reject wildcards, duplicates, or orphans | changed declarations only; 2 report-only finding classes; 3 parser-header controls, the anonymous-instance boundary, 7 legacy detector controls, 14 imported-copy visibility controls, the generator's 31 schema/symbol/trigger-dispatch, Jaune-dispatch-surface and registry/harness-enumeration controls and 21 module-path controls (15 shared policy controls, 5 actual explicit-site out-and-back cases, and the optional aggregate wrong-case case), and its `--self-test` also runs the 18 duplication controls of the row below | `--self-test` 2 min 56 s measured on this host (the generator's own 2 min 43 s of it); an ordinary run is ~4.8 s (measured 2026-09-24 on an empty diff; was 19 s): the registry pass now strips each module once per process, by token. The superseded ~30 s figure was already stale before this cycle — the 14-control predecessor measures 89 s on the same host — and each added control is one more whole-tree `load_and_validate` |
...
| `scripts/check-extraction-ownership.sh` | the manifest's 14 execution-settlement declarations exist only under their common owner, now Jaune's `Jaune/ExecSettlement.lean` in the pinned package; no donor declaration or common-owner basename shadow survives across the historical WETH10 donor family or the Lido family; all 21 exact compatibility abbreviations are present without drift; no alias/export survives; and `Weth10HolderFlow` directly imports the common module. It also reads `scripts/jaune-exec-relocation-manifest.json`, the 197 handwritten declarations of the canonical execution layer that moved from Blanc to Jaune: each must be declared under its `Jaune.*` name by its named module of the pinned Jaune package (`RELOCATED-MISSING`), and no `Blanc/**/*.lean` module or `Blanc.lean` may declare it again under its old `Blanc.*` name or its `Jaune.*` name (`RELOCATED-REDEFINED`, a leftover or disconnected copy). Its twelve negative controls run in `--self-test` | 14 moved declarations; 21 exact abbreviations; 197 relocated declarations | ~3 s (was ~26 s with the controls) |
| `scripts/check-extraction-ownership.sh --self-test` | shows every audited channel bites: eleven mutations of a temporary copy of `Blanc/`, `Blanc.lean`, the pinned Jaune package's `Jaune/` sources and both manifests (donor alias/export, flow-block removal, attribution-block drift, Lido shadow/alias, missing common declaration, missing direct import, a leftover `Blanc.Exec` copy, a disconnected `Jaune.*`-named copy declared from Blanc, a relocated declaration missing from its Jaune module) and one trailing-`?` parser probe must each fail with their own tag, after the copy audits clean. The manifests and harness are fingerprinted; the copied Lean population is declared as subject-only reads (evidence economy rule 3) | 12 controls | ~28 s |
| `scripts/check-trust-surface.sh` | exact transitive local import closure of `Blanc.lean` contains no new or stale source occurrence of `sorry`, bespoke `axiom`, `opaque`, `@[extern]`, `implemented_by`, `native_decide`, object-level `partial def`, or `dbg_trace` in code; comments, docstrings and string/character literal text are masked (a brace in a string is read both as literal text and as an interpolation hole, and masked only if both readings agree), and an unterminated comment, string or hole is a setup failure; exact reviewed TacticM/MetaM rows are fail-closed allowlisted; unimported helpers are outside scope until imported | 492 closure modules; 12 exact allowlisted occurrences; 28 lexer controls in `scripts/test-check-trust-surface.py` | sub-second |
...
| `scripts/check-execution-occurrence.sh --static-only` | corpus-wide occurrence owner, header, consumer, shadow, alias, export and proposition-copy controls | 10 moved-owner rows + exact headers and parser controls | ~2.5 s (measured 2026-09-24; was 17 s — the shared ownership parser now memoizes and scans by token) |
| `scripts/check-cycle-write-free.sh --static-only` | the real-tree audit once: exact owner/signature/exemption manifest, contract-wide shadow/alias/export absence and the 22 required positive proofs; the in-memory falsifiers run in `--self-test` | 22 positive proof pins; 19 owners; 7 signatures | ~3 s (was ~62 s with the falsifiers) |
| `scripts/check-cycle-write-free.sh --self-test` | shows the controls bite: a clean real-tree baseline, then 21 in-memory ownership/signature falsifiers and the `Prog.function?` parser check, 22 positive-deletion controls, and the 18 fixture mutants that a pinned positive or fixture-local fact already contradicts, in one elaboration with each diagnostic inside its own lines. Harness and fixture definitions are fingerprinted; contract Lean files are declared as subject-only reads | 22 falsifiers; 22 deletions; 18 mutants | ~8 s |
| `scripts/check-transient-settlement.sh --static-only` | sole shared owner, exact imports/signatures, donor moves, touched consumers and frozen predecessor assurances; the parser falsifiers and deletion controls run in `--self-test` | 14 owners; 12 moved donors | ~9 s |
| `scripts/check-transient-settlement.sh --self-test` | shows the controls bite: parser falsifiers, a clean owner-audit baseline, 14 static owner and 12 moved-donor deletion controls, 2 live positive deletions, and the 20 mutants that restate the negation of a conjunct `concrete_controls` pins, in one elaboration with each diagnostic inside its own lines. Harness, frozen control definitions and fixture are fingerprinted; Blanc Lean files are declared as subject-only reads | 20 mutants; 28 deletions | ~6 s |
| `scripts/check-keccak-rate-boundary.sh` | checks the sole in-repo pure-Python Keccak-256 implementation against frozen digests derived from the pinned external execution-specs oracle `ethereum.crypto.hash.keccak256` at `4198b9c5996713b268aed602739d5aa40e277694`. All evidence consumers reuse that primitive while retaining their own schema and semantic checks. The vectors include 135, 271, 407 and 543 with their immediate neighbors, other selected lengths and two selector digests. Consumer aliases retain bytes, bare lowercase hex and `0x`-prefixed text representations. A structural syntax-tree scan of `scripts/**/*.py` must find exactly `scripts/keccak.py`; it recognizes round constants, rho offsets or rate/lane/pad values, including differently written copies. Two planted extra sponges must fail ownership reconciliation and their historical padding defect must fail exactly the four defective lengths while tested neighbors and selectors stay green. The old variable-name text match is required to miss the first planted form. **Known limit, pinned by its own control:** a sponge that derives all constants, offsets and parameters from variables can evade this structural scan. | 1 canonical sponge x (16 lengths + 2 selectors) = 18 digest comparisons; 7 bare-hex and 4 byte aliases plus TWG text adapters; 1 ownership reconciliation; 2 planted sponges and 1 pinned limit | sub-second |
...
| `scripts/check-lido-circuit-breaker-registry.sh --static-only` | Registry sole-owner/namespace/header pins, forbidden-trust scan and in-memory corpus falsifiers | 24 Registry declarations; 34 SHA-256 headers; 11 falsifiers | ~3.7 s (measured 2026-09-24; was 22 s — each falsifier now re-strips only the text it changed) |
...
| `scripts/check-lido-twg-differential.sh --composed-prerequisites` | evaluates the production TWG creation/runtime family after consuming the separately registered reference and census verdicts; requires strict Blanc wins on runtime, complete CREATE, and all 51 named gas cells; binds the repaired packed-limit/nested-keccak/per-role layout and one-read route through four positive artifact controls plus paired mutants | 71 declared rows; 62 agreement + 9 registered-deviation rows; 2/2 byte wins + 51/51 gas wins; 8 performance control/mutant checks; exact artifact and semantic channels | ~6 s (ledger median of 12 record(s), imported 2026-09-23) |
...
| `scripts/check-prorata-current-mainnet.sh --composed-prerequisites` | PRORATA BPO2 fixture and benchmark regeneration after consuming the shared current-mainnet verdict | 14 fixture blocks + 30 benchmark blocks | ~67 s (ledger median of 4 record(s), imported 2026-09-23) |
| `scripts/check-lido-ossifiable-proxy-current-mainnet.sh --composed-prerequisites` | OssifiableProxy BPO2 replay after consuming the shared current-mainnet verdict | 21 represented primary transaction scenarios | ~63 s (ledger median of 2 record(s), imported 2026-09-23) |
| `scripts/check-lido-twg-pinned-target-current-mainnet.sh --composed-prerequisites` | consumes the registered target and exact compiler-emitted official CircuitBreaker/control-locator gateway runtimes, then replays finite and sentinel composed public pauses plus matching family pause/query chains under literal BPO2. Two further live executions prove the controls bite: STOP-only target code makes the composed query return no canonical word and reverts the outer receipt, while an admin-address target re-enters the production heartbeat setter, returns canonical true, and leaves a retained write in the protected parent cell. The shell checker independently pins the four-row/five-channel/two-mutant contract, artifact identities, EIP-7825 gas bound, the independently derived EIP-7934 singleton-block RLP bound, applicability-ledger digest and shared runtime-lock digest; the generator reconstructs the CircuitBreaker tagged layout and the gateway's nested-keccak membership plus tagged pause slot, exact event order/data and captured outputs before byte-comparing the generated result | 4 scenario rows / 8 state-test transitions; 2 duration arms; 2 execution mutants; 1,000,000 ≤ 16,777,216 transaction gas; 8,435 ≤ 8,388,608 RLP bytes; exact storage/events/outputs; zero semantic mismatches | ~15 s after the evaluator cache is warm |
| `scripts/check-lido-circuit-breaker-registry.sh --semantic-only` | compiled Registry positives, mutants and exact axiom rows; each fixture is elaborated once, with the shared from-scratch walker and the `#full_axioms` rows it owns appended to the compiled source | 3 concrete exact-code controls; 7 storage mutants; 4 axiom rows; 2 elaborations (was 6) | ~25 s (measured 2026-09-24; was 74 s) |
| `scripts/check-execution-occurrence.sh --semantic-only --composed-prerequisites` | compiled occurrence/direct-code positives, then the three production mutants (initial-prefix field type, child-before-continuation order, source-cursor indexing) in one elaboration, after consuming settlement evidence | 22 controls; 3 Lean mutants | ~32 s (was 389-423 s with 22 per-mutant and 7 deletion elaborations) |
| `scripts/check-execution-occurrence.sh --self-test` | shows the fixtures discriminate: the 17 occurrence and 2 direct-code mutants that a kept positive, a Blanc theorem or a fixture-local fact already contradicts, in one elaboration each, and the 7 live direct-code deletions in one elaboration; each diagnostic must fall inside its own lines. Inputs are the harness and fixtures only | 19 mutants; 7 deletions | ~55 s |
| `scripts/check-cycle-write-free.sh --semantic-only` | concrete evaluator, then the two production mutants (entry-body and branch-index substitution, which no pinned positive contradicts) in one elaboration | 23 evaluator controls; 2 Lean mutants | ~4 s (was ~50 s with 20 per-mutant elaborations) |
| `scripts/check-transient-settlement.sh --semantic-only` | concrete evaluator, then the two production mutants (TSTORE operand order; a nonce-ahead transaction rejected) in one elaboration | 25 evaluator controls; 2 Lean mutants | ~4 s (was ~8 s) |
| `scripts/check-proxy-pair-upgrade.sh --semantic-only --composed-prerequisites` | axiom and executable witness evidence plus the six-control untrusted stack producer and compiler-bound generated-data freshness check, after consuming total layering | 13 axiom pins; 12 evaluator rows; 47 generated stack rows across 74 actual compiler bytes | ~9 s (ledger median of 11 record(s), imported 2026-09-23) |
| `scripts/check-prorata-weth-vault-artifact.sh` | after the caller establishes the family-owned G2 artifact through Creme's single-owner build wrapper, statically and fail-closed binds its complete selector/name/type/arity/body routing surface, nonpayable/static-ABI/default behavior, configured WETH and virtual-offset constants, flat storage-key scheme, append-only auxiliary layout, approved full-width `maxMint` cap, event and code-size theorem pins, contiguous generated literal with repeated chunks canonicalized, exact runtime SHA-256, EIP-170 bound, and the exact public bounded kernel compile equality. The unchanged Kernel-checked marker terminates the literal join; private proof helpers precede that join. `python3 scripts/test-prorata-weth-vault-artifact.py` rejects deleted/corrupt/private/comment-or-string-spoofed witnesses, byte drift, join reordering and a missing join boundary; disabling only the witness pin restores the byte-correct witness mutants. The gate runs no build of its own | 25 selectors; 10 auxiliaries; 69 logical chunks (68 literals + 1 canonical alias); 17,481 runtime bytes; 1 kernel compile witness | sub-second after its owner-managed build prerequisite |
| `scripts/check-prorata-weth-vault-boundary.sh` | after the caller establishes the composition-owned G3 root through Creme's single-owner build wrapper, statically and fail-closed pins the exact configured WETH account and code, retained CALL/STATICCALL program occurrences, success and rollback projections, source-derived calldata and caller/owner/receiver roles, canonical true-return checks, whole-vault exclusion of hidden external call sites, absence of WETH10 imports and alias premises, and root/layering admission. The default gate runs no build of its own. `--falsify` is optional and requires the semaphore's exclusive hard hold; it compiles seven isolated diagnostic-pinned mutants covering wrong target, code, rollback polarity, calldata, owner role, false return, and a hidden `approve` path | 3 exact child forms; 16 G3 headline axiom owners; whole-source call closure; 7 optional Lean mutants | sub-second (ledger median 0.5 s of 12 record(s), imported 2026-09-23) by default; optional mutation campaign unmeasured |
...
| `scripts/check-drip.sh` | regenerates the committed oracle vectors byte-for-byte and checks oracle semantics and the committed runtime/creation artifacts, then 39 real Lean arithmetic rows compared with independently computed Python integers, and every committed BPO2 fixture through pinned Jaune. Requires owned full/runner build, current evaluator imports and adjacent artifact-integrity/certification; never builds implicitly. Source/compiler/cache/import identity drift, missing prerequisites, malformed arithmetic output or incomplete native replay fail closed. Finite arithmetic comparison is not the deployed-loop theorem. Pinned Jaune derives receipt encodings, blooms and trie roots from immutable manifest annotations and binds them to decoded block headers before native replay of the same snapshot | 38 frozen obligations; 39 independent arithmetic rows; 91 fixtures / 137 transactions; 1,762-byte runtime + 2,001-byte creation; arithmetic and complete family replay validated | ~23 s (measured 2026-09-24; was 46 s with the harness suites); lazy per-gate host admission before Lean evaluation |
| `scripts/check-drip.sh --self-test` | shows the DRIP checkers' controls bite: the oracle and artifact checkers' in-memory corruption suites (each validates the committed vectors or artifacts first), and the mocked or synthetic suites for the fixture verifier, replay driver, evaluator transport, receipt protocol, arithmetic protocol, evaluator identity projection and the wrapper's lazy host admission. No Lean child and no host hold; harness and evaluator identity definitions are fingerprinted, while DRIP sources and vectors are declared as subject-only reads (evidence economy rule 3) | 2 corruption suites (12 corruptions); 7 mocked suites | ~23 s (measured 2026-09-24) |
| `scripts/check-drip-current-mainnet.sh --composed-prerequisites` | DRIP's isolated BPO2 consumer first consumes the shared current-mainnet verdict, then runs the 60-cell and 88-cell cost-measurement suites and `gen-drip-fixtures.py --check-runtime` under the verified target interpreter and byte-compares its complete generated JSON population. The committed population was established by the generator's explicit `--write` mode after runtime validation and reproduces under this read-only check. CI replay is owned by `check-drip.sh` | 38 obligation-owned runtime scenarios, including exact block/header/signature/transaction-root and observer/rollback assertions in the reproduced population; 60-cell + 88-cell cost suites | generated population reproduced |
...
| `python3 -B scripts/test-elab-migration-comparison.py` | creates disposable Git/Lake/host-identity fixtures and exercises the read-only comparator's positive result plus record, origin, runtime, environment, threshold, schema, and write-safety falsifiers. The comparator reads the shared timing store through the timing gate's own selector module — one evidence reader and one environment-identity recipe, reproduced on staged immutable blobs — and adds only comparison policy (every reader refusal refuses, exact record selection, exact-corpus coverage, a Jaune-only transition); the selector's bytes stay untouched because they are a fingerprinted measurement input. The production comparison is deliberately outside this row: it reads two explicit clean shared-baseline records after normal genesis and never adopts either record | 23 synthetic CLI controls | ~14 s (ledger median of 1 record(s), imported 2026-09-23) |
...
| `scripts/check.sh --no-build` | elaborates the committed proof-recipe suggestion controls after the authoritative build, then audits the audited top theorems, each against its own pinned expected axiom set; the common rows include the direct spawned-code-address and source-chronology theorems, the proxy-pair rows cover its compiled programs, concrete success/revert executions, correspondence and write authority with biting controls, canonical empty CREATE, nonempty setup chronology, the exact both-slot setup child and complete settled CREATE, and failed whole-CREATE rollback, and the Lido rows include the universal runtime compile equation, source inventories, cycle canary/mutant, Registry mutation bridges, arbitrary-finite enumeration, coherent views, local raw/error/committed observability boundaries, and the complete direct-deployment proof family; the PRORATA rows cover its compile witness and deployed-byte body effects, the exact rounding-direction and residue arithmetic, zero-tolerance preview/actual consistency at both body and deployed-byte altitude, the `ContractSpec` instantiation and genesis-anchored accounting invariant, the eleven realized-accounting rungs with both carrier projections, the three attack-trace headlines, the generic spawned-frame statistics used by the vault rely-preservation rung, and the PRORATA WETH vault rows (its view and conversion compiled effects, pair and in-flight rungs, the 16 G3 boundary headlines, the P1 rounding and capacity arithmetic, the generic reverting-walk inversion `Prog.runCompiledTo_of_exec_revert`, the `max*` revert-cause, never-reverts and success-within headlines, and the terminal inventory `vault_terminals_return_or_revert`); the WETH10 rows pin the schedule-parametric generic surfaces, every current-mainnet specialization, every retained Prague corollary, the four-rule gas ceiling and the schedule-wide EIP-7825 discharge that consumes it in the direction `AdmissibleRedemptionTx.gas_cap` is stated, schedule selection, and the BPO2 timestamp; the Lido TWG rows cover selector-route construction, the completed pinned-target bundle, the S1 sentinel execution control, the A2 waypoint, and the shared branch-inversion and accepted-boolean adapters used by A3; BeaconDeposit covers its opening model, compiled P1–P6, direct deployment root/occurrence, exact dispatcher frame preservation, Prague-only prefix extension, and deployment-rooted count/root headline; and the contract-neutral operand-stack certificate rows pin the same-frame transport trio, the checker's `checkTable_certificate` soundness theorem and the module's live minimal use, so the finite validation stays KERNEL-checked, including the first real consumer's transported entry result. Every row's axiom set comes from `#full_axioms`, the one from-scratch walker, `Jaune.AxiomAudit.walk` in the pinned Jaune package's `AxiomAudit` module (Jaune's `scripts/AxiomAudit.lean`), which `AxiomCheck.lean` imports and `scripts/axiom_audit.py` checks before elaborating it: a fresh visited set per row over `Environment.find?`, no per-constant result cache, no read of the per-module axiom sets Lean precomputes at olean export, one task per row. Lean's `#print axioms` is never the verdict source (user decision 2026-09-24, until lean4#15226 is fixed): since v4.30.0 it reads that precomputed set, whose cycle sentinel can record an imported inductive as axiom-free depending on declaration hash order, which let `ReplayCarrier.nilOfEq` pass a "no axioms" pin it never had. Before Lean runs, the driver requires `AxiomCheck.lean`, comments aside, to hold only its imports and `#full_axioms NAME` rows (no `#print axioms`, `#eval`, new `elab` or report marker; no row inside a comment; no duplicate row), and the gate's inventory is that validated row list rather than a text grep. An empty audit is a REGRESSION | 1331 theorems | ~69 s (measured 2026-09-24; was ~22 s with `#print axioms`) |
...
| `scripts/generate-proof-recipes.py` | `check-proof-recipes.sh` and explicit `--write` regeneration | validates the strict TOML registry, referenced symbols, example anchors and review dates through the shared module-path policy, validates disjoint explicit trigger inventories in core `Tactics` and leaf `ProofRecipeTactic` with final false fallbacks, then deterministically generates `docs/PROOF_RECIPES.md` and the import-safe `Blanc.ProofRecipesGenerated` lookup table, consumed only by the unimported `Blanc.ProofRecipeTactic` leaf, through raw-string, alias-checked output bindings. Trigger soundness is checked in three layers and the terminal line reports what each one actually covered, so a run that could resolve nothing does not read like a run that resolved everything. `validate_trigger_dispatch` resolves, per arm, every `Blanc.` name the arm compares against, with `UNCHECKED_DISPATCH_ARMS` naming each arm it cannot see and why. `validate_jaune_dispatch` closes the other namespace, which was unchecked in two distinct ways: six arms dispatch on `Jaune.` names only, and two further arms sat *inside* the checked population with silently unchecked `Jaune.` disjuncts. `JAUNE_DISPATCH_SURFACE` therefore names all 32 such names, must equal exactly what the arms dispatch on in both directions, and is bound to `JAUNE_DISPATCH_PIN`, which must equal the `lake-manifest.json` and `lakefile.lean` rev — so a pin bump, the event that silently kills these arms, fails until the surface is re-verified, with no dependency materialized. When `.lake/packages/jaune` is materialized at that exact rev every name is additionally resolved against a census of its library (`Jaune.lean` and `Jaune/**` only, reading `mutual` scopes, French-quoted structure fields and constructors, none of which Blanc's own reader needs); the dependency being absent is reported, never silently skipped. `validate_harness_coverage` enumerates the registry against `scripts/ProofRecipeSuggestions.lean`, because liveness is not reachability and reachability is not statically decidable here: the dispatcher's head comes from a goal at tactic time, neither the registry nor either matcher contains a goal, and the property belongs to the corpus of statements authors write. The only evidence is an exhibited goal, so every registered recipe has a `-- EXPECT:` case or is named in `UNWITNESSED_RECIPES` with what a case would have to exhibit, every `-- EXPECT:` id is registered, every trigger string in the harness names a real arm — a misspelled `expect_no_recipe_trigger` otherwise passes over nothing, because the dispatcher's fail-closed wildcard returns false for an unknown trigger — and the triggers with a positive `expect_recipe_trigger` case, the only machine-checked reachability evidence in the repository, are pinned by name in `REACHABILITY_WITNESSED_TRIGGERS` so a lost witness cannot be masked by an unrelated new one. A `-- EXPECT:` comment and a bare `blanc_suggest` are narration, not evidence: the tactic only logs, so an example compiles whether or not any trigger matched. `--check` byte-compares both outputs and `--self-test` exercises 31 schema/trigger/matcher-ownership/trigger-dispatch/Jaune-dispatch/harness-enumeration/symbol/anchor/drift controls, the shared 15-control path campaign, 4 actual registry/aggregate out-and-back controls, and the optional aggregate wrong-case control |
| `scripts/ProofRecipeStackSafety.lean` | explicit current-import Lean diagnostics at the leaf matcher checkpoint | actual exact-head, old-trigger, unknown/wrong-head and `blanc_suggest` controls for the leaf extension; the pure recipe gate binds this input but does not elaborate it. Record actual diagnostics separately after the owned leaf build |
| `scripts/check-proof-recipes.py` | `check-proof-recipes.sh` and `check-proof-duplication.sh` | parses same-line declarations and the exact production wrapped-header form from original source spans while failing closed on unsupported declaration-looking syntax; all Git/import-derived and walked source dereferences use the shared exact-spelling, alias-rejecting, root-contained policy. In ordinary mode it runs the generator check, computes changed declarations from Git including untracked files, traverses local imports, reports only high-confidence normalized declaration copies and misplaced selector tables. Imported-copy matching tries the strict name/documentation-normalized key first, then a parsed-header-only fallback that erases `private`/`protected` and their immediate raw spaces/tabs while preserving header comments, attributes, options, declaration kind, semantic modifiers, body bytes and other whitespace. It validates narrow expiring exceptions fail-closed; in `--duplication` mode it reuses that same parser, normalization and substantive floor to inventory every K1 family over the whole production corpus, compares it with the digest-sealed shrink-only baseline, validates family-scoped expiring exceptions, and blocks on any rise. `--write-baseline` is its shrink-only evidence refresh, `--list` prints where each live family's sites are, and `--self-test` runs the three parser-header controls, anonymous-instance boundary, seven legacy detector controls and fourteen imported-copy visibility controls together with the generator/path controls and 18 duplication controls while `check-proof-duplication.sh --self-test` runs the duplication controls alone |
...
| `scripts/check-doc-counts.py` | `check-doc-counts.sh` | derives each registered published claim from the one committed artifact that owns it — the audited-theorem count, the claim-pin count, the WETH10 differential row count and the Lido CircuitBreaker differential row count — and fail-closed checks every registered quotation, the per-surface occurrence census behind it, the two published `check-claims.sh` transcripts and the published `check.sh` axiom-audit transcript against those gates' own verdict format strings, `PORTING.md`'s references into this tree, and this catalogue's account of the gate's own scale |
| `scripts/check-lido-circuit-breaker-assurance.py` | `check-lido-circuit-breaker-assurance.sh` | parses the assurance register's frozen seven-field row schema, pins per-pillar/total/gate-owned row counts in its own source, reads every axiom-expectation authority's pin table with `ast` without importing or executing it, resolves each cited declaration against the authority its row's **Gate** field names, compares axiom sets order-insensitively in both directions, checks gate existence and catalogue registration, and requires every pinned non-claim phrase; `--probe` additionally regenerates and elaborates a from-scratch `#full_axioms` probe through `scripts/axiom_audit.py` and needs the Lean toolchain |
| `scripts/check-proxy-pair-upgrade.py` | `check-proxy-pair-upgrade.sh` | pins the exact proxy-pair upgrade declarations and assurance theorems, generic/product placement, claim tokens, forbidden trust escapes, live axiom sets, exact executable success/identity/rollback/R2/post-upgrade rows, and the compiler-bound stack-data check; `--self-test` runs 45 isolated fail-closed controls, including exact comment/raw-string spoofs for all five root/stack surfaces, nested/multi-hash comment/raw-string cases, ordinary/interpolated/macro string spoofs, and fail-closed malformed-input controls |
| `scripts/check-extraction-ownership.py` | `check-extraction-ownership.sh` | strictly parses the lift manifest and the Jaune-relocation manifest, common declaration/import ownership, WETH10/Lido shadow absence, all 21 exact compatibility abbreviations, alias/export absence, and relocated-declaration ownership (declared by Jaune, redeclared nowhere in Blanc); under `--negative-controls` (the wrapper's `--self-test`) eleven mutation controls on a temporary copy of `Blanc/`, `Blanc.lean`, the pinned Jaune sources and both manifests, plus one parser control |
...
| `scripts/check-execution-occurrence.py` | `check-execution-occurrence.sh` | compiles 16 concrete occurrence controls plus six kernel-only actual-spawn direct-code controls, parser-pins 26 public proofs and seven exact direct-code propositions, runs the three production mutants in one diagnostic-pinned elaboration (`--self-test`: the other 17 occurrence mutants, the CREATE/CALLCODE premise-deletion mutants and the seven live direct-code deletions), pins the exact common direct-code header, sole owner, retired name, and 1/1/2/1 WETH/Lido consumer inventory, rejects canonical/retired shadows, aliases, exports, and unrelated-name proposition copies across WETH10/Lido fail-closed, retains the historical ownership/parser falsifiers, composes the raw-attribution chronology ownership/signature/kernel audit, then composes the two CREATE raw-retention falsifiers |
| `scripts/check-cycle-write-free.py` | `check-cycle-write-free.sh` | validates the exact owner/signature/exemption manifest, compiles the concrete cycle/arbitrary-outcome/frame-boundary fixture to an exact evaluator vector and rejects the two production mutants in one diagnostic-pinned elaboration; `--self-test` runs the in-memory falsifiers, the per-proof deletion controls and the other 18 mutants |
| `scripts/check-transient-settlement.py` | `check-transient-settlement.sh`; explicit `--write-manifest` regeneration | validates exact ownership, imports, signatures, donor moves, compatibility and frozen predecessor assurances; is the sole writer of the owner manifest's checker hashes; compiles the 25-control concrete fixture and rejects the two production mutants in one diagnostic-pinned elaboration; `--self-test` runs the parser falsifiers, the live deletions of every required public owner and positive fixture theorem, and the other 20 mutants |
...
| `scripts/check-trust-surface.py` | `check-trust-surface.sh` | traverses `Blanc.lean`'s transitive local import closure, masks comments and literal text, and compares every normalized forbidden-token occurrence in code against the exact fail-closed allowlist |
| `scripts/test-check-trust-surface.py` | run directly after editing the checker | 28 temp-tree controls: a token in code still fails, comment/doc-comment/nested-comment/string/raw-string occurrences pass, comment markers inside strings and quotes inside comments or character literals hide nothing, interpolation holes stay code, unterminated comments and strings fail closed, and a comment-only allowlist row is stale |
| `scripts/test-keccak-rate-boundary.py` | `check-keccak-rate-boundary.sh` | requires the canonical Keccak module to be the only structurally detected sponge, checks its external vectors and consumer representations, and proves the ownership control rejects two differently written extra implementations while retaining a pinned detection limit |
...
| `scripts/gen-lido-twg-differential.py` | `check-lido-twg-differential.sh` | executes both complete CREATE inputs and the exact 71-case causal runtime matrix in fresh pinned Prague worlds, independently projects Solidity and tagged Blanc state, admits only three exact active registered differences while retaining D04/D05 as repaired regression rows, requires 2/2 byte and 51/51 gas wins, records per-boundary resource evidence, and runs live channel/identity/semantic falsifiers |
...
| `scripts/test-lido-twg-differential-falsifiers.py` | `check-lido-twg-differential.sh` | mutates every compared channel plus artifact, oracle, row/deviation, sentinel, resource, strict-gas-dominance, four production-shape controls and four mutant-rejection controls, document-template, digest and duplicate-key evidence to prove the independent schema rejects coherent-looking corruption |
| `scripts/eval-lido-twg-artifacts.lean` | `check-lido-twg-differential.sh` | emits the production-owned TWG creation template, both full CREATE inputs and patched runtimes, locator offsets/controls, selector list, constructor write/call inventories, projection metadata and EIP size boundaries without owning a duplicate artifact literal; independently compiles or evaluates the packed-limit, nested-keccak, per-role enumeration and one-read route shapes and requires a paired losing mutant for each |
...
| `scripts/test-lido-circuit-breaker-deployment-falsifiers.py` | `check-lido-circuit-breaker-deployment.sh` (`--lean-controls-only`) and its `--self-test` (`--source-cases-only`) | the main gate compiles the arbitrary-premise root-extraction control and the non-deployment synthetic-world boundary control; the self-test runs 64 temporary source mutations, including public-theorem deletion/addition and axiom-expectation controls, re-pinning the changed declaration for all secondary controls |
| `scripts/axiom_audit.py` + the pinned Jaune's `AxiomAudit` module | `check.sh`, `check-proxy-pair-upgrade.sh`, the Lido registry, enumeration, access and history gates, and `check-lido-circuit-breaker-assurance.sh --probe` | the one axiom walker and its one driver. Blanc keeps no copy of the walker: Jaune's `scripts/AxiomAudit.lean` (module `AxiomAudit`, a root of Jaune's `Assurance` library, imports only `Lean`, namespace `Jaune.AxiomAudit`) defines `#full_axioms`, and Blanc's library `needs` that library, so Blanc's build also builds Jaune's `ExecutionAxioms` destination audit. `#full_axioms`: per audited name, a fresh visited set over `Environment.find?` (types, definition/theorem/opaque values, inductive constructors), no per-constant result cache, no read of Lean's precomputed per-module axiom sets, one task per name, one `FULL-AXIOMS` line out. `axiom_audit.py` first checks the pinned walker source (`.lake/packages/jaune/scripts/AxiomAudit.lean`: exactly `import Lean`, no other import, `#full_axioms` defined in `Jaune.AxiomAudit`, no collector in its code); it requires a committed audit source to `import AxiomAudit` itself, builds a probe from plain module and declaration names plus that import, or prefixes the import to a Registry fixture and appends the rows. It then elaborates with `lake env lean --stdin` and requires exactly one report per requested name and none for any other. A committed audit source must hold, comments aside, only imports and `#full_axioms NAME` rows, each once, with no row inside a comment and no report marker; `rows FILE` prints that validated inventory. A fixture may not mention the command, the report marker, the walker's namespace or Lean's collector, and the walker's code may not name the collector. These rules tie each verdict to the walker; they do not sandbox arbitrary Lean in a reviewed fixture file |
| `scripts/lean_mutant_batch.py` | the execution-occurrence, cycle-write-free and transient-settlement checkers | inserts every mutant of a run into one copy of its fixture, maps each to its line span and attributes every Lean error to the span it falls in: a mutant passes only when an error inside its own lines carries its pinned diagnostic, and an error outside every span fails the batch |
...
| `scripts/current-mainnet-target.json` | `check-current-mainnet.sh`, `check-weth10-current-mainnet.sh`, `check-beacon-deposit-current-mainnet.sh`, `check-prorata-current-mainnet.sh`, `check-lido-ossifiable-proxy-current-mainnet.sh`, `check-drip-current-mainnet.sh`, and `check-lido-twg-pinned-target-current-mainnet.sh` | the strict shared profile for literal BPO2 execution, Osaka compiler lineage, exact target/overlay provenance, and canonical native macOS arm64/Linux x86_64 uv identities; it does not describe a cross-OS alias or repoint the legacy Prague lane |
| `scripts/current-mainnet-runtime-lock.json` | all seven current-mainnet gates | generated exact native runtime evidence for both supported platform rows: executable digest, complete selected site-packages fingerprint, common entrypoint body, target commit, and exclusion policy |
| `scripts/current-mainnet-runtime-recipe.json` | `setup-current-mainnet.py`, runtime-lock generator and verifier | exact source/overlay, Python and installer artifacts, frozen dependency export and build-backend wheel identities; its digest is bound by the runtime lock |
| `scripts/setup-current-mainnet.py`, `scripts/reference/current-mainnet/target-overlay.patch`, `scripts/reference/current-mainnet/target-commit.txt` | current-mainnet environment construction | reconstruct a new exact target without modifying an existing environment or rewriting the runtime lock; recovery instructions are in `docs/CURRENT_MAINNET_SETUP.md` |
| `scripts/current_mainnet_runtime_controls.py` | runtime-lock self-check | portable metadata/RECORD controls and explicit disposable dependency-code/path-escape controls with sole-restoration verification |
...
| `scripts/current_mainnet.py` | `check-current-mainnet.sh`, `check-weth10-current-mainnet.sh`, `check-beacon-deposit-current-mainnet.sh`, `check-prorata-current-mainnet.sh`, `check-lido-ossifiable-proxy-current-mainnet.sh`, `check-drip-current-mainnet.sh`, and `check-lido-twg-pinned-target-current-mainnet.sh` | contract-neutral public API (`load_profile`, `resolve_root`, `verify_target`, `target_paths`, `run_t8n`) with no fork parameter; it selects the native profile row, validates the full runtime lock and provenance, sanitizes the child environment, and owns target execution |
...
| `scripts/check-lido-twg-pinned-target-current-mainnet.sh` | `check-lido-twg-pinned-target-current-mainnet.sh`; optional composition of the shared boundary | independently pins the scenario/channel/mutant/profile/artifact/ledger/runtime-lock contract, evaluates only the compiler-owned runtime pair, and launches the generator under the selected isolated target interpreter |
| `scripts/eval-lido-twg-pinned-target-current-mainnet.lean` | `check-lido-twg-pinned-target-current-mainnet.sh` | emits the exact official CircuitBreaker and locator-`0x800` gateway runtime bytes from their production owners; it owns no runtime literal, result, or theorem |
| `scripts/gen-lido-twg-pinned-target-current-mainnet.py` | `check-lido-twg-pinned-target-current-mainnet.sh`; explicit `--write` regeneration | imports exactly the five fork-override-free shared functions, executes the four finite/sentinel family/composed rows as six singleton transitions plus the query-code and reentrant-noninterference mutants as two more transitions, rejects any storage/event/output/control mismatch, binds the applicability ledger and two-platform runtime lock, and byte-compares the committed result by default |
| `scripts/fixtures/lido-twg-current-mainnet/results.json` | `check-lido-twg-pinned-target-current-mainnet.sh` | generated finite BPO2 replay evidence and exact observations for both live execution mutants against the compiler-owned artifact pair; never a theorem premise and never a live-chain role/state attestation |
...
| `scripts/gen-drip-fixtures.py` | `check-drip-current-mainnet.sh`; explicit `--write` regeneration | sole writer of `scripts/fixtures/drip/*.json`. It consumes the shared current-mainnet API, DRIP oracle, strict literal reader, blockchain-test helper, and observer helper; runtime check mode executes the pinned BPO2 target and byte-compares the entire population without writing. The separate `--measure-costs` (60 cells) and `--measure-supplemental-costs` (88 warmth/guard cells) modes load the registered cost helper and emit measurements only; neither writes fixtures |
| `scripts/drip_cost_measurements.py`, `scripts/test-drip-cost-measurements.py` | `gen-drip-fixtures.py --measure-costs`; synthetic controls through `check-drip-current-mainnet.sh` | supplemental read-only 60-cell baseline/candidate and access-list cost measurement; authenticates signed type-0/type-1 envelopes and reconstructs single-receipt roots. Reports net receipt charges with explicit unobserved channels. No fixture writer; does not replace the frozen 91-scenario/137-transaction population |
| `scripts/test-drip-supplemental-costs.py` | `check-drip-current-mainnet.sh` | pure controls for the separate 17-scenario/88-transaction warmth/guard mode in `drip_cost_measurements.py`; exact scenario/access-list/artifact binding and authenticated envelope/root corruption rejection. Preserves the legacy 60-cell and frozen 91/137 populations; synthetic tests establish no EELS execution or reachable-history claim |
| `scripts/drip_fixture_blocks.py` | `gen-drip-fixtures.py` through `check-drip-current-mainnet.sh` | DRIP-only canonical BPO2 block/header/RLP/signature/transaction-trie helper; it owns no output and is loaded only under the verified target interpreter |
| `scripts/drip_fixture_observers.py` | `gen-drip-fixtures.py` through `check-drip-current-mainnet.sh` | DRIP-only callback/returndata observer bytecode helper; it owns no output and supplies exact result/log chronology instrumentation for fixture generation |
| `scripts/check-drip-current-mainnet.sh` | `check-drip-current-mainnet.sh` | owns the sanitized target launch and direct shared-boundary composition; it invokes the two cost-measurement suites and only `gen-drip-fixtures.py --check-runtime` of the generator's modes, never `--write`, and records no CI replay or arithmetic-evaluator claim |
...
| `scripts/check-drip-fixtures.py` | `check-drip.sh` through `check-drip-replay.py` | fail-closed structural schema, obligation, artifact, target, observer and transaction/receipt-reference checks; no execution claim |
| `scripts/check-drip-replay.py` | `check-drip.sh` | no-argument deterministic replay of the complete structurally accepted population with `.lake/packages/jaune/.lake/build/bin/jaune FIXTURE --network BPO2`; records pin/binary hash and each terminal verdict; no implicit build, override or writing mode |
| `scripts/test-check-drip-fixtures.py`, `scripts/test-check-drip-replay.py` | `check-drip.sh --self-test` | synthetic corruption/restoration controls; replay subprocesses are mocked and provide no real fixture evidence |
| `scripts/eval-drip-arithmetic.lean` | `check-drip-arithmetic.py` through `check-drip.sh` | output-only fixed 39-row evaluation of public Jaune/Blanc arithmetic definitions; large scalar exponents remain logarithmic and expanded trees use only 0..16. No golden input, writer or theorem claim |
| `scripts/eval-drip-receipts.lean` | `check-drip-receipts.py` through `check-drip-replay.py` and `check-drip.sh` | fixed 91-fixture/137-receipt protocol; decodes blocks and derives legacy receipt encodings, trie keys/roots and blooms through pinned Jaune, comparing roots/blooms/gas with headers. No writer, golden input, or theorem claim; native replay remains separately required |
| `scripts/test-drip-gate-coordination.py` | `check-drip.sh --self-test` | real shell routing against mock Python/semaphore commands: lazy admission, refusal before both evaluators, static failure without a hold, evaluator failure release, explicit inheritance/off modes, and the default/`--self-test` split; no real host hold or Lean process |
| `scripts/check-drip-arithmetic.py` | `check-drip.sh`; fixed standalone arithmetic validation | independently computes integer expectations from frozen inputs, compares every versioned Lean row and input echo fail-closed, and checks source/compiler/environment/import identities around the fixed evaluator. Requires the owned build/import and artifact-integrity/certification prerequisites; no implicit build |
| `scripts/drip_evaluator.py`, `scripts/drip-evaluator-identity.py` | evaluator drivers; `drip-evidence` material-output identity | fixed compiler dispatch and immutable source/compiler/environment/import snapshot; identity projection performs no Lean evaluation and binds these inputs for cached verdict reuse. Cache presence and depHash metadata do not establish artifact byte integrity |
| `scripts/test-drip-evaluator.py`, `scripts/test-drip-receipts.py`, `scripts/test-drip-arithmetic.py`, `scripts/test-drip-evaluator-identity.py` | `check-drip.sh --self-test` | 50 mocked transport/protocol/identity controls; temporary compiler/cache/import prerequisites and no real evaluator subprocess. Receipt tests do not authenticate annotations |
| `scripts/drip_oracle.py` | `check-drip.sh`; future DRIP fixture generators | exact unbounded-integer state model with explicit word guards, message-entry credit, checks-effects-interactions, nested callback, and whole-frame rollback semantics; it writes no artifact |
| `scripts/gen-drip-oracle-vectors.py` | `check-drip.sh`; explicit `--write` regeneration | sole writer of `scripts/drip-oracle-vectors.json`; normal mode byte-compares the complete deterministic output and writes nothing |
| `scripts/gen-drip-stack-certificate.py` | `check-drip-stack-certificate.sh`; explicit `--write` data regeneration and `--write-proofs` fixed proof regeneration | sole writer of `Blanc/DripStackSafetyData.lean`, `Blanc/DripStackSafetyRegion576.lean`, `Blanc/DripStackSafetyRegion1022.lean`, `Blanc/DripStackSafetyRegion1459.lean` and `Blanc/DripStackSafetyCertificate.lean`; deterministically derives exact whole-stack patterns from `Blanc/DripCode.lean`, names small subtrees of the unchanged balanced tree, emits dependency-ordered proof text over the complete table, composes the four checked regions into the complete 735-row same-frame facade, and records runtime identity; ordinary mode compares only. Generated data and proof text supply no trusted Lean premise; separately required Lean builds check every theorem |
| `scripts/test-drip-stack-certificate.py` | `check-drip-stack-certificate.sh` | checks both conditional arms, the actual backedge and CALL shape, exact literals/full-stack matching, independent outgoing bounds, high DUP/SWAP cases, byte/branch/row corruptions, the exact 559 accepted-row union and 176-row remainder, exact Region576 proof reproduction, independently parsed Region1022/Region1459 topology/bounds/count, cross-pack successor checking through the complete table, the complete same-frame facade, fixed-output writer behavior, and compare-default/write-explicit restoration on disposable inputs |
...
| `scripts/check-runtime-bytes.py` | `check-fmint.sh`, `check-weth.sh`, `check-prorata.sh`, `check-weth10-redemption.sh` | parses the committed Lean literal and compares it byte-for-byte against every fixture's pre-state code for that contract; masks each file text once, so a chunk-joined literal (the 70-chunk vault literal) parses in under a second |
...
| `scripts/gate-lock.sh` | `check-elab.sh` | exclusive gate locking; sourced, never run. Reclaims a lock only when `kill -0` answers "No such process"; a denied probe is unknown liveness and is REFUSED with the lock left in place, and a `mkdir` failure other than an existing lock is REFUSED with mkdir's own error |
| `scripts/test-gate-lock.py` | run directly after editing `gate-lock.sh` | 11 temp-tree controls with stubbed probes: live owner refused, confirmed-dead owner reclaimed, denied or inconclusive probes refused with the lock unchanged, and missing-parent, unwritable-parent and non-directory paths refused as creation failures rather than contention |
...
not queue and does not fall back. A holder whose liveness a sandbox hides is
REFUSED as unknown rather than reclaimed, and a lock path that cannot be created
(missing parent, permissions, read-only) is REFUSED with the real error, not
...
Both halves are held to that contract by controls, and `check-gates.sh
--self-test` runs them. Four state the modes of `gate_semaphore.py`; since
2026-09-08 five more source `gate-semaphore.sh` itself under bash against a
stand-in entry point, one per mode plus the shape of the request it makes. A
mode control makes that stand-in answer `refuse`, so `off` and `inherited` are
shown never to ask rather than merely to ignore the answer. Deleting either
`case` arm, turning the absent entry point into a refusal, or letting an
inherited hold be released each fails a negative control.
...
## Evidence economy (standing)

A standing user directive (2026-09-23) for all of Blanc. Jaune follows it by the
same reasoning.

1. **Finite evidence covers only what no theorem states.** Fixtures,
   differentials, oracles and matrices cover three things: fidelity of a Lean
   statement to its written spec, agreement with an external referent (a
   reference implementation or deployed code), and measurements. A property a
   named theorem proves is not re-checked with finite cases. A review finding
   against a proved property is answered by citing the theorem.
2. **EVM conformance is Jaune's concern.** Jaune-against-EELS agreement is
   established by Jaune's own conformance gates and is not re-run per Blanc
   contract.
3. **A harness self-test shows a control bites, when the control is written
   or changed.** Its cache inputs are the harness's own files. A restore is
   shown by byte identity to a green baseline, not by a rerun. Each mutant runs
   the narrowest mode that reaches its diagnostic.
4. **Every gate row carries a measured cost.** An expensive or large row
   states the unique protection it gives.
5. **Trim wherever found.** This directive authorizes removing finite evidence
   under rules 1-3, and such a removal is not "weakening" under the
   registered-provenance rule, on two conditions: a removal ledger maps each
   removed check to the theorem, Jaune test or retained check that covers it,
   and a model-diverse reviewer accepts that ledger. A check with no covering
   item stays.

First application: the PRORATA WETH vault differential (ledger in Plans
`reports/evidence-economy-20260923/vault-differential-ledger.md`).

Batch 1 (ledger in Plans `reports/evidence-economy-20260923/trim-b1-ledger.md`)
split a `--self-test` row out of extraction ownership, execution occurrence,
cycle-write-free, transient settlement and the Lido deployment root. A mutant
or control stays in its main row when it is the only check that would notice a
production change — no kept positive or theorem statement contradicts it — and
moves to the self-test when a named positive, theorem or fixture-local fact
already implies its failure. Nothing was deleted. Mutants that stay run in one
shared elaboration through `scripts/lean_mutant_batch.py`, which requires each
mutant's pinned diagnostic inside that mutant's own lines. The deployment gate
no longer reruns the repository axiom audit that the `axiom-audit` row owns.

Batch 2 (ledger in Plans `reports/evidence-economy-20260923/trim-b2-ledger.md`)
split the DRIP harness suites into `check-drip.sh --self-test`, folded the Lido
Registry axiom probes into the two fixture compiles, and replaced the
BeaconDeposit naive-root series with `naive_agrees`, a theorem the evaluator
elaborates. The execution-occurrence and Lido Registry static rows and the
proof-recipe row keep every control; their cost was a character-at-a-time
comment stripper run over the whole corpus once per control, which now scans
by token and is memoized, with identical output on every Lean file of Blanc,
its packages and Jaune. The doc-counts row now fingerprints the producers and
the Lean population its counts are derived from.
...
1. **Never weaken a gate to make it green.** A baseline, budget, manifest,
   ...
   movement still stops for user reconciliation. Removing finite evidence
   under the evidence economy section above, with an accepted ledger, is not
   weakening.
...
5. **Generated artifacts come from their generators**, never from hand editing:
   ...
   population and manifest from `scripts/gen-prorata-fixtures.py --write` and the DRIP
   population from `scripts/gen-drip-fixtures.py --write`), PRORATA's canonical
   arithmetic vector from `scripts/gen-prorata-oracle-vectors.py`, borrower bytes from their
      committed artifact JSONs, `Blanc/FmintCode.lean`, `Blanc/WethCode.lean`,
      `Blanc/DripCode.lean`, and `Blanc/DripCreationCode.lean` from their named
      `scripts/gen-*-code.lean` writers; `Blanc/ProrataWethVaultCode.lean` from
      `scripts/gen-prorata-weth-vault-code.lean`, using the maintained bounded proof
      in `scripts/prorata-weth-vault-code-proof.lean.in`; the generator computes
      the frozen byte chunks from the program, and the template supplies only
      their private source-to-byte certificates; and the WETH10 differential manifest from
   ...
   The composed Lido CircuitBreaker × TWG BPO2 result comes only from
   `scripts/check-lido-twg-pinned-target-current-mainnet.sh --write`.
   ...
   `Blanc/DripStackSafetyData.lean` comes only from
   `python3 scripts/gen-drip-stack-certificate.py --write`; the fixed
   `Blanc/DripStackSafetyRegion576.lean`,
   `Blanc/DripStackSafetyRegion1022.lean`,
   `Blanc/DripStackSafetyRegion1459.lean`, and
   `Blanc/DripStackSafetyCertificate.lean` proof-text outputs come only from
   the same registered adapter's `--write-proofs` mode. Its normal mode
   compares all five generated surfaces and never changes them. Generated
   proof text is untrusted source and still requires Lean/kernel checking.
6. **CI runs a registered subset of this file**, not a different thing. The
   ...
   BeaconDeposit differential/current-mainnet consumers, PRORATA regeneration consumer, and
   DRIP current-mainnet and Lido CircuitBreaker × TWG replay consumers are local
   full-set gates too; CI retains the target-independent committed BPO2 replay through
   `check-prorata.sh` and DRIP's target-independent arithmetic/artifact evidence and
   committed BPO2 replay through `check-drip.sh`; the runner must already be built by
   the compilation owner. Extending
