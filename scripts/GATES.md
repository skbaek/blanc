...
| `scripts/check-prorata-weth-vault-differential.sh` | executes the committed 17481-byte vault runtime and the 4347-byte constructor-patched OpenZeppelin reference (deployed on Jaune from the locked creation input against Blanc's WETH and identity-checked) through `jaune t8n` at BPO2, projects each side's storage through its own layout, and compares normalized application state, status, exact logs and captured return data against the independent oracle across the declared finite matrix. Both compiled sides execute through Jaune; pinned EELS separately replays the selector matrix. Four seeded arithmetic capacity cases cover upper-supply, one-share-room, converter representability, and high-word donation; they are not payable histories. Both engines require exact Blanc-empty/reference-`Panic(0x11)` converter-overflow returndata. Fixed-recorder causal histories execute actual multi-transaction WETH funding, approval and delegated-exit sequences through a contract caller, feeding each t8n post-state forward. The executed-ID ledger credits only check groups that actually completed, and undeclared SF rows remain uncredited. The manifest declares the whole required finite surface, so before any execution the gate requires each declared case to hold exactly one disposition: implemented here, superseded by named successors that are themselves implemented, or recorded unimplemented with its reason. A declared case can therefore never be silently uncredited, and neither non-executing record is coverage. The foreign-child rows are canonical-return probes outside exact-pair admission, with Jaune rollback evidence and EELS outcome replay only. Exact-pair rollback cases compare failed-child storage and logs with spent-allowance restoration and distinguish rejected transactions from accepted reverting executions; explicit SF5 D8 outbound burn→WETH→Withdraw order cases complement the inbound order; and pre-transfer quote cases assert each flow's output against the pre-state quote alongside a disagreeing post-transfer witness. At A=U, zero-amount flows and nonzero deposit, mint, withdraw and redeem execute through the 257-bit route on Blanc while the reference reverts under deviation 6 (the nonzero deposit and mint wrap the vault's WETH row, SF Amendment A2 half 1); a withdraw or redeem payout that wraps the receiver's WETH row executes on both sides; supply-ceiling flows revert minting on Blanc with the oracle supply-cap class while the reference succeeds unbounded under deviation 5 and exits execute on both; and exact-child provenance binds the child target, code, caller and calldata with a byte-identical decoy required untouched. The finite collision premise evaluates six executed allowance pairs for binding, pairwise key distinctness and synthetic-collision rejection, and donation classification asserts a gift mints no shares. It records both runtime sizes and the existing six gas rows in `scripts/prorata-weth-vault-reference-measurements.json` and fails on drift | 47 Jaune check groups; 129 declared required cases (125 implemented, 4 superseded, 0 recorded unimplemented); 25 selectors × 2 sides; 1 reference deployment; 6 gas rows | unmeasured after expansion |
...
| `scripts/check-prorata-weth-vault-differential.sh --self-test` | composes seven campaigns in disposable copies with manifest regeneration, and fails if any one of them does. The legacy campaign perturbs the oracle's offset and all four rounding directions, independently removes the Jaune arithmetic-capacity, deposit event-order, infinite-allowance, outbound-role and EELS arithmetic-capacity implementations while retaining their declarations, corrupts the vault self-receiver WETH balance and the outbound finite delegated post-spend allowance, checks synthetic receipt/rollback false positives and executed return-capture controls, and perturbs the committed measurements and the locked reference runtime identity. The causal-return campaign mutates the fixed recorder's observation and its real funding/approval/code setup; the child-return campaign mutates the foreign-child executed-ID ledger, long-return policy, rollback projection and operational return word; the disposition campaign drops a named successor, gives one case two dispositions, and excuses a case the declaration never names; the rollback-order campaign omits the exact-rollback, outbound-order, mint-order and quote-timing implementations, corrupts the exact rollback projection, confuses a rejected transaction with an accepted one, expects the post-transfer quote, and corrupts the outbound Withdraw words; the capacity-provenance campaign omits the A=U zero-flow, ceiling-flow and provenance implementations, corrupts the zero-mint expectation, the reported maximum, the reference mint projection, the decoy comparison and the expected allowance spend; the collision-donation campaign omits the pair-set and donation implementations, drops a recorded pair, confuses the allowance key order, weakens the evaluator against a synthetic colliding set, and mints on donation. Every mutant must reach its own named diagnostic and removing only that mutation must restore green. The valid-call-as-revert record preserves emitted diagnostics but does not yet require the named status diagnostic | 24 legacy controls (5 semantic oracle perturbations; 5 executed-ID omissions; 2 state/allowance falsifiers; 1 diagnostic-presence revert probe; 4 receipt/rollback falsifiers; 5 return-capture controls; 2 reference-half corruptions); 7 causal-recorder mutations; 4 foreign-child mutations; 3 disposition mutations; 8 rollback-order controls (4 executed-ID omissions; 4 semantic mutants); 8 capacity-provenance controls (3 executed-ID omissions; 5 semantic mutants); 6 collision-donation controls (2 executed-ID omissions; 4 semantic mutants) | unmeasured after expansion |
...
| `scripts/check-execution-occurrence.sh --semantic-only --composed-prerequisites` | compiled occurrence/direct-code positives and mutants after consuming settlement evidence | 22 controls; 22 Lean mutants; live positive deletions | unmeasured after split |
