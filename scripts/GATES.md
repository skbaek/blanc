...
| the harness, fixture or control table of the extraction-ownership, execution-occurrence, cycle-write-free, transient-settlement or Lido deployment gate (including `scripts/lean_mutant_batch.py`) | that gate's `--self-test` | the **full set**, in the order below |
...
| `scripts/check-extraction-ownership.sh --self-test` | shows every audited channel bites: eight mutations of a temporary copy of `Blanc/` plus the lift manifest (donor alias/export, flow-block removal, attribution-block drift, Lido shadow/alias, missing common declaration, missing direct import) and one trailing-`?` parser probe must each fail with their own tag, after the copy audits clean. Inputs are the harness only (evidence economy rule 3) | 9 controls | ~28 s |
...
| `scripts/check-execution-occurrence.sh --static-only` | corpus-wide occurrence owner, header, consumer, shadow, alias, export and proposition-copy controls | 10 moved-owner rows + exact headers and parser controls | ~16 s (ledger median of 12 record(s), imported 2026-09-23) |
...
| `scripts/check-lido-circuit-breaker-registry.sh --static-only` | Registry sole-owner/namespace/header pins, forbidden-trust scan and in-memory corpus falsifiers | 24 Registry declarations; 34 SHA-256 headers; 11 falsifiers | ~20 s (ledger median of 12 record(s), imported 2026-09-23) |
...
| `scripts/check-lido-circuit-breaker-registry.sh --semantic-only` | compiled Registry positives, mutants and exact axiom rows | 3 concrete exact-code controls; 7 storage mutants; 4 axiom rows | ~78 s (ledger median of 11 record(s), imported 2026-09-23) |
...
| `scripts/check-drip.sh` | CI-safe artifact/oracle controls and mocked evaluator protocols, then 39 real Lean arithmetic rows compared with independently computed Python integers, and every committed BPO2 fixture through pinned Jaune. Requires owned full/runner build, current evaluator imports and adjacent artifact-integrity/certification; never builds implicitly. Source/compiler/cache/import identity drift, missing prerequisites, malformed arithmetic output or incomplete native replay fail closed. Finite arithmetic comparison is not the deployed-loop theorem. Pinned Jaune derives receipt encodings, blooms and trie roots from immutable manifest annotations and binds them to decoded block headers before native replay of the same snapshot | 38 frozen obligations; 50 mocked evaluator/identity tests; 39 independent arithmetic rows; 91 fixtures / 137 transactions; 1,762-byte runtime + 2,001-byte creation; arithmetic and complete family replay validated | arithmetic and complete replay validated; lazy per-gate host admission before Lean evaluation |
...
| `scripts/test-check-drip-fixtures.py`, `scripts/test-check-drip-replay.py` | `check-drip.sh` | synthetic corruption/restoration controls; replay subprocesses are mocked and provide no real fixture evidence |
...
| `scripts/test-drip-gate-coordination.py` | `check-drip.sh` | real shell routing against mock Python/semaphore commands: lazy admission, refusal before both evaluators, static failure without a hold, evaluator failure release, and explicit inheritance/off modes; no real host hold or Lean process |
...
| `scripts/test-drip-evaluator.py`, `scripts/test-drip-receipts.py`, `scripts/test-drip-arithmetic.py`, `scripts/test-drip-evaluator-identity.py` | `check-drip.sh` | 50 mocked transport/protocol/identity controls; temporary compiler/cache/import prerequisites and no real evaluator subprocess. Receipt tests do not authenticate annotations |
