...
| the pinned Prague EELS reference environment, its semantic closure, constraints file, or the shared closure deriver | `scripts/check-eels-prague-closure.sh` | the **full set**, in the order below |
...
| the reusable current-mainnet profile/helper/runtime lock, its BPO2 canary, isolated target, pinned semantic closure, or a current-mainnet consumer | `scripts/check-current-mainnet.sh` | the **full set**, in the order below |
...
| a named-fork literal, a `_mainnet` specialization, or a `_prague` corollary anywhere under `Blanc/` | `scripts/check-fork-containment.sh` | `scripts/check.sh --no-build`, then the **full set**, in the order below |
...
**No gate here takes `--jobs`.** The recorded complete elaboration pass took
about 22 minutes; runtime depends on the host and selected population. The
...
scripts/check-eels-prague-closure.sh
...
scripts/check-fork-containment.sh
```
...
**CI trust boundary.** Production CI executes its 46 registered commands fresh
...
|---|---|---|---|
| `scripts/check-doc-counts.sh` | every public surface that quotes either repository-wide proof count agrees with its source inventory: the gate computes the audited-theorem count from `scripts/AxiomCheck.lean`, the statement-pin count from `scripts/ClaimCheck.lean`, and checks each registered quotation in `README.md`, this file, `docs/index.html`, and `docs/contracts/weth10.html`. Anti-vacuous per pattern — a surface reworded out of the gate's sight FAILS rather than passing silently. Jaune's site quotes the same number and no gate can cross the repository boundary, so a pass prints that reminder rather than implying coverage it does not have | 16 quotations across 4 files (12 theorem-audit and 4 statement-pin); 2 published numbers deliberately unchecked and named in the script | sub-second |
| `scripts/check-fork-containment.sh` | Blanc's proof surface stays stated over an arbitrary valid `ChainConfig`. Two independent static properties over the committed `Blanc/` tree. **Containment:** every identifier whose name begins with a fork this repository can name — `prague*`, `osaka*`, `bpo1*`…`bpo5*`, matched at an identifier boundary, so the rule records (`pragueRules`), the schedule constructor (`ChainConfig.pragueOnly` and a hand-inlined `⟨.prague, 0⟩` alike), the component records (`pragueCodeLimits`, `praguePrecompiles`, `pragueOpcodeRules`, …) and fork-specific lemma names (`pragueOnly_rulesAt`) all count. The population was deliberately widened from the five spellings its originating goal enumerated, because five was measurably too narrow: two generic modules were already binding EIP-170/EIP-3860 limits to `pragueCodeLimits` under a BPO2 lane assertion, and neither was visible. Such an identifier may appear in Lean *code* only inside a module or a single declaration whose allowance in the gate's own source says why naming a fork is its job. The allowance has two levels and the level is the claim: a module allowance says the whole module is a specialization or compatibility module by design — the only two reasons its originating goal's fixed decision 7 authorises; a declaration allowance says one retained corollary, ladder bridge, published specialization, or recorded debt lives inside a module that stays otherwise generic. The category is the word before the colon in each allowance's reason, and the vocabulary is closed and checked, so a third category cannot be introduced by prose alone. Adding a generic module to the module allowance to obtain green is a weakening, not a fix. **Pair symmetry:** every declaration whose fully qualified name ends in `_mainnet` has a `_prague` partner under the same stem, and every `_prague` has its `_mainnet`. Anti-vacuous in five ways: an allowance whose reason opens with a word outside the closed category vocabulary fails before a single file is read; both allowance levels carry an exact count, so a new literal inside an already allowed module or declaration still fails; an allowance that matches nothing fails as an orphan, so a deleted module or renamed declaration leaves no stale blessing; a floor on modules scanned, declarations parsed and pairs found turns a scan that silently reads nothing into a REGRESSION; and code before a module's first declaration belongs to a synthetic `<preamble>` owner that no allowance can name. Comments and docstrings are deliberately outside the scanned text — prose may say `rules := pragueRules` — and `mainnetChainConfig` and the `mainnet*Timestamp` constants remain deliberately outside the detected population — they name a network schedule rather than a fork — named in the script with the reason. It elaborates no Lean, so it cannot know whether an allowed declaration's statement is *really* the corollary it claims to be; that stays a review obligation. **It catches fork *naming*, not fork *dependence*.** The population is a vocabulary of identifiers, so `def a : Nat := 24576` in a generic module passes where `def a : Nat := pragueCodeLimits.maxCodeSize` fails — the same dependence, told and not told; verified by planting each in `Blanc/Ladder.lean`. Blocklisting the value is not the fix, because `Blanc/BeaconDepositCode.lean` legitimately pins it in `eip170RuntimeLimit_exact : eip170RuntimeLimit = 24576`, which is the pin that proves the value. The two live instances of the debt are visible at all only because they also name a fork — `Blanc/BeaconDepositCode.lean`'s `eip170RuntimeLimit` and `Blanc/BeaconDepositDeploy.lean`'s `eip3860InitcodeLimit`, both bound to `pragueCodeLimits`, both asserted by name at BPO2 by `scripts/check-beacon-deposit-current-mainnet.sh`, and both carried as the `debt` declaration allowances the scale cell counts. An inlined fork-sensitive value that names no fork stays a review obligation this gate cannot discharge | 357 modules, 14978 declarations; 151 literals across 7 allowed modules and 9 allowed declarations, of which 2 are recorded debt rather than design; 38 `_mainnet`/`_prague` pairs; `--self-test` runs 4 controls: three in disposable tree copies and one in a mutated copy of the checker itself | sub-second |
...
| `scripts/check-beacon-deposit-assurance.sh` | the Beacon claim map is structurally complete and still agrees with its independent authorities. It pins the exact OPEN-1/P1–P8 row identities and order, per-pillar populations, 11-row/40-declaration totals, all seven non-empty fields, fully qualified declarations, exact axiom expectations from `scripts/AxiomCheck.lean` plus `scripts/check.sh`, exact owning-gate lists, gate existence/catalogue registration, and eight load-bearing non-claim phrases. The default wrapper always runs two in-memory falsifiers: one misspelled declaration and one wrong axiom must both be rejected. It does not elaborate Lean or pretend to judge prose/premise/channel fairness; those remain review obligations | 11 protected rows across 3 pillars; 40 declarations; 8 non-claim pins; 2 mutation controls | sub-second |
...
| `scripts/check-lido-ossifiable-proxy-performance.sh` | validates the immutable result-free 25-cell Prague performance contract, its exact A1–A4/F1–F7/C1–C9/N1–N5 order, symmetric worlds, 13-win threshold, reference/artifact/compiler identities and no-result-smuggling boundary; independently constructs coherent synthetic ledgers and rejects manifest, result, classification, score, lineage, external-identity and threshold corruption, then validates the runner’s evidence construction and its exact launcher/bootstrap/loader-guard/loader-lock identity without executing a measurement | 25 frozen cells; threshold 13 strict wins; 58 campaign hostile cases + 5 runner hostile cases | ~1 s |
...
| `scripts/check-eels-prague-closure.sh` | owns the reference Python environment every Prague differential executes in. The lane's checkout commit pins the specification source while this gate derives the full executed semantic closure under `-I -s -B -X pycache_prefix=/dev/null`: transition and mandatory runtime-package imports, every loaded distribution, the complete executable standard-library tree, native interpreter images, and exact bytes for loaded site-packages source without a RECORD owner. After derivation this gate requires fresh-interpreter admission through the same bootstrap and guard used by consumers, before either platform-policy branch can pass. Before target imports the guard audits startup modules and installs a live finder guard that admits only built-in/frozen interpreter code, non-executing namespaces, trusted checkout/guard source, measured stdlib, exact RECORD owners, exact platform file records, and identity-checked stdlib registry proxies; sourceless bytecode, custom loaders, missing specs, duplicate distribution owners, external source, and newly unowned source fail before their bodies run. Runtime-created Cython/CFFI registry objects are admitted only after the importing extension passed that guard. The committed schema-4 pin holds versions everywhere and exact launcher/distribution/stdlib/runtime-image/unowned-file bytes on generated rows; its constraints output is generator-owned. macOS arm64 is generated. On another platform the version and loader policy still bind, and any unowned startup source must have a generated exact-byte row before execution can proceed | 18 pinned distributions across 769 files plus 736 executable standard-library files, 1 loaded interpreter runtime image and 1 byte-pinned unowned site-packages source on macOS arm64; exact clean checkout; CPython 3.11 series; 47 self-check mutants rejected (41 existing closure falsifiers and 6 bootstrap refusals), plus 2 positive bootstrap controls | sub-second |
...
| `scripts/check-current-mainnet.sh` | verifies Blanc's reusable contract-neutral current-mainnet lane without changing any historical Prague consumer: exact clean target checkout/parent/four-file overlay, native OS/architecture selection, isolated CPython/venv/import/entrypoint identity, the pinned semantic closure of the reference Python environment, literal BPO2 execution module and blob schedule, logical Osaka compiler lineage with the pinned `cancun` testing backend and no external solc, plus a live BLOBBASEFEE canary. Generated rows bind the complete executable standard-library tree as well as distribution bytes and the interpreter executable. The public helper exposes no fork override; unsupported platforms and the explicitly ungenerated Linux x86_64 content row fail closed; no cross-OS uv alias is accepted | 1 BPO2 canary transition; 4 live fork/CLI controls + 1 BPO3 identity control; 19 static semantic/profile mutants + platform-selection controls; 41 runtime-lock/closure self-check mutants; exact target checkout + 45 pinned distributions across 1698 files + 736 executable standard-library files + 1 native interpreter runtime image + 1 byte-pinned unowned site-packages source + native CPython 3.11.9 on macOS arm64 | ~8 s |
...
A complete sequential elaboration gate can exceed 1,000 seconds. The
359-file full verification measured 1,305.5 seconds of elaboration and
1,316.873 seconds for the gate as a whole (about 22 minutes). Runtime depends
on the host and affected population; use the prescribed exclusive admission
and preserve the gate’s existing comparison thresholds.
...
| `scripts/check-proxy-pair-upgrade.py` | `check-proxy-pair-upgrade.sh` | pins the exact proxy-pair upgrade declarations and assurance theorems, generic/product placement, claim tokens, forbidden trust escapes, live axiom sets, and exact executable success/identity/rollback/R2/post-upgrade rows; `--self-test` runs 24 isolated fail-closed controls |
...
| `scripts/test-lido-ossifiable-proxy-performance-runner-static.py` | `check-lido-ossifiable-proxy-performance.sh` | exercises the real runner’s evidence construction against one valid 25-cell synthetic baseline and five hostile mutations without importing EELS target code or taking a measurement |
| `scripts/run-lido-ossifiable-proxy-performance.sh` | explicit Prague measurement only | resolves the pinned EELS interpreter and invokes the shared fail-closed isolated bootstrap before the performance runner; it retains the frozen campaign's exact `PYTHONPATH` environment value, but `-I` ignores that value and the bootstrap derives the executable source root from the clean checkout; no gate or writer calls it implicitly |
| `scripts/run-lido-ossifiable-proxy-performance.py` | explicit immutable Prague result generation and `check-lido-ossifiable-proxy-current-mainnet.sh` | executes the frozen 25-cell campaign and binds every evidence record to exact evaluator, checker, launcher, bootstrap, loader-guard, loader-lock, runner, and schema hashes |
| `scripts/check-lido-ossifiable-proxy-performance-results.py` | explicit immutable performance-evidence verification | validates each 25-cell evidence directory, diagnostic record, exact implementation identity, and baseline/final result lineage without executing a measurement |
...
| `scripts/run-isolated-python.py` | every Prague oracle launcher | the common fail-closed `-I -s -B -X pycache_prefix=/dev/null` bootstrap: CPython removes the entrypoint directory from `sys.path`, so this admits exactly one existing sibling `.py` entrypoint, restores only the repository `scripts/` directory, derives and inserts EELS `src` only from its clean pinned checkout, installs the executed-loader guard before the target import, and gives the target ordinary `__main__` argument semantics. Its exact `EELS_ROOT --check` form runs the same admission and returns without executing a target. Absolute, nested, missing, and non-Python entrypoints reject; every target independently reasserts or requires that same guard before its first EELS import; the explicit historical WETH/fmint writers use this path too |
| `scripts/run-eels-t8n.py` | the historical WETH/fmint fixture writers’ nested t8n subprocesses | requires the shared Prague guard already installed, then runs `ethereum_spec_tools.evm_tools t8n`; the parent writers invoke it only through `run-isolated-python.py`, so a nested oracle process cannot drop the loader policy |
| `scripts/run-current-mainnet-isolated.py` | all four current-mainnet consumer wrappers | the current-mainnet counterpart: under the selected target `-I -s -B -X` interpreter it restores only `scripts/`, verifies the exact checkout and complete native runtime lock, rejects preloaded target packages, installs the live guard, then imports one sibling generator. It uses private infrastructure only and does not expand the five-function public API |
| `scripts/eels_semantic_closure.py` | `check-eels-prague-closure.sh`, all five current-mainnet gates, and every Prague differential's pin check | shared schema validation, derivation, comparison, startup audit, and live import guard for the executed semantic closure. It inventories transition and mandatory runtime imports, exact distribution/native-runtime bytes, the complete executable standard-library tree, and exact loaded unowned site-packages source. RECORD ownership is restricted to the lock's distribution population and rejects duplicate canonical owners. The guard checks every later import spec before its body: built-in/frozen interpreter code, non-executing namespaces, trusted source, exact RECORD owners, measured stdlib, and generated platform file rows are admitted; sourceless bytecode, custom loaders, missing specs, external source, and new unowned files fail closed. It retains its installation boundary and exact contract so a repeated assertion accepts imports that this guard admitted while still rejecting target modules loaded before it. Runtime-created extension side effects are accepted only after a guarded import. It owns no lane's target policy or generated pin |
| `scripts/current-mainnet-runtime-lock.json` | all five current-mainnet gates | the schema-5 reference-environment pin: exact versions for the complete 45-distribution transition and mandatory runtime-tool population, executed-loader and bytecode policies, and generated-platform records for 1698 distribution files, 736 executable stdlib files, 1 native interpreter image, the launcher, and the exact unowned `_virtualenv.py` startup source. macOS arm64 is measured. Linux x86_64 is explicitly ungenerated and fails closed before target execution; no cross-platform byte evidence is inferred or copied |
| `scripts/gen-current-mainnet-runtime-lock.py` | explicit `--write` regeneration and `--check` verification | sole runtime-lock writer; runs the isolated transition and mandatory runtime-tool probe with the shared startup audit/live guard, refreshes the executing platform's distribution, stdlib, native-runtime, launcher, and unowned-site-packages records, and carries another generated platform row forward only while the platform-independent closure and loader policy remain identical — otherwise resetting it to ungenerated |
| `scripts/current_mainnet.py` | `check-current-mainnet.sh`, `check-weth10-current-mainnet.sh`, `check-beacon-deposit-current-mainnet.sh`, `check-prorata-current-mainnet.sh`, and `check-lido-ossifiable-proxy-current-mainnet.sh` | contract-neutral public API (`load_profile`, `resolve_root`, `verify_target`, `target_paths`, `run_t8n`) with no fork parameter; it selects the native profile row, validates exact target and full schema-5 runtime provenance, audits already loaded modules, installs the live executed-loader guard before target imports, sanitizes the child environment, and guards every target execution and signer path |
...
library derived from that interpreter. The lock's two platform rows are
`macos-arm64` and `linux-x86_64`; the pinned versions bind on both, and either
host verifies its own installed bytes while carrying the other row forward. Consumers never add a fork option,
...
5. **Generated artifacts come from their generators**, never from hand editing:
   ...
   `scripts/gen-current-mainnet-runtime-lock.py --write`;
   `scripts/eels-prague-closure.json` and
   `scripts/eels-prague-constraints.txt` from
   `scripts/gen-eels-prague-closure.py --write`; fixtures from
