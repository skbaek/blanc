...
| `scripts/gen-drip-stack-certificate.py` | `check-drip-stack-certificate.sh`; explicit `--write` regeneration | sole writer of `Blanc/DripStackSafetyData.lean`; deterministically derives exact whole-stack patterns from `Blanc/DripCode.lean`, names small subtrees of the unchanged balanced tree and records runtime identity; ordinary mode compares only; generated data supplies no trusted Lean premise |
| `scripts/test-drip-stack-certificate.py` | `check-drip-stack-certificate.sh` | checks both conditional arms, the actual backedge and CALL shape, exact literals/full-stack matching, independent outgoing bounds, high DUP/SWAP cases, byte/branch/row corruptions, and compare-default/write-explicit behavior on disposable inputs |
...
5. **Generated artifacts come from their generators**, never from hand editing:
   ...
   `python3 scripts/gen-drip-stack-certificate.py --write`; its normal mode
   compares the complete generated data and never changes it.
