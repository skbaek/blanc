...
| `scripts/check-drip-current-mainnet.sh --composed-prerequisites` | DRIP's isolated BPO2 consumer first consumes the shared current-mainnet verdict, then runs `gen-drip-fixtures.py --check-runtime` under the verified target interpreter and byte-compares its complete generated JSON population. The committed population was established by the generator's explicit `--write` mode after runtime validation and reproduces under this read-only check. CI replay is owned by `check-drip.sh` | 38 obligation-owned runtime scenarios, including exact block/header/signature/transaction-root and observer/rollback assertions once generated | generated population reproduced |
...
| `scripts/check-drip-current-mainnet.sh` | `check-drip-current-mainnet.sh` | owns the sanitized target launch and direct shared-boundary composition; it invokes only `gen-drip-fixtures.py --check-runtime`, never `--write`, and records no CI replay or arithmetic-evaluator claim |
