...
| `scripts/axiom_audit.py` + `scripts/AxiomAudit.lean` | `check.sh`, `check-proxy-pair-upgrade.sh`, the Lido registry, enumeration, access and history gates, and `check-lido-circuit-breaker-assurance.sh --probe` | the one axiom walker and its one driver. `AxiomAudit.lean` defines `#full_axioms`: per audited name, a fresh visited set over `Environment.find?` (types, definition/theorem/opaque values, inductive constructors), no per-constant result cache, no read of Lean's precomputed per-module axiom sets, one task per name, one `FULL-AXIOMS` line out. `axiom_audit.py` splices that file after an audit source's imports (or builds a probe from imports and names), elaborates it with `lake env lean --stdin`, and requires exactly one report per name. It refuses an audit source that contains `#print axioms` and a walker whose code names Lean's collector, so the under-reporting report (lean4#15226) cannot come back behind the same verdict |
