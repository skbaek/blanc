...
| `scripts/check-extraction-ownership.sh` | the manifest's 14 execution-settlement declarations exist only under their common owners; no donor declaration or common-owner basename shadow survives across the historical WETH10 donor family or the Lido family; all 21 exact compatibility abbreviations are present without drift; no alias/export survives; and `Weth10HolderFlow` directly imports the common module. Its nine negative controls run in `--self-test` | 14 moved declarations; 21 exact abbreviations | ~3 s (was ~26 s with the controls) |
| `scripts/check-extraction-ownership.sh --self-test` | shows every audited channel bites: eight mutations of a temporary copy of `Blanc/` plus the lift manifest (donor alias/export, flow-block removal, attribution-block drift, Lido shadow/alias, missing common declaration, missing direct import) and one trailing-`?` parser probe must each fail with their own tag, after the copy audits clean. The lift manifest and harness are fingerprinted; the copied Lean population is declared as subject-only reads (evidence economy rule 3) | 9 controls | ~28 s |
...
| `scripts/check-extraction-ownership.py` | `check-extraction-ownership.sh` | strictly parses the sole lift manifest, common declaration/import ownership, WETH10/Lido shadow absence, all 21 exact compatibility abbreviations and alias/export absence; under `--negative-controls` (the wrapper's `--self-test`) eight mutation controls on a temporary copy of `Blanc/` plus the lift manifest and one parser control |
...
| `scripts/axiom_audit.py` + `scripts/AxiomAudit.lean` | `check.sh`, `check-proxy-pair-upgrade.sh`, the Lido registry, enumeration, access and history gates, and `check-lido-circuit-breaker-assurance.sh --probe` | the one axiom walker and its one driver. `AxiomAudit.lean` defines `#full_axioms`: per audited name, a fresh visited set over `Environment.find?` (types, definition/theorem/opaque values, inductive constructors), no per-constant result cache, no read of Lean's precomputed per-module axiom sets, one task per name, one `FULL-AXIOMS` line out. `axiom_audit.py` splices that file after a committed audit source's imports, builds a probe from plain module and declaration names, or appends it to a Registry fixture. It then elaborates with `lake env lean --stdin` and requires exactly one report per requested name and none for any other. A committed audit source must hold, comments aside, only imports and `#full_axioms NAME` rows, each once, with no row inside a comment and no report marker; `rows FILE` prints that validated inventory. A fixture may not mention the command, the report marker, the walker's namespace or Lean's collector, and the walker's code may not name the collector. These rules tie each verdict to the walker; they do not sandbox arbitrary Lean in a reviewed fixture file |
