...
| `lake build` | integration elaboration, including the audited compile witnesses, production Lido runtime/constructor artifact family, WETH10 deployment declarations and configured deployment root, stable-state packaging, constructive redemption certificates, and committed holder-flow conservation | 1297 jobs | incremental builds are a few seconds; clean rebuilds are substantially longer |
| `scripts/check.sh --no-build` | axiom audit of the audited top theorems, each against its own pinned expected axiom set; the common rows include the direct spawned-code-address and source-chronology theorems, the proxy-pair rows cover its compiled programs, concrete success/revert executions, correspondence and write authority with biting controls, and the Lido rows include the universal runtime compile equation, source inventories, cycle canary/mutant, Registry mutation bridges, arbitrary-finite enumeration, coherent views, local raw/error/committed observability boundaries, and the complete direct-deployment proof family; the PRORATA rows cover its compile witness and deployed-byte body effects, the exact rounding-direction and residue arithmetic, zero-tolerance preview/actual consistency at both body and deployed-byte altitude, the `ContractSpec` instantiation and genesis-anchored accounting invariant, the eleven realized-accounting rungs with both carrier projections, and the three attack-trace headlines; the Lido TWG rows cover selector-route construction, pinned-target behavior, and the A2 waypoint; BeaconDeposit covers its opening model family and the compiled artifact, success/error/view, complete-write, constructor, and model-bridge flagships | 930 theorems | ~7 s |
| `scripts/check-claims.sh` | Lean-checked statement pins for the common direct spawned-code-address and source-chronology theorems, WETH10 flagships, and the Lido artifact, projection, Registry mutation, arbitrary-finite enumeration, coherent-view, local raw/settled observability, constructor/message/transaction/block, and direct-deployment-root boundaries, plus PRORATA's SF-frozen P3 headlines (genesis-anchored reachable invariant, the pure and realized cumulative-dust identities, and both directions of the realized carrier's non-vacuity against chain reachability) and its three P4 headlines, plus BeaconDeposit's compiled P1–P6 flagships | exactly 288 definitions/statements and constructors | ~2 s |

### Medium — before a commit or push candidate

| gate | proves | scale | time |
|---|---|---|---|
| `scripts/check-elab.sh` | affected-module elaboration time vs the committed `scripts/baseline-elab.txt`; every file is represented, but a file with unchanged recursive local-source and Lake transitive-artifact fingerprints reuses its last successful local measurement | 0–224 measured files; 224 represented; 863.100 s committed baseline | from a few seconds when nothing is affected to ~21 min cache-cold or `--full` historically (latest admission measured all 224 files in 911.1 s against the then-823.6 s committed total, 2026-08-30) |
| `scripts/check-elab.sh --calibrate` | the measurement command for admitting a new row: every module with no committed row, plus a commit-seeded stratified sample of provably-unaffected modules held to the row threshold, refusing at `2.0x` and annotating at `1.5x`. Writes nothing to the baseline and nothing to the local cache | current baseline bands 100/69/38/17; latest admission measured 19 mandatory rows and drew no separate controls because all 205 existing rows were measured outright | latest admission measured 205 affected existing rows + 19 mandatory rows in 911.1 s; ordinary isolated admissions remain a small fraction of a whole-tree pass |
...
| `scripts/current-mainnet-target.json` | `check-current-mainnet.sh`, `check-beacon-deposit-current-mainnet.sh`, and `check-prorata-current-mainnet.sh` | the strict shared profile for literal BPO2 execution, Osaka compiler lineage, exact target/overlay provenance, isolated target paths, blob schedule and live canary; it does not describe or repoint the legacy Prague lane |
| `scripts/current_mainnet.py` | `check-current-mainnet.sh`, `check-beacon-deposit-current-mainnet.sh`, and `check-prorata-current-mainnet.sh` | contract-neutral public API (`load_profile`, `resolve_root`, `verify_target`, `target_paths`, `run_t8n`) with no fork parameter; it validates profile/provenance/runtime identity, sanitizes the child environment, and owns target execution |
...
6. **CI runs a subset of this file**, not a different thing:
   ...
   BeaconDeposit differential, and separately pinned current-mainnet consumers
   remain mandatory full-set local gates. CI retains the target-independent
   committed BPO2 replay through `check-prorata.sh`. Extending CI ownership to
   any external target environment requires a catalogue review.
