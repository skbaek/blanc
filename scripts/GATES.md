...
|---|---|---|---|
| `scripts/check-layering.sh` | contracts are siblings in the import hierarchy: no cross-contract import, no shared module importing a contract, no unclassified module (rule and rationale in `README.md`) | 3 contracts, 28 modules, 26 non-root | sub-second |
...
| `lake build` | integration elaboration, including the two compile witnesses | 935 jobs | ~65 s from a clean Blanc build; incremental rebuilds far less |
| `scripts/check.sh --no-build` | axiom audit of the audited top theorems, each against its own pinned expected axiom set | 98 theorems | ~7 s |

### Medium — before a commit or push candidate

| gate | proves | scale | time |
|---|---|---|---|
| `scripts/check-elab.sh` | per-module elaboration time vs the committed `scripts/baseline-elab.txt` | 28 files, ~112 s of elaboration | ~2 min |
