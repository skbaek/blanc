...
| `scripts/check-execution-occurrence.py` | `check-execution-occurrence.sh` | compiles 13 concrete occurrence controls, runs six diagnostic-pinned Lean mutants, checks exact WETH projection removal, parses nine move owners plus alias/absolute-export/ancestor-relative-export/root-export/missing/wrong-kind controls, and composes the CREATE raw-commit falsifier |
