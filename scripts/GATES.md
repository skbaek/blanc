...
|---|---|---|---|
| `scripts/check-drip-stack-certificate.sh` | byte-compares the deterministic finite stack data against analysis of the current compiler-owned DRIP runtime and runs disposable byte/row/branch/height/post-pattern corruption and named-subtree coverage controls. This is data provenance, not a substitute for the kernel-checked `Blanc.DripStackSafety` certificate in the full build; it invokes no Lean or external evaluator | 735 decoded instructions across 1762 bytes; 18 deterministic test groups | sub-second |
