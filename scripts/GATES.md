...
| `scripts/check-eels-prague-closure.sh` | owns the reference Python environment every Prague differential executes in. The lane's checkout commit pins the specification's own source and nothing else: EELS declares keccak-256, secp256k1 recovery, the BN254/BLS/KZG precompiles, RLP encoding and U256 arithmetic as version *ranges*, so two clean checkouts at the same commit can disagree on a differential result. This gate derives the semantic closure — installed distributions, file-backed standard-library modules, and native interpreter images actually loaded by the isolated specification import under an unreachable bytecode-cache prefix — and holds the live environment against a committed pin: by exact distribution version on every platform, and by exact interpreter-launcher/distribution/standard-library/runtime-image bytes on any platform whose row has been generated there. It renders the constraints file provisioning consumes, so the pin and the install recipe cannot drift apart. Distributions a test-support package's `__init__` chain loads are reported for attribution and deliberately not pinned; they cannot reach a transition. Because the closure is re-derived on every run, a revision that reaches for a new distribution or standard-library module reddens the pin rather than passing unnoticed | 18 pinned distributions across 769 files plus 163 loaded standard-library files and 1 loaded interpreter runtime image on macOS arm64; exact clean checkout; CPython 3.11 series; 14 pin mutants + 1 in-process refusal + 11 shared closure controls rejected | sub-second |
...
| `scripts/eels_semantic_closure.py` | `check-eels-prague-closure.sh`, all five current-mainnet gates, and every Prague differential's pin check | the shared derivation and comparison of a pinned environment's semantic closure: it imports the pinned transition code under an unreachable bytecode-cache prefix, reports which installed distributions, file-backed standard-library modules, and native interpreter images Python actually loaded, digests their exact bytes, and names every version, membership and byte difference individually. Generated rows also bind the interpreter launcher. It owns no lane's entry-point policy and no lane's pin |
| `scripts/current-mainnet-runtime-lock.json` | all five current-mainnet gates | the schema-4 reference-environment pin: every distribution, file-backed standard-library module, and base-prefix native image the isolated transition probe actually loads, plus derivation, exclusion, and bytecode-isolation policy. Distribution versions are platform-independent and bind on every platform; a generated platform row additionally records exact distribution/stdlib/runtime-image bytes and the interpreter launcher. The macOS arm64 row is measured; Linux x86_64 is explicitly ungenerated and fails closed rather than carrying a digest that describes nothing. It deliberately does not pin what a test-support package's `__init__` chain loads |
| `scripts/gen-current-mainnet-runtime-lock.py` | explicit `--write` regeneration and `--check` verification | sole runtime-lock writer; derives the closure from the live target, refreshes the executing platform's distribution, loaded-standard-library, and loaded native-interpreter bytes, and carries the other platform's row forward only while the pinned semantic closure has not moved — resetting it to ungenerated otherwise, because a byte measurement of a closure that no longer exists is not evidence |
