...
| `scripts/check-execution-occurrence.sh` | compiles proof-indexed all-outcome, retained-child, compiler-source, exact-identity and last-writer controls; requires no-op/reverted/later-OOG writes, committed/caught child calls, PUSH-payload rejection and final no-op maximality; reruns the CREATE code-deposit rollback-after-SSTORE control; and requires success-only, raw-pruning, raw-byte, first-writer, identity, WETH-bridge and raw-commit mutants to fail | 11 concrete controls; 7 mutant families | ~11 s |
...
| `scripts/check-execution-occurrence.py` | `check-execution-occurrence.sh` | compiles 11 concrete occurrence controls, runs five Lean mutants, checks the WETH projection-removal mutant, and composes the CREATE raw-commit falsifier |
