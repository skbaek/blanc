...
| `scripts/gen-drip-stack-certificate.py` | `check-drip-stack-certificate.sh`; explicit `--write` data regeneration and `--write-proofs` fixed proof regeneration | sole writer of `Blanc/DripStackSafetyData.lean`, `Blanc/DripStackSafetyRegion576.lean` and `Blanc/DripStackSafetyRegion1022.lean`; deterministically derives exact whole-stack patterns from `Blanc/DripCode.lean`, names small subtrees of the unchanged balanced tree, emits dependency-ordered proof text over the complete table and records runtime identity; ordinary mode compares only. Generated data and proof text supply no trusted Lean premise; separately required Lean builds check every theorem |
| `scripts/test-drip-stack-certificate.py` | `check-drip-stack-certificate.sh` | checks both conditional arms, the actual backedge and CALL shape, exact literals/full-stack matching, independent outgoing bounds, high DUP/SWAP cases, byte/branch/row corruptions, exact Region576 proof reproduction, independently parsed Region1022 topology/bounds/count, full-table proof predicates, fixed-output writer behavior, and compare-default/write-explicit restoration on disposable inputs |
...
5. **Generated artifacts come from their generators**, never from hand editing:
   ...
   `Blanc/DripStackSafetyRegion576.lean` and
   `Blanc/DripStackSafetyRegion1022.lean` proof-text outputs come only from
   the same registered adapter's `--write-proofs` mode. Its normal mode
   compares all three generated surfaces and never changes them. Generated
