...
| `scripts/check.sh --no-build` | axiom audit of the audited top theorems, each against its own pinned expected axiom set | 355 theorems | ~4 s |
...
Twenty-one helpers do the actual work and are not gates in their own right — they are
...
| `scripts/check-execution-occurrence.py` | `check-execution-occurrence.sh` | compiles 13 concrete occurrence controls, runs six diagnostic-pinned Lean mutants, checks exact WETH projection removal, rejects every alias/export command in WETH donors fail-closed, compile-checks absolute/ancestor-relative/root alias and export controls (including attributed public/protected and multiline forms), checks missing owner and wrong kind, then composes the CREATE raw-commit falsifier |
