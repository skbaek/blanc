...
| `scripts/check-execution-occurrence.py` | `check-execution-occurrence.sh` | compiles 13 concrete occurrence controls, runs six diagnostic-pinned Lean mutants, checks exact WETH projection removal, parses nine move owners plus absolute/ancestor-relative/root alias and export controls, missing owner, and wrong kind, then composes the CREATE raw-commit falsifier |
