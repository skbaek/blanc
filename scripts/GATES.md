...
| `scripts/check-prorata-weth-vault-differential.sh --self-test` | perturbs the oracle's offset and all four rounding directions in disposable copies with manifest regeneration, removes the live arithmetic-capacity implementation while retaining its declaration, checks synthetic receipt/rollback false positives and executed return-capture controls, and perturbs the committed measurements and the locked reference runtime identity. The valid-call-as-revert record preserves emitted diagnostics but does not yet require the named status diagnostic | 5 semantic oracle perturbations; 1 executed-ID omission; 1 diagnostic-presence revert probe; 4 receipt/rollback falsifiers; 5 return-capture controls; 2 reference-half corruptions | unmeasured after expansion |
