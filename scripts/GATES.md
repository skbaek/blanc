...
| `scripts/gen-drip-stack-certificate.py` | `check-drip-stack-certificate.sh`; explicit `--write` regeneration | sole writer of `Blanc/DripStackSafetyData.lean`; deterministically derives exact whole-stack patterns from `Blanc/DripCode.lean`, balances their search tree and records runtime identity; ordinary mode compares only; generated data supplies no trusted Lean premise |
