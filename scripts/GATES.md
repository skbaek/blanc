...
| `scripts/current-mainnet-target.json` | `check-current-mainnet.sh`, `check-weth10-current-mainnet.sh`, `check-beacon-deposit-current-mainnet.sh`, `check-prorata-current-mainnet.sh`, `check-lido-ossifiable-proxy-current-mainnet.sh`, and `check-drip-current-mainnet.sh` | the strict shared profile for literal BPO2 execution, Osaka compiler lineage, exact target/overlay provenance, and canonical native macOS arm64/Linux x86_64 uv identities; it does not describe a cross-OS alias or repoint the legacy Prague lane |
...
| `scripts/current_mainnet.py` | `check-current-mainnet.sh`, `check-weth10-current-mainnet.sh`, `check-beacon-deposit-current-mainnet.sh`, `check-prorata-current-mainnet.sh`, `check-lido-ossifiable-proxy-current-mainnet.sh`, and `check-drip-current-mainnet.sh` | contract-neutral public API (`load_profile`, `resolve_root`, `verify_target`, `target_paths`, `run_t8n`) with no fork parameter; it selects the native profile row, validates the full runtime lock and provenance, sanitizes the child environment, and owns target execution |
...
6. **CI runs a registered subset of this file**, not a different thing. The
   ...
   the DRIP current-mainnet consumer are local full-set gates too; CI retains the
   target-independent committed BPO2 replay through `check-prorata.sh` and DRIP's
   target-independent arithmetic/artifact evidence and committed BPO2 replay through
   `check-drip.sh`; the runner must already be built by the compilation owner. Extending
