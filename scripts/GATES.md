...
| `scripts/check-drip.sh` | target-independent DRIP evidence while the family artifact is under construction: byte-compares the sole-writer regeneration of the canonical integer vectors, independently pins the complete 38-obligation inventory and exact constants/boundaries/witnesses, exercises 222,806 arithmetic, 10 rollback/chronology, and 192 randomized coalition-accounting checks, and rejects six vector-schema corruptions. It explicitly claims neither EVM replay nor compiled-byte evidence; those join this same CI-safe family gate when the G2 artifact and G8 fixtures exist | 38 frozen obligations; 222,806 arithmetic checks; 64 accounting traces; 6 schema corruptions | sub-second |
