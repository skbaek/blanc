...
|---|---|---|---|
| `scripts/check-layering.sh` | contracts are siblings in the import hierarchy: no cross-contract import, no shared module importing a contract, no unclassified module (rule and rationale in `README.md`) | 3 contracts, 92 modules, 90 non-root | sub-second |
| `scripts/check-trust-surface.sh` | exact transitive local import closure of `Blanc.lean` contains no new or stale source occurrence of `sorry`, bespoke `axiom`, `opaque`, `@[extern]`, `implemented_by`, `native_decide`, object-level `partial def`, or `dbg_trace`; exact reviewed comment/TacticM/MetaM rows are fail-closed allowlisted; unimported helpers are outside scope until imported | 91 closure modules; 21 exact allowlisted occurrences | sub-second |
...
| `scripts/check.sh --no-build` | axiom audit of the audited top theorems, each against its own pinned expected axiom set | 318 theorems | ~4 s |
| `scripts/check-claims.sh` | Lean-checked exact statement pins for the WETH10 compile, flash-depth, backing, creation, static-certificate, stable-chain, configured deployment root/projections, creation-seed, constructive redemption, full accounted history, dynamic balance-write completeness, committed no-wrap, conservation, residual-floor, and hardened attribution/future-redeemability flagships; constructor pins fail closed on record-field changes or hidden success premises; `Stor.Weth10Inv` is pinned by `rfl` unfolding, and the hardened-attribution computation, the touched-pair list and the projected allowance key's runtime identities are pinned definitionally so a *weakened conclusion term* fails closed too | 165 definitions/statements and constructors | ~2 s |

### Medium — before a commit or push candidate

| gate | proves | scale | time |
|---|---|---|---|
| `scripts/check-elab.sh` | per-module elaboration time vs the committed `scripts/baseline-elab.txt` | 92 files, 498.2 s baseline | ~7.3 min |
