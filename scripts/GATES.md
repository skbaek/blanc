...
| `scripts/check-eels-prague-closure.sh` | owns the reference Python environment every Prague differential executes in. The lane's checkout commit pins the specification's own source and nothing else: EELS declares keccak-256, secp256k1 recovery, the BN254/BLS/KZG precompiles, RLP encoding and U256 arithmetic as version *ranges*, so two clean checkouts at the same commit can disagree on a differential result. This gate derives the semantic closure — installed distributions and file-backed standard-library modules actually loaded by the isolated specification import — and holds the live environment against a committed pin: by exact distribution version on every platform, and by exact interpreter/distribution/standard-library bytes on any platform whose row has been generated there. It renders the constraints file provisioning consumes, so the pin and the install recipe cannot drift apart. Distributions a test-support package's `__init__` chain loads are reported for attribution and deliberately not pinned; they cannot reach a transition. Because the closure is re-derived on every run, a revision that reaches for a new distribution or standard-library module reddens the pin rather than passing unnoticed | 18 pinned distributions across 769 files plus 161 loaded standard-library files on macOS arm64; exact clean checkout; CPython 3.11 series; 11 pin mutants + 1 in-process refusal + 8 closure-comparator mutants rejected | sub-second |
...
| `scripts/eels_semantic_closure.py` | `check-eels-prague-closure.sh`, all five current-mainnet gates, and every Prague differential's pin check | the shared derivation and comparison of a pinned environment's semantic closure: it imports the pinned transition code, reports which installed distributions and file-backed standard-library modules Python actually loaded, digests their exact bytes, and names every version, membership and byte difference individually. Generated rows also bind the interpreter executable that owns built-in/frozen code. It owns no lane's entry-point policy and no lane's pin |
| `scripts/current-mainnet-runtime-lock.json` | all five current-mainnet gates | the schema-3 reference-environment pin: every distribution and file-backed standard-library module the isolated transition probe actually loads, plus derivation/exclusion policy. Distribution versions are platform-independent and bind on every platform; a generated platform row additionally records exact distribution/stdlib bytes and the interpreter executable. The macOS arm64 row is measured; Linux x86_64 is explicitly ungenerated and fails closed rather than carrying a digest that describes nothing. It deliberately does not pin what a test-support package's `__init__` chain loads |
| `scripts/gen-current-mainnet-runtime-lock.py` | explicit `--write` regeneration and `--check` verification | sole runtime-lock writer; derives the closure from the live target, refreshes the executing platform's distribution and loaded-standard-library bytes, and carries the other platform's row forward only while the pinned semantic closure has not moved — resetting it to ungenerated otherwise, because a byte measurement of a closure that no longer exists is not evidence |
