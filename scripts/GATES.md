...
| the PRORATA BPO2 generator, current-mainnet consumer wrapper, or generated fixture population | `scripts/check-current-mainnet.sh` then `scripts/check-prorata-current-mainnet.sh` | the **full set**, in the order below |
...
| `scripts/check-layering.sh` | contracts are siblings in the import hierarchy: no cross-contract import, no shared module importing a contract (rule and rationale in `README.md`) | 6 contracts, 163 modules, 161 non-root | sub-second |
...
| `scripts/check-prorata-current-mainnet.sh` | consumes exactly the shared current-mainnet API, reconstructs the five canonical BPO2 system predeploys from the pinned target, runs all 14 PRORATA blockchain scenarios with explicit non-state-test block semantics, checks receipt status/no-log and integer-model postconditions, constructs canonical BPO2 headers and transaction bodies, then byte-compares every fixture and the manifest without writing. It does not invoke solc, accept a fork argument, repoint `EELS_ROOT`, or evaluate Lean | 14 BPO2 blocks; 131 generation-time assertions; exact byte comparison of 14 fixtures + manifest | ~22 s |
