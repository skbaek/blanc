...
| the DRIP exact-integer model, canonical vectors, or evidence checker | `scripts/check-drip.sh` | the **full set**, in the order below |
...
| `scripts/check-drip.sh` | CI-safe DRIP evidence: byte-compares the sole-writer regeneration of canonical integer vectors; independently pins all 38 obligations and exact constants/boundaries/witnesses; exercises 222,806 arithmetic, 10 rollback/chronology, and 192 randomized coalition-accounting checks; binds the committed runtime and creation literals byte-for-byte; independently executes the exact constructor at four timestamps; and rejects six vector-schema plus six artifact/constructor corruptions. External fixture regeneration is separately registered by `check-drip-current-mainnet.sh`; CI committed-fixture replay remains pending | 38 frozen obligations; 222,806 arithmetic checks; 64 accounting traces; exact 1,917-byte runtime + 2,156-byte creation; 12 corruptions | sub-second |
| `scripts/check-drip-current-mainnet.sh --composed-prerequisites` | DRIP's isolated BPO2 consumer first consumes the shared current-mainnet verdict, then runs `gen-drip-fixtures.py --check-runtime` under the verified target interpreter and byte-compares its complete generated JSON population. The fixture population is currently absent and is established only by the generator's explicit `--write` mode after a successful runtime validation; this row therefore has no green runtime verdict yet. CI retains the target-independent arithmetic/artifact evidence only, while committed-fixture replay remains pending | 38 obligation-owned runtime scenarios, including exact block/header/signature/transaction-root and observer/rollback assertions once generated | unavailable until the generated population exists |
...
6. **CI runs a registered subset of this file**, not a different thing. The
   ...
   target-independent arithmetic/artifact evidence through `check-drip.sh`; DRIP committed-fixture
   replay remains pending. Extending
