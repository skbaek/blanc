...
| `scripts/check-extraction-ownership.py` | `check-extraction-ownership.sh` | strictly parses the sole lift manifest, common declaration/import ownership, WETH10/Lido shadow absence, the sole approved retained-trace export and 21 exact compatibility abbreviations, additional alias/export absence, and nine temp-tree mutation controls plus one parser control |
...
| `scripts/current-mainnet-target.json` | `check-current-mainnet.sh` and `check-prorata-current-mainnet.sh` | the strict shared profile for literal BPO2 execution, Osaka compiler lineage, exact target/overlay provenance, isolated target paths, blob schedule and live canary; it does not describe or repoint the legacy Prague lane |
| `scripts/current_mainnet.py` | `check-current-mainnet.sh` and `check-prorata-current-mainnet.sh` | contract-neutral public API (`load_profile`, `resolve_root`, `verify_target`, `target_paths`, `run_t8n`) with no fork parameter; it validates profile/provenance/runtime identity, sanitizes the child environment, and owns target execution |
...
6. **CI runs a subset of this file**, not a different thing:
   ...
   remain mandatory full-set local gates. The isolated current-mainnet lane and
