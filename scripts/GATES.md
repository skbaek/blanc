...
|---|---|---|
| `scripts/module_path_policy.py` | the proof-recipe and proof-module-size helpers | owns the one raw module-path language and the reject-all filesystem-alias implementation: raw slash splitting precedes path construction; exact NFC Unicode-identifier components, literal prefix/suffix, exact directory spellings, no symlink component/file, link count one, and canonical containment are mandatory. It statically matches every literal policy-call site bidirectionally against `scripts/module-path-dereference-census.json`; its control campaign removes a live census row, attacks all five falsified raw spellings and every explicit site with out-and-back, and covers file/directory symlinks, an external hardlink, wrong case and NFD. Case/normalization aliases that the host cannot express skip closed and are reported |
| `scripts/generate-proof-recipes.py` | `check-proof-recipes.sh` and explicit `--write` regeneration | validates the strict TOML registry, referenced symbols, example anchors and review dates through the shared module-path policy, then deterministically generates `docs/PROOF_RECIPES.md` and the import-safe `Blanc.ProofRecipesGenerated` lookup table through alias-checked output bindings; `--check` byte-compares both outputs and `--self-test` exercises 9 schema/trigger/symbol/anchor/drift controls plus the shared 18-control path campaign |
