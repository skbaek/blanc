...
|---|---|---|---|
| `scripts/check-drip-stack-certificate.sh` | byte-compares deterministic finite stack data and fixed untrusted proof text against analysis of the current compiler-owned DRIP runtime and runs disposable byte/row/branch/height/post-pattern corruption, named-subtree coverage, exact distinct-row reconciliation, exact Region576 reproduction, Region1022/Region1459 topology, cross-pack successor and complete same-frame facade controls. This is data/source provenance, not a substitute for the kernel-checked production certificates in separately required builds; it invokes no Lean or external evaluator | 735 decoded instructions across 1762 bytes; 30 deterministic test groups; 4 fixed proof outputs | sub-second |
...
| `scripts/generate-proof-recipes.py` | `check-proof-recipes.sh` and explicit `--write` regeneration | validates the strict TOML registry, referenced symbols, example anchors and review dates through the shared module-path policy, validates disjoint explicit trigger inventories in core `Tactics` and leaf `ProofRecipeTactic` with final false fallbacks, then deterministically generates `docs/PROOF_RECIPES.md` and the import-safe `Blanc.ProofRecipesGenerated` lookup table, consumed only by the unimported `Blanc.ProofRecipeTactic` leaf, through raw-string, alias-checked output bindings; `--check` byte-compares both outputs and `--self-test` exercises 14 schema/trigger/matcher-ownership/symbol/anchor/drift controls, the shared 15-control path campaign, 4 actual registry/aggregate out-and-back controls, and the optional aggregate wrong-case control |
