...
5. **Generated artifacts come from their generators**, never from hand editing:
   ...
   its explicit `--write-manifest` route; the BeaconDeposit differential
   ...
   `scripts/check-beacon-deposit-current-mainnet.sh --write-manifest`. The Lido deployment-root fixture and
   metadata are intentionally temporary and come only from
