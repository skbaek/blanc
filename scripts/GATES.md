...
| `scripts/check-layering.sh` | contracts are siblings in the import hierarchy: no cross-contract import, no shared module importing a contract, no unclassified module (rule and rationale in `README.md`) | 4 contracts, 105 modules, 103 non-root | sub-second |
...
6. **CI runs a subset of this file**, not a different thing:
   ...
   `check-lido-circuit-breaker-enumeration.sh`, `check-error-data.sh`,
