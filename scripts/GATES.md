...
| `scripts/check-drip.sh --self-test` | shows the DRIP checkers' controls bite: the oracle and artifact checkers' in-memory corruption suites (each validates the committed vectors or artifacts first), and the mocked or synthetic suites for the fixture verifier, replay driver, evaluator transport, receipt protocol, arithmetic protocol, evaluator identity projection and the wrapper's lazy host admission. No Lean child and no host hold; inputs are the harness only, so it reruns when a checker or control changes (evidence economy rule 3) | 2 corruption suites (12 corruptions); 7 mocked suites | ~23 s (measured 2026-09-24) |
...
| `python3 -B scripts/test-elab-migration-comparison.py` | creates disposable Git/Lake/host-identity fixtures and exercises the read-only comparator's positive result plus record, origin, runtime, environment, threshold, schema, and write-safety falsifiers. The production comparison is deliberately outside this row: it reads two explicit clean shared-baseline records after normal genesis and never adopts either record | 23 synthetic CLI controls | unmeasured |
