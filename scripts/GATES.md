...
| `scripts/check.sh --no-build` | axiom audit of the audited top theorems, each against its own pinned expected axiom set; the Lido rows include the universal runtime compile equation, source-inventory counts/cardinalities, cycle canary/mutant, Registry model and exact execution bridges, Registry preservation/caller/corollary results, and narrow constructor artifact/cardinality facts | 426 theorems | ~4 s |
| `scripts/check-claims.sh` | Lean-checked exact statement pins for the WETH10 flagships plus the Lido artifact, projection, Registry-model, source-trace, exact compiled-execution, caller/settlement, and derived membership/cleanup/conservation boundaries | 224 definitions/statements and constructors | ~2 s |

### Medium — before a commit or push candidate

| gate | proves | scale | time |
|---|---|---|---|
| `scripts/check-elab.sh` | affected-module elaboration time vs the committed `scripts/baseline-elab.txt`; every file is represented, but a file with unchanged recursive local-source and Lake transitive-artifact fingerprints reuses its last successful local measurement | 0–102 measured files; 102 represented; 538.3 s full baseline | from a few seconds when nothing is affected to ~9 min cache-cold or `--full` |
