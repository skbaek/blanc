...
| `scripts/check-layering.sh` | contracts are siblings in the import hierarchy: no cross-contract import, no shared module importing a contract, no unclassified module (rule and rationale in `README.md`) | 4 contracts, 102 modules, 100 non-root | sub-second |
...
| `scripts/check-lido-circuit-breaker-enumeration.sh` | compiles the S3 controls; pins normalized headers and exact standard axioms for the seven public enumeration, occurrence, three-view, local-event, target-zero, settlement, and observation roles; exercises exact empty/singleton/64 ABI images, full 64-entry memory readback, order/omission/duplication/truncation, ABI header/size/padding, witness-bound wrap, stack-cursor non-aliasing, the executable reachable-SSTORE mutant, no-op-shaped transitions, event topic/data shape, exact owner/code identity, and rollback visibility; rejects deletion, semantic header, and trust mutations fail-closed | 12 Lean controls; 7 public header pins; 7 exact axiom pins; 6 theorem-level header mutant families; deletion and trust controls | ~3 s |
...
| `scripts/check.sh --no-build` | axiom audit of the audited top theorems, each against its own pinned expected axiom set; the Lido rows include the universal runtime compile equation, source inventories, cycle canary/mutant, Registry mutation bridges, arbitrary-finite enumeration, coherent views, and local raw/settled observability boundaries | 432 theorems | ~4 s |
| `scripts/check-claims.sh` | Lean-checked exact statement pins for the WETH10 flagships plus the Lido artifact, projection, Registry mutation, arbitrary-finite enumeration, coherent-view, and local raw/settled observability boundaries | 230 definitions/statements and constructors | ~2 s |
...
| `scripts/LidoCircuitBreakerEnumerationControls.lean` | the empty/singleton/64, memory/cursor, executable writer, ABI/order/wrap, no-op transition, and event-shape controls |
