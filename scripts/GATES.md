...
| gate | proves | scale | time |
|---|---|---|---|
| `scripts/check-elab.sh` | affected-module elaboration time vs the committed `scripts/baseline-elab.txt`; every file is represented, but a file with unchanged recursive local-source and Lake transitive-artifact fingerprints reuses its last successful local measurement | 0–205 measured files; 205 represented; 823.580 s committed baseline | from a few seconds when nothing is affected to ~21 min cache-cold or `--full` historically (latest admission measured all 205 files in 881.5 s against the then-776.0 s committed total, 2026-08-30) |
