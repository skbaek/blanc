...
| `scripts/check-eels-prague-closure.sh` | owns the reference Python environment every Prague differential executes in. The lane's checkout commit pins the specification's own source and nothing else: EELS declares keccak-256, secp256k1 recovery, the BN254/BLS/KZG precompiles, RLP encoding and U256 arithmetic as version *ranges*, so two clean checkouts at the same commit can disagree on a differential result. This gate derives the semantic closure — the distributions the specification package actually imports, found by importing it and reading what Python loaded — and holds the live environment against a committed pin: by exact version on every platform, and by exact installed bytes on any platform whose row has been generated there. It renders the constraints file provisioning consumes, so the pin and the install recipe cannot drift apart. Distributions a test-support package's `__init__` chain loads are reported for attribution and deliberately not pinned; they cannot reach a transition. Because the closure is re-derived on every run, a revision that reaches for a new library reddens the pin rather than passing unnoticed | 18 pinned distributions across 769 files; exact clean checkout; CPython 3.11 series; 15 pin mutants + 6 closure-comparator mutants rejected | sub-second |
...
| `scripts/check-prorata-current-mainnet.sh --composed-prerequisites` | PRORATA BPO2 fixture and benchmark regeneration after consuming the shared current-mainnet verdict | 14 fixture blocks + 30 benchmark blocks | ~27 s |
| `scripts/check-lido-ossifiable-proxy-current-mainnet.sh --composed-prerequisites` | OssifiableProxy BPO2 replay after consuming the shared current-mainnet verdict | 21 represented primary transaction scenarios | ~25 s |
...
| `scripts/eels_semantic_closure.py` | `check-eels-prague-closure.sh`, all five current-mainnet gates, and every Prague differential's pin check | the shared derivation and comparison of a pinned environment's semantic closure: it imports the pinned transition code, reports which installed distributions Python loaded, digests their exact bytes, and names every version, membership and byte difference individually. It owns no lane's policy and no lane's pin |
| `scripts/current-mainnet-runtime-lock.json` | all five current-mainnet gates | the reference environment's pinned semantic closure: every distribution the pinned transition code imports, named with its exact version, plus the derivation policy and exclusion rules. Versions are platform-independent and bind on every platform; a platform row additionally records the exact installed bytes behind them, and a platform whose bytes have never been measured fails closed rather than carrying a digest that describes nothing. It deliberately does not pin what a test-support package's `__init__` chain loads |
| `scripts/gen-current-mainnet-runtime-lock.py` | explicit `--write` regeneration and `--check` verification | sole runtime-lock writer; derives the closure from the live target, refreshes the executing platform's bytes, and carries the other platform's row forward only while the pinned versions have not moved — resetting it to ungenerated when they have, because a byte measurement of a closure that no longer exists is not evidence |
