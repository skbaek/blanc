...
| Lido CircuitBreaker Registry-history proof owners, their public statements, the dormant Chain owner's recorded pins, or the S7 assurance checker | `scripts/check-lido-circuit-breaker-history.sh` | the **full set**, in the order below |
...
| `scripts/check-lido-circuit-breaker-history.sh` | fail-closed static assurance for the Stage 7 Registry-history family, aimed at a quietly narrowed claim rather than at a broken proof. `RegistryCoherent`, `registrySpec`, `RegistryStable`, `StorFixed` and `Coherent` are a `def`/`structure` layer whose gutting would leave every theorem header byte-identical while making the family vacuous, so the gate pins their whole declaration text and, as a second net, the tokens each body must still mention — a careless re-pin over a gutted body still trips. Coverage is derived, not declared: the seventeen dispatch targets come from `funcs`' own source in `Blanc/LidoCircuitBreaker.lean` under a pinned digest, and the two collection theorems must account for exactly them in program order, each arm read out of the proof's own `rcases` split so an endpoint demoted from proved to assumed stops naming a `*_funcSound` theorem and starts naming a binder. Closed-world narrowing is barred by a POSITIVE allowlist rather than a denylist: every binder of every pinned statement must be a declared data type or match an admissible hypothesis shape, so a pinned callee, a non-reentrancy or direct-call restriction, target honesty, an identification of the post-callback entry list with the entry list, `PauseSuccessNoninterference`, or a premise nobody anticipated fails by default, whatever it is called. Also pins each owner's imports and section variables, requires every section variable to be data rather than a hypothesis, scans for trust escapes with the two reviewed comment rows allowlisted exactly, and probes every public theorem for exactly the standard axioms. The in-flight `Blanc/LidoCircuitBreakerHistoryChain.lean` is dormant behind the single `CHAIN.active` switch: its twelve required public statements and its recorded pins are checked for presence even while dormant, so a pin deleted meanwhile still fails, and `--chain-dry-run` judges those pins against the pinned committed revision without reading the working file. `--static-only` skips the Lean probe, `--self-test` runs the gate's own falsifiers, and `--mutations-dry-run` checks every mutation patch still applies without elaborating anything; the credited campaign is `--mutations --worktree` in an isolated tree | 91 exact pins across 2 active owners; 209 binders past the open-world allowlist; 17 derived dispatch entries against 15 discharged endpoints + 2 Registry-mutating obligations (`pause`, `registerPauser`); 2 reviewed comment trust rows; 19 self-test falsifiers; 5 mutation patches, 3 judged statically and 2 pending; 14 chain pins recorded and PENDING on `Blanc/LidoCircuitBreakerHistoryChain.lean` (`CHAIN.active = False`). Recorded from `--static-only`, `--self-test` and `--mutations-dry-run`; the axiom probe's theorem count is not represented here yet because the normal-mode run needs a completed `lake build` | sub-second without the axiom probe; a few seconds with it |
...
The three WETH10 execution gates and the Lido CircuitBreaker dispatcher and
differential gates additionally require the clean EELS checkout
...
6. **CI runs a subset of this file**, not a different thing:
   ...
   `check-lido-circuit-breaker-history.sh`,
