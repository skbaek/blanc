...
| Lido CircuitBreaker access/temporal-authority proof owners, their public role statements, or the S5 assurance checker | `scripts/check-lido-circuit-breaker-access.sh` | the **full set**, in the order below |
...
| `scripts/check-layering.sh` | contracts are siblings in the import hierarchy: no cross-contract import, no shared module importing a contract, no unclassified module (rule and rationale in `README.md`) | 4 contracts, 116 modules, 114 non-root | sub-second |
...
| `scripts/check-lido-circuit-breaker-access.sh` | compiles the S5 access controls; pins exact public headers and standard-axiom expectations across the six proof owners — `Sites`, `Access`, `Authority`, `OwnerClosure`, `RetainedAuthority`, `Deploy`; checks the AT4 twenty-site classifier's uniqueness, inverse coverage, exact PCs and three-domain separation, the AT2 strict-liveness boundary and interval/canonical-expiry views, the AT3 admin-necessity and checked-extension transitions, AT5 raw all-frame write authority with its permitted-role sets, AT6 owner closure, retained last writer, settlement and the noncommitting negatives, and the constructor's disjoint 2/0/0 effect domain; rejects labelled header mutations, deletion and trust controls fail-closed. The five AT7 registration-chronology modules are inside its trust scan from the day each exists, but none of their headers are pinned yet — `AT7_ROLES` is deliberately empty and carries a tripwire that FAILS if it is populated without being folded into `ROLES` | 18 Lean controls; 52 exact headers and axiom pins across 6 owners; 23 labelled header mutations; deletion and trust controls; 5 AT7 modules scanned, 0 pinned | ~4 s |
