...
| `scripts/check-layering.sh` | contracts are siblings in the import hierarchy: no cross-contract import, no shared module importing a contract, no unclassified module (rule and rationale in `README.md`) | 3 contracts, 94 modules, 92 non-root | sub-second |
| `scripts/check-extraction-ownership.sh` | the manifest's 14 execution-settlement declarations exist only under their common owners, no donor declaration/alias/export survives in a WETH10 module, and `Weth10HolderFlow` directly imports the common module; three built-in temp-tree controls remove each required condition | 14 moved declarations; 3 negative controls | sub-second |
| `scripts/check-trust-surface.sh` | exact transitive local import closure of `Blanc.lean` contains no new or stale source occurrence of `sorry`, bespoke `axiom`, `opaque`, `@[extern]`, `implemented_by`, `native_decide`, object-level `partial def`, or `dbg_trace`; exact reviewed comment/TacticM/MetaM rows are fail-closed allowlisted; unimported helpers are outside scope until imported | 93 closure modules; 21 exact allowlisted occurrences | sub-second |
| `scripts/check-execution-settlement.sh` | compiles the concrete execution-level CREATE code-deposit rollback fixture, requires its constructor SSTORE and branch conditions to evaluate exactly true, proves settlement traversal prunes while the test-only raw-commit traversal retains the storage-writing child, and requires the raw-commit mutant to fail with the retained-frame goal | 1 concrete `Exec.runOk` fixture; 1 raw-commit mutant | ~4 s |
| `scripts/check-execution-occurrence.sh` | compiles proof-indexed all-outcome, explicit child-order, compiler-source, exact-identity and PC-14 last-writer controls; requires terminal-error result evidence, no-op/reverted/later-OOG writes, committed/caught child raw/retained equations, an actual fatal-resume `runErr` retaining its entered child roots with no continuation, exact compiled SSTORE path, PUSH-payload rejection, inhabited owner/code-address filtering and final no-op maximality; parses both occurrence ownership manifests fail-closed, rejects contract-basename shadows, exactly pins the selected-root/`ParentPrefix` source-attribution signature, reruns CREATE rollback after a child SSTORE, and requires success-only, raw-pruning, raw-byte, first-writer, storage/code-identity, commitment-filtered child, unconditional-main-cursor, child/continuation reversal, duplicate-child, continuation-as-frame, commit-required attribution, `runErr` child-pruning, WETH-bridge and CREATE mutants to fail for pinned reasons | 14 concrete evaluator controls; 13 Lean mutants; 1 bridge mutant; 9 moved-owner rows + 9 parser controls; 24 raw-attribution owners + 1 exact signature + 4 controls; 1 CREATE mutant | ~15 s |
...
| `scripts/check.sh --no-build` | axiom audit of the audited top theorems, each against its own pinned expected axiom set | 362 theorems | ~4 s |
...
| `scripts/check-execution-settlement.py` | `check-execution-settlement.sh` | compiles the concrete CREATE rollback-after-SSTORE fixture, pins its dependent child-write witness and evaluator verdict, and requires a raw-retention mutant to fail for the child-frame reason |
| `scripts/check-execution-occurrence.py` | `check-execution-occurrence.sh` | compiles 14 concrete occurrence controls, runs thirteen diagnostic-pinned Lean mutants spanning all-outcome traversal including fatal-resume `runErr`, frame ownership/order, cursor reachability, attribution without commitment and exact identity, checks exact WETH projection removal, rejects every alias/export keyword token outside comments in WETH donors fail-closed, compile-checks wrapped/absolute/ancestor-relative/root alias and export controls (including attributed public and multiline forms), checks missing owner and wrong kind, composes the raw-attribution ownership/signature audit, then composes the CREATE raw-commit falsifier |
