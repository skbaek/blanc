...
|---|---|---|---|
| `scripts/check-drip-stack-certificate.sh` | byte-compares deterministic finite stack data and fixed untrusted proof text against analysis of the current compiler-owned DRIP runtime and runs disposable byte/row/branch/height/post-pattern corruption, named-subtree coverage, exact Region576 reproduction, Region1022 topology and full-table proof-text controls. This is data/source provenance, not a substitute for the kernel-checked production certificates in separately required builds; it invokes no Lean or external evaluator | 735 decoded instructions across 1762 bytes; 27 deterministic test groups; 2 fixed proof outputs | sub-second |
