...
| `scripts/check-lido-twg-pinned-target-current-mainnet.sh --composed-prerequisites` | consumes the registered target and exact compiler-emitted official CircuitBreaker/control-locator gateway runtimes, then replays finite and sentinel composed public pauses plus matching family pause/query chains under literal BPO2. The shell checker independently pins the four-row/five-channel contract, artifact identities, EIP-7825 gas bound, the independently derived EIP-7934 singleton-block RLP bound, applicability-ledger digest and shared runtime-lock digest; the generator reconstructs both tagged storage layouts, exact four-log composition order/data and captured empty/canonical-true outputs before byte-comparing the generated result | 4 scenario rows / 6 state-test transitions; 2 duration arms; 1,000,000 ≤ 16,777,216 transaction gas; 8,435 ≤ 8,388,608 RLP bytes; exact storage/events/outputs; zero semantic mismatches | ~11 s after the evaluator cache is warm |
...
| `scripts/check-lido-twg-pinned-target-current-mainnet.sh` | `check-lido-twg-pinned-target-current-mainnet.sh`; optional composition of the shared boundary | independently pins the scenario/channel/profile/artifact/ledger/runtime-lock contract, evaluates only the compiler-owned runtime pair, and launches the generator under the selected isolated target interpreter |
...
| `scripts/gen-lido-twg-pinned-target-current-mainnet.py` | `check-lido-twg-pinned-target-current-mainnet.sh`; explicit `--write` regeneration | imports exactly the five fork-override-free shared functions, executes the four finite/sentinel family/composed rows as six singleton transitions, rejects any storage/event/output mismatch, binds the applicability ledger and two-platform runtime lock, and byte-compares the committed result by default |
| `scripts/fixtures/lido-twg-current-mainnet/results.json` | `check-lido-twg-pinned-target-current-mainnet.sh` | generated finite BPO2 replay evidence for the exact compiler-owned artifact pair; never a theorem premise and never a live-chain role/state attestation |
