...
| the reusable current-mainnet profile/helper, its BPO2 canary, target checkout, isolated Python environment, or a new current-mainnet fixture consumer | `scripts/check-current-mainnet.sh` | the **full set**, in the order below |
...
| `scripts/check-current-mainnet.sh` | verifies Blanc's reusable contract-neutral current-mainnet execution lane without touching any historical Prague consumer: a distinct `JAUNE_T8N_TARGET`, exact clean checkout/parent/four-file overlay identity, isolated CPython/venv/import/entrypoint identity, selected `ethereum.forks.bpo2` module, BPO2 blob schedule, logical Osaka compiler lineage with EEST's `cancun` backend mapping and no external solc invocation, then executes a BLOBBASEFEE storage canary at fixed parent excess blob gas. The public helper exposes no fork override; live Prague/Osaka/BPO1/missing-fork controls and static profile/environment mutants prove that the BPO2 claim is observable rather than a label. BPO3 is rejected separately at the explicit identity boundary because this checkout gives its relevant schedule the same canary result as BPO2. This is BPO2 under the pinned mainnet schedule; Glamsterdam/Amsterdam remains a separate pre-mainnet lane | 1 BPO2 state transition; 4 live fork/CLI controls + 1 BPO3 identity control; 16 static semantic/profile controls; exact target checkout + venv/site-packages + CPython 3.11.9 standard-library fingerprint | ~5.5 s |
...
| `scripts/current-mainnet-target.json` | `check-current-mainnet.sh` and every current-mainnet consumer | the single strict profile for execution fork BPO2, selected module and blob schedule, logical Osaka compiler lineage, exact target/upstream/overlay provenance, isolated target paths, canary vector, and live fork-falsifier inventory; it does not repoint or describe the legacy Prague oracle |
| `scripts/current_mainnet.py` | `check-current-mainnet.sh` and every current-mainnet consumer | importable contract-neutral API (`load_profile`, `resolve_root`, `verify_target`, `target_paths`, `run_t8n`) with a literal BPO2 command and no public fork parameter; validates profile/provenance/runtime identity, sanitizes the child environment, owns target execution, and runs static/live falsifiers |
| `scripts/check-current-mainnet.sh` | `check-current-mainnet.sh` | independently owns the expected BPO2/profile/provenance/canary/falsifier literals, resolves only `JAUNE_T8N_TARGET` or its declared default, and starts the selected venv interpreter in an empty environment |
| `scripts/gen-beacon-deposit-current-mainnet.py` | `check-beacon-deposit-current-mainnet.sh` | sole BPO2 manifest writer; imports exactly the five public current-mainnet functions, executes the two creation and fourteen runtime transactions, validates canonical raw receipts/logs and exact side-qualified state, derives size and gas comparisons, enforces completed positive-gas markers, fingerprints direct inputs, and runs bounded inventory/API/channel/manifest falsifiers |
...
Current-mainnet consumers do not copy the wrapper or add a fork option. Proxy
and PRORATA fixture gates import `scripts/current_mainnet.py`, call
`load_profile`/`verify_target` and `run_t8n`, and register their own consumer
files plus the shared profile/helper, `JAUNE_T8N_TARGET`, exact `t8n_target`
external pin, target venv/entrypoint/site-packages, and CPython runtime inputs.
Where compilation is relevant, “Osaka” is the logical compiler lineage; this
profile records EEST's current `cancun` backend lowering and does not claim an
installed solc accepts an `osaka` flag. Existing Lido and TWG Prague fixtures
remain historical evidence under `EELS_ROOT`; an optional current-mainnet
consumer is a new additive gate using this API and must never relabel or
repoint the committed Prague fixture. Amsterdam/Glamsterdam uses a different
pre-mainnet profile rather than weakening this BPO2 baseline.
...
5. **Generated artifacts come from their generators**, never from hand editing:
   ...
   documented `--write-manifest --manifest-only` mode; the BeaconDeposit
   differential manifest comes only from
   `scripts/check-beacon-deposit-differential.sh --write-manifest`; the
   BeaconDeposit BPO2 manifest comes only from
   `scripts/check-beacon-deposit-current-mainnet.sh --write-manifest`. The Lido deployment-root
...
6. **CI runs a subset of this file**, not a different thing:
   ...
   and the separately pinned current-mainnet lane remain mandatory full-set
   local gates: extending CI ownership to those target environments or
