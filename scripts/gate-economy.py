...
def validated() -> tuple[list[dict[str, Any]], dict[str, dict[str, Any]], dict[str, str], dict[str, Any]]:
    ...
    if len(gates) != economy["launch_population"] + len(economy["split_families"]):
        raise EconomyError("final population does not equal launch plus the five split rows")
...
def render() -> str:
    ...
    lines += [
        ...
        "",
        "- Five launch composite rows retain their static halves at the original positions and",
        "  add semantic halves at positions 54–58: "
        + ", ".join(f"`{item}`" for item in economy["split_families"]) + ".",
        "  The final population is therefore 53 + 5 = 58; the required conjunction did not shrink.",
        "- CI makes the same five splits, so its registered command population moves from 34",
        "  to 39. The prerequisite column also records nested launch composition now represented",
        "  by runner-enforced dependencies that consume exact earlier green evidence.",
