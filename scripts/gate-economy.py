...
def ci_population() -> int:
    ...
    for raw in (ROOT / ".github/workflows/ci.yml").read_text(encoding="utf-8").splitlines():
        ...
        if line.startswith("run:"):
            line = line[4:].strip()
        words = line.split()
        if words and words[0].startswith("scripts/check") and words[0].endswith(".sh"):
            commands.add(line)
    return len(commands)
...
def render() -> str:
    ...
    lines += [
        ...
        "",
        "- Five launch composite rows retain their static halves and add semantic halves:",
        "  " + ", ".join(f"`{item}`" for item in economy["split_families"]) + ".",
        "- Post-launch rows are " + ", ".join(f"`{item}`" for item in economy["post_launch_gates"]) + ".",
        f"  The final population is {economy['launch_population']} + "
        f"{len(economy['split_families'])} + {len(economy['post_launch_gates'])} = {len(gates)}.",
        "  Existing material-output certificates and positive/mutation obligations are retained.",
        "- The CI census includes `scripts/check.sh --no-build` and the proof-recipe,",
        "  proof-module-size, and Lido TWG census rows. Containment follows layering;",
        "  Prague closure follows constrained EELS installation before deployment replay.",
        f"  Its exact registered command population is {economy['final_ci_population']}.",
        "  Current-mainnet and full differential rows remain local/full-catalogue.",
        "  Prerequisites consume exact earlier green evidence.",
