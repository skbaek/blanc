...
#
# `scripts/check-execution-occurrence.py` is a **frozen predecessor assurance file** of the
...
case " $* " in
  *" --static-only "*) ;;
  *) gate_semaphore_acquire "the execution occurrence fixtures" || exit 2 ;;
esac

# Not `exec`: the EXIT trap that releases the hold has to survive the driver.
python3 scripts/check-execution-occurrence.py "$@"
