...
def keccak256(data: bytes) -> bytes:
    """Return the 32-byte Ethereum Keccak-256 digest of ``data``."""

    # Copy before padding so the caller's input is never mutated.  The final
    # pad bit must merge into the domain byte when data ends one byte short of
    # the rate; appending a separate 0x80 byte would add a spurious block.
...
def selector(signature: str) -> bytes:
    """Return the four-byte selector of a canonical ABI signature."""
