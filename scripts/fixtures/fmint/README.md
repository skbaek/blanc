...
execution evidence (per the fmint code plan; program source of truth the
flash-mint proposal), the sibling of the
[WETH fixture suite](../weth/README.md) (the WETH evidence plan): every
...
inputs, not a proof. fmint's conservation invariant (the flash-mint
proposal's headline 1) **is proved preserved**, from a state where it
...
witness that it did. `base+2` and `base+3` are the fmint-hygiene plan's Step 2's
...
(the fmint-hygiene plan's Step 1), so every guard failure is a clean,
...
requirement (the fmint-hygiene plan's fixed decision 7): the assertions
...
the fmint-hygiene Step 2 report, a planning record outside this repository.
...
never `decide +kernel` — see the kernel-decidability note). No case in this
