...
WRAPPER_CACHE_OWNERSHIP="the shared runtime lock owns the reference environment's semantic closure: exact versions on every platform and exact distribution plus loaded-standard-library bytes on each generated row; the Linux row is explicitly ungenerated and fail-closed; the gate registry additionally fingerprints the selected exact checkout, site-packages population, and selected CPython 3.11.9 standard library"
