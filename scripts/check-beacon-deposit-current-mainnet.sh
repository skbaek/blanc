...
WRAPPER_CACHE_OWNERSHIP="the shared runtime lock owns the reference environment's semantic closure: exact versions on every platform and exact installed bytes on each platform whose row has been generated there; the gate registry additionally fingerprints the selected exact checkout, site-packages population, and CPython 3.11.9 standard library"
