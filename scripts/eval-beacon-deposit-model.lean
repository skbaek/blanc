...
-- outputs under the oracle's keccak-256 and SHA-256 hash instantiations, for
...
-- (compiler/interpreter evaluation); no kernel-level decision procedures are
-- involved.
--
...
def Hs : Bytes → B256 := Bytes.sha256
...
def emitZeroHashes (H : Bytes → B256) : IO Unit := do
  for h in List.range 33 do
    IO.println s!"zero_hash {h} {hex32 (zeroHash H h)}"

def emitLe64 : IO Unit := do
  for n in le64Samples do
    IO.println s!"le64 {n} {(le64 n).toHex}"

def emitRoots (H : Bytes → B256) : IO Unit := do
  for n in rootCounts do
    match chain H n with
...
def emitDepositCases (H : Bytes → B256) : IO Unit := do
  let mut s := Acc.empty
  for i in List.range 6 do
    let c := depositInputs i
    let amountLE := le64 (c.value / oneGwei)
    let node := depositDataNode H c.pubkey c.wc c.sig amountLE
    IO.println s!"deposit_inputs {i} {c.pubkey.toHex} {c.wc.toHex} {c.sig.toHex} {c.value}"
    IO.println s!"deposit_encoding {i} {amountLE.toHex} {hex32 (pubkeyRoot H c.pubkey)} {hex32 (signatureRoot H c.sig)} {hex32 node}"
    match deposit H s c.pubkey c.wc c.sig node c.value with
    ...
    | .ok (s', ev) => do
        ...
        IO.println s!"deposit_after {i} {s'.count} {hex32 (Acc.root H s')}"
        s := s'
...
def emitBoundary (H : Bytes → B256) : IO Unit := do
  for c in [2 ^ 32 - 3, 2 ^ 32 - 2] do
    let ok := (Acc.insert H ⟨fun _ => 0, c⟩ (leaf 0)).isSome
    IO.println s!"insert_at_count {c} {ok}"
  let okAtCap := (Acc.insert H ⟨fun _ => 0, 2 ^ 32 - 1⟩ (leaf 0)).isSome
  IO.println s!"insert_at_count {2 ^ 32 - 1} {okAtCap}"
  let walkFalls := (walk H (fun _ => 0) 32 0 (2 ^ 32) (leaf 0)).isNone
...
def emitRegime (name : String) (H : Bytes → B256) : IO Unit := do
  IO.println s!"eval_beacon_deposit_model {name}"
  emitZeroHashes H
  emitLe64
  emitRoots H
  emitBranchStates H
  ...
  emitBoundary H
  emitErc165
  IO.println s!"eval_regime_done {name}"

def emitAll : IO Unit := do
  emitRegime "keccak256" Hk
  emitRegime "sha256" Hs
