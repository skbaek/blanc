...
and abstraction evidence for the frozen storage layout remain pending until
their named compiled proofs exist. Artifact identities and the complete
runtime and constructor differential/gas matrices are measured.
...
`abiDepositEvent_mk` and `abiDepositEvent_length`. The exact-event differential
rows and live log-channel falsifier are green. **`[PENDING]`** the compiled LOG
and raw-versus-settled chronology theorems.
