...
- **Premises:** no external run premise. The two closed worlds install the production CircuitBreaker runtime and the compiler's exact gateway runtime, configure only their explicit role, Registry, time and storage data, and construct the complete public-pause executions. The finite world uses the derived `107635`-gas schedule; the separately executed infinite world follows the sentinel arm and stores `2^256 - 1` itself. The real child `CALL` and warm query `STATICCALL`, callback noninterference, committed outcomes and TWG-2 conclusion all come from the walks. No literal runtime byte string, evaluator result or `Nonempty` assumption is reflected into a theorem.
- **Axioms:** `propext`, `Classical.choice`, `Quot.sound`
