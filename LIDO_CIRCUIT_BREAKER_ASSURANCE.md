...
- **Premises:** exactly three. **(1)** a canonical deployment base — twenty-one pre-execution fields including that the target address is the address computed from the sender and its nonce, and four system-code equalities. The base is now the contract-neutral configured shell instantiated at `ChainConfig.pragueOnly`/`pragueRules`; the five fields it gained over the pre-hoist shell — schedule validity and selected-rules non-precompile membership for the four protocol system addresses — are provable outright at Prague and weaken nothing here. **(2)** the canonical official envelope — the block's transaction list is the single transaction, no ommers, no withdrawals, a type-2 transaction on this chain, zero value, data exactly the official full CREATE input, with recovered, validated, and checked sender and the gas and funding bounds. **(3)** `hstep`, the successful configured Prague-only transition — **the theorem's only premise about the named deployed chain**. Everything else — the prepared message, the collision check, the constructor, message and transaction results, receipt success, poststate, installed code, witness, and stability — is **derived, not assumed**. The root record itself is a structure outside the axiom audit and is named here as vocabulary.
- **Axioms:** `propext`, `Classical.choice`, `Quot.sound`
...
- **Source:** `reports/lido-circuit-breaker-deployment-root.md`; `Blanc/LidoCircuitBreakerDeploymentRoot.lean:50-60`
