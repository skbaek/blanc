...
| 6 | `A = U` denominator | checked `A + 1` reverts on overflow | exact 257-bit `A + 1` | the vault still quotes at the word ceiling instead of reverting. Because the exact WETH credit wraps modulo 2^256 (SF Amendment A2, half 1), a nonzero `deposit` from `S = 0, A = U` executes on Blanc, mints 0 shares and wraps the vault's WETH row `U → 9`, where the reference reverts. That world violates WETH's solvency invariant (`State.Inv`: booked rows are covered by WETH's ETH balance, which is below 2^256), which `pair_history_backed` proves at every configured pair state under the D9 premise; the row exercises the arithmetic route, not a reachable history | the standard's converters are required not to revert; wrapping would be worse | oracle: `A = U` is one of the boundary states in every battery; differential: `check_a_u_zero_flows` and `check_a_u_nonzero_{deposit,mint,withdraw,redeem}` (`scripts/check-prorata-weth-vault-differential.py`) execute on Blanc while the reference reverts with `Panic(0x11)` |
...
row. The compiled-reference differential establishes agreement on its boundary
matrix — every one of the 25 selectors executed on both compiled sides, at the
empty, nonempty and donated states and the registered capacity boundaries
(`scripts/check-prorata-weth-vault-differential.py`) — and nowhere it does
not execute: finite evidence on a stated corpus, never a proof.
