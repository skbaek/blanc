...
|---|---|---|
| Operational event delivery, reorg/finality handling, off-chain monitoring, and arbitrary-history event monitoring | Arbitrary-finite exact-code Registry enumeration, coherent three-view snapshots, the local `PauserSet` state/event suffix, top-level error log erasure, and configured-chain Registry-history induction are proved. They do not establish provider delivery, event-stream completeness, monitoring availability, reorg policy, or finality. | Operational monitoring/finality work. |
| Callback-time count/expiry coherence, target truth, unconditional exact successful-pause final state, and public-`pause` composition | All-frame persistent-write/access classification and settled-effect attribution are proved, and Registry coherence is preserved under arbitrary callee bytecode and same-instance descendant reentry. The history theorem deliberately preserves an existential Registry witness; it does not claim the old entry list or live-expiry/count relation survives an intermediate callback boundary, discharge the successful-pause noninterference premises, establish target truth, or compose the full public pause result. | Focused successor proofs if these stronger observations are required. |
| Arbitrary deployment shapes | One exact official direct, zero-endowment, singleton type-2 Prague deployment now establishes the configured root and every reachable Registry-history consequence. Constructor compatibility and the clone namespace remain finite evidence; co-blocks, factory/proxy/clone/CREATE2 creation, arbitrary parameters, nonzero endowment, arbitrary block shapes or forks, signing, propagation, and historical inclusion are not covered. | Separate deployment goals if desired. |
