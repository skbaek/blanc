...
- Registered symbols: `module:Blanc/MessageExecution.lean`, `module:Blanc/MessageExecutionInversion.lean`, `module:Blanc/StaticPrecompileMessage.lean`, `declaration:MessageExecution.processMessage_eq_settle_exec_of_enter`, `declaration:MessageExecution.frameEnter_eq_run_afterTransfer_of_notPrecompile`, `declaration:MessageExecution.processMessage_eq_settle_exec_afterTransfer_of_notPrecompile`, `declaration:MessageExecution.processMessage_eq_settle_exec_afterTransfer_of_noCodeAddress`, `declaration:MessageExecution.processMessage_eq_settle_exec_afterTransfer`, `declaration:MessageExecution.processMessage_eq_settle_exec`, `declaration:MessageExecution.processMessage_clean_of_exec_afterTransfer`, `declaration:MessageExecution.processMessage_eq_settle_exec_afterTransfer_of_codeEntry`, `declaration:MessageExecution.processMessage_clean_of_exec_afterTransfer_of_codeEntry`, `declaration:MessageExecution.executeCode_enter_of_codeAddress_not_precompile`, `declaration:MessageExecution.processMessage_clean_of_exec`, `declaration:MessageExecution.processMessage_revert_of_exec_afterTransfer`, `declaration:MessageExecution.processMessage_revert_of_exec`, `declaration:MessageExecution.processMessage_halt_of_exec_afterTransfer`, `declaration:MessageExecution.processMessage_halt_of_exec`, `declaration:MessageExecution.processMessage_clean_rawPost`, `declaration:MessageExecution.processMessage_entry_facts`, `declaration:MessageExecution.processMessage_entry_stack`, `declaration:MessageExecution.processMessage_entry_memory`, `declaration:Blanc.stor_of_processMessage_staticPrecomp`, `declaration:Blanc.gasSha25664_le_of_processMessage_clean`, `declaration:Blanc.output_of_processMessage_sha256_64_clean`, `declaration:Blanc.frame_of_processMessage_sha256_64_clean`, `declaration:MessageExecution.settledRevert`, `declaration:MessageExecution.settledHalt`, `declaration:Msg.initDevm_stack`, `declaration:Msg.initSevm_data`
- Review: `proof-infrastructure` on `2026-09-01`
...
- Preferred path: A long-lived memory word that must survive unrelated scratch traffic is carried by the shared proof-carrying window in `Blanc/MemoryImage.lean`; import it with `import Blanc.MemoryImage` and do not declare a contract-local survival predicate. Use `MemImage` while the proof still needs the whole reader image — it keeps `Mem.Wf` beside the image so the write algebra never re-derives it — and drop to `MemWordAt` as soon as only one 32-byte window matters, because its existential image is what keeps a large scratch region out of every downstream goal; `MemWordAt.of_memImage` and `MemWordAt.memImage` move between them. Establish a window with `MemWordAt.of_write` or `of_run_mstoreAt_mem` and read one back onto the stack with `prefix_of_loadWord_window`. Transport it with `writeMiss`/`writeMissBytes` for a missing write, `extend`/`extends` for logical extension, `extendsWrite` for the combined CALL-resume shape, `acrossLine`/`acrossNinst`/`acrossMload`/`acrossLoadWord`/`acrossMstoreAt`/`acrossLogWith` for lines and instructions, and `acrossStaticcall`/`acrossSuccessfulCall` across a call boundary from the raw `Ninst.Run` and its operand stack. For a scratch trace confined below a fixed boundary, state the frame once as `Bytes.WordFrameFrom` and apply it with `MemWordAt.of_wordFrame`.
- Boundary: Every theorem here is frame-shaped: it carries an already-known window across a step and proves nothing about what that step computed. The disjointness side condition is always the caller's explicit premise — `writeMiss`, `writeMissBytes` and `extendsWrite` each take `offset + 32 ≤ n ∨ n + len ≤ offset`, and the call-boundary lemmas take `outputOffset + outputSize ≤ offset` — so a contract's layout argument belongs at the call site and is deliberately not automated. The module supplies no multi-region layout, footprint or staging algebra, and no automation that discharges region disjointness; a proof needing those still writes them by hand. These are small structural proofs and do not justify a raised `maxRecDepth` or `maxHeartbeats` scope: the expensive part of a memory proof is the surrounding walk, which is the reason to compose this carrier instead of re-proving transport inside the walk. The carriers were hoisted unchanged out of the Lido CircuitBreaker proofs and nothing about them is Lido-specific.
...
- Registered symbols: `module:Blanc/MemoryImage.lean`, `declaration:Blanc.MemImage`, `declaration:Blanc.MemImage.write`, `declaration:Blanc.MemWordAt`, `declaration:Blanc.MemWordAt.of_memImage`, `declaration:Blanc.MemWordAt.memImage`, `declaration:Blanc.MemWordAt.of_write`, `declaration:Blanc.MemWordAt.writeMiss`, `declaration:Blanc.MemWordAt.writeMissBytes`, `declaration:Blanc.MemWordAt.extend`, `declaration:Blanc.MemWordAt.extends`, `declaration:Blanc.MemWordAt.extendsWrite`, `declaration:Blanc.MemWordAt.acrossLine`, `declaration:Blanc.MemWordAt.acrossNinst`, `declaration:Blanc.MemWordAt.acrossStaticcall`, `declaration:Blanc.MemWordAt.acrossSuccessfulCall`, `declaration:Blanc.Bytes.WordFrameFrom`, `declaration:Blanc.MemWordAt.of_wordFrame`, `declaration:Blanc.prefix_of_loadWord_window`, `declaration:Blanc.of_run_mstoreAt_mem`
...
- Preferred path: Absence of an operand-stack fault over fixed compiled bytes is a checked finite table, not a hand-rolled per-contract height counter. Import the chain with `import Blanc.AbstractStackCertificate`, write one `Table` row per reachable program counter as a whole-stack `Pattern` of `Option B256` (`none` forgets a literal but never an operand position), and discharge the whole obligation with `checkTable_certificate` applied to a single `decide` of `checkTable code table maximum = true`. The four declarations at the end of `Blanc/AbstractStackCertificate.lean` — `exampleCode`, `exampleTable`, `exampleTable_checked`, `exampleTable_certificate` — are the complete minimal use. Transport the resulting `Certificate` over the actual same-frame chronology with `Certificate.parentStep`, `Certificate.parentPrefix` and `Certificate.at_parentPrefix`; they reuse `Exec.Deriv.ParentPrefix` rather than introducing a parallel execution relation. Compose independently checked pieces with `Table.all_node` instead of enlarging one tree, and reach for `Table.count_le_one` or `Table.checkLayout` only when row uniqueness or a contiguous layout is separately wanted. `docs/COMMON_API.md` section E9 is the need-first entry point.
- Boundary: The conclusion is local and same-frame. It states that every reached same-frame node satisfies the checked invariant and that no step generates a `StackFault`; it proves nothing about gas, liveness, termination, whether any program counter is reached, or the code of a spawned child frame. A CALL's child is arbitrary: only the parent's resumption is covered, and a stack fault arriving through the child settlement is attributed by `InheritedStackFault`, not excluded. The accepted family is narrow and fails closed to `false` on everything else — `regularTransfer` covers ADD, MUL, SUB, DIV, LT, GT, EQ, ISZERO, AND, SHR, CALLER, CALLVALUE, CALLDATALOAD, CALLDATASIZE, TIMESTAMP, POP, MLOAD, MSTORE, SLOAD, SSTORE, GAS, DUP and SWAP; CALL is the only accepted external instruction; SELFDESTRUCT is rejected. `checkTable` hard-caps the stack ceiling at `maximum ≤ 8`, which is the limit of the proved transfer family rather than a tuning knob: raising it requires new transfer theorems, not a larger literal. Validation is one kernel `decide` over the tree, so cost grows with row count and pattern width and a very large table should be split with `Table.all_node` rather than checked whole. The tree shape supplies no trusted premise — a wrong tree fails `Table.checkOrder` or a row check instead of weakening the theorem. Finally, the checker validates a table and does not produce one: there is no row synthesis, candidate generation or coverage measurement here, and authoring the rows for a real program remains the caller's work.
