...
- Preferred path: Use `func_execute n` or `func_execute_with line` to expose a known prefix. For a successful conditional whose selected arm calls a known nonreturning auxiliary, use `of_run_branch_call_of_not_run`; the `rev`, `revWith`, and `revReturnData` specializations discharge the standard reverter bodies.
...
- Registered symbols: `tactic:func_execute`, `tactic:func_execute_with`, `declaration:Func.Run`, `declaration:Blanc.of_run_branch_call_of_not_run`, `declaration:Blanc.of_run_branch_call_rev`, `declaration:Blanc.of_run_branch_call_revWith`, `declaration:Blanc.of_run_branch_call_revReturnData`, `declaration:Blanc.Func.not_run_revReturnData`
- Review: `proof-infrastructure` on `2026-08-20`
