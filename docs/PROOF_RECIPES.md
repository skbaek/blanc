...
- Boundary: This destructive cleanup removes the state and every local fact that depends on it. Check whether a completed continuation summary already discharges the goal, and transport every fact that must survive. The context-count heuristic is only a prompt, never an automatic cleanup command; it cannot establish whether summary availability is semantic. Experimental history: Blanc commit 0eee78d571e673f37543e4d306608af445065017, `scripts/proof-recipes.toml`, recipe `state-context-cleanup`.
- Owner module: [Blanc/Tactics.lean](../Blanc/Tactics.lean)
- Canonical example: [Blanc/Conserved.lean](../Blanc/Conserved.lean) — `Fmint.of_prepApprove`
