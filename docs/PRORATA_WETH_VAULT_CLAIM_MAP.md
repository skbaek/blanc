...
| `mint` and `withdraw` as accounting steps | A **registered modelling incompleteness**, not a missing proof and not an unsoundness: they are quoted in the inverse direction, `ProrataAccountingEffect.deposit` pins its mint by equality, and `convertToShares (previewMint s) = s` is not an identity, so neither can be exhibited as a carrier step without an inverse-quoted class — which would change a model `Blanc/ProrataAccounting.lean` shares with PRORATA, whose proofs case-split on its constructors. The *direction* of the slack is proved: `mint_never_overmints` and `withdraw_never_overpays` (`Blanc/Composition/ProrataWethVaultBacking.lean`, audited) show the inverse quote rounds the charge up and the forward quote rounds the mint down, so both compose in the pool's favour. The composition (`SteppedMessages.toPath`) is indifferent to which class a message was, so the gap blocks only these two messages |
...
| "Finite OpenZeppelin/oracle evidence supports conformance" | Carried as finite evidence, never as a theorem: `scripts/check-prorata-weth-vault-oracle.sh` (11 property batteries over 63 boundary states, offset-disabled control biting), `scripts/check-prorata-weth-vault-reference.sh` (the vendored OpenZeppelin v5.7.0 closure, compiler input/output and ABI surface identity-checked against the G1 hashes) and `scripts/check-prorata-weth-vault-differential.sh` (11 cases each of the committed runtime and of the constructor-patched reference on Jaune, both agreeing with the oracle; sizes and gas recorded). All three carry biting self-tests |
